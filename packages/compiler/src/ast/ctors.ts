 ...
 *
 * Constructor facts (owning type, arity, field type exprs, runtime keys) used to be derived independently three times: `check.ts`'s registry (arity + exhaustiveness), `infer.ts`'s ctor loop (schemes), and `codegen.ts`'s ctorKeys (runtime destructure keys) — drift between them was caught by nobody's tests. This module is the single source: `check` validates through it, `infer` builds ctor schemes from its entries, and `codegen` reads its keys.
 */
import { err, ok, type Result } from "@onrails/result";
import { checkErr, type Diagnostic } from "../errors/errors";
import { builtinTypeDecls } from "../prelude/prelude";
...
/** Constructor lookup tables: ctor name → facts; type name → ctor names. */
export type CtorTable = {
  ctor: Map<string, CtorEntry>;
