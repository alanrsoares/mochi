...
await match(cmd)
  .with("fmt", async () => {
    ...
    const path = requireArg(
      write ? rest[1] : rest[0],
      "usage: bun packages/compiler/src/cli.ts fmt [--write] <file.mochi>",
    );
  ...
  .with("dts", async () => {
    ...
    const path = requireArg(rest[0], "usage: bun packages/compiler/src/cli.ts dts <file.mochi>");
    const src = await Bun.file(path).text();
    const r = emitDts(src);
    if (isErr(r)) die(r.error, src);
  ...
  .with("ts", async () => {
    ...
    const path = requireArg(rest[0], "usage: bun packages/compiler/src/cli.ts ts <file.mochi>");
    const src = await Bun.file(path).text();
  ...
  .with("build", async () => {
    ...
    const entry = requireArg(
      ...
      "usage: bun packages/compiler/src/cli.ts build [--emit=ts] <entry.mochi>",
    );
  ...
  .otherwise(async (path) => {
    ...
    const file = requireArg(
      path,
      "usage: bun packages/compiler/src/cli.ts <file.mochi>  |  bun packages/compiler/src/cli.ts fmt [--write] <file.mochi>",
    );
