...
|---|---|---|---|---|---|
| 0 | Parser error recovery — slices a–f (ADR → parser multi-error → no-cascade check/infer → formatter passthrough → LSP → bootstrap parity) | [C9](tickets/c09-parser-error-recovery.md) | HITL→AFK | 2 (ADR slice only) | a,b done (ADR 0045) |
