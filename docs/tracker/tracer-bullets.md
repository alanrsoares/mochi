...
|---|---|---|---|---|---|
| 0 | Parser error recovery — slices a–f (ADR → parser multi-error → no-cascade check/infer → formatter passthrough → LSP → bootstrap parity) | [C9](tickets/c09-parser-error-recovery.md) | HITL→AFK | 2 (ADR slice only) | a,b,c done (ADR 0045) |
...
| # | Title | Ticket | Type | Blocked by | Status |
|---|---|---|---|---|---|
| 3 | Type-name imports — qualified `TypeExpr` (`Alias.T`) parser → infer → dts/hover, TS + bootstrap parity | [C5](tickets/c05-type-name-imports.md) | AFK | — | open |
