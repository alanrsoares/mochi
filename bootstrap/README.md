...
same behavior to `packages/compiler/src/`. The TypeScript compiler remains the
seed and independent parity oracle; it is not a second design source.

Keep these checks green before handing off a core change:

```bash
bun run fixpoint       # stage 2 == stage 3 == TypeScript reference
