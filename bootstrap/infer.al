...
let bindParam = (p, env, st) => switch p {
  | LPName(name) => let (t, st1) = freshVar(st) in
    (t, Map.set(name, mono(t), env), st1)
  | LPTuple(names) => let (elems, env1, st1) = bindParamNamesFrom(
      names,
      env,
      st
    ) in
    (tTuple(elems), env1, st1)
  | LPRecord(fields) => let (rowBase, st1) = freshRowVar(st) in
...
let inferCallArgs = (fnT, args, ctx, st) => switch args {
  | [] => Ok((fnT, st))
  | [arg, ...rest] => let? (argT, st1) = inferExpr(arg, ctx, st) in
...
let inferInterpParts = (parts, ctx, st) => switch parts {
  ...
  | [IPExpr(ex), ...rest] => let? (t, st1) = inferExpr(ex, ctx, st) in
...
let inferTupleElems = (elements, ctx, st) => switch elements {
  | [] => Ok(([], st))
  | [el, ...rest] => let? (t, st1) = inferExpr(el, ctx, st) in
...
let inferSeqExprElems = (elem, elements, ctx, st) => switch elements {
  | [] => Ok(st)
  | [el, ...rest] => let? (et, st1) = inferExpr(el, ctx, st) in
...
let inferMapEntries = (k, v, entries, ctx, st) => switch entries {
  | [] => Ok(st)
  | [ent, ...rest] => let? (kt, st1) = inferExpr(ent.key, ctx, st) in
...
let inferArms = (scrutT, resultT, arms, ctx, st) => switch arms {
  | [] => Ok(st)
  | [arm, ...rest] => let? (patT, bindings, st1) = inferPat(
      arm.pattern,
      ctx,
      st
    ) in
    ...
    let? st3 = switch arm.guard {
      | None => Ok(st2)
      | Some(g) => let? (guardT, stg) = inferExpr(g, armCtx, st2) in
...
let inferMatch = (scrutinee, arms, ctx, st) =>
  ...
  let (resultT, st2) = freshVar(st1) in
  let? st3 = inferArms(scrutT, resultT, arms, ctx, st2) in Ok((resultT, st3))
...
let inferExpr = (e, ctx, st) => switch e {
  ...
  | ELambda(params, body, _) => let (paramTypes, bodyEnv, st1) = bindParamsFrom(
      params,
      ctx.env,
      st
    ) in
  ...
  | ELetIn(name, _, value, body, _) => let? (valT, st1) = inferExpr(
      value,
      ctx,
      st
    ) in
  ...
  | ELetBind(param, paramSpan, value, body, _) => inferLetBind(
      param,
      paramSpan,
      value,
      body,
  ...
  | ECall(fn, args, _) => let? (fnT, st1) = inferExpr(fn, ctx, st) in
  ...
  | ERecord(fields, spread, sp) => switch spread {
      | None => let? (row, st1) = inferRecordRow(fields, ctx, st) in
        Ok((tRecord(row), st1))
      | Some(spreadExpr) =>
  ...
  | EInterp(parts, _) => let? st1 = inferInterpParts(parts, ctx, st) in
    Ok((tString, st1))
}
...
let inferPatRecordFrom = (fields, ctx, row, bindings, st) => switch fields {
  | [] => Ok((row, bindings, st))
  | [f, ...rest] => let? (subT, subBindings, st1) = inferPat(f.pat, ctx, st) in
    inferPatRecordFrom(
      rest,
      ctx,
...
let inferPatRecord = (fields, ctx, st) =>
  ...
  let? (row, bindings, st2) = inferPatRecordFrom(
    fields,
    ctx,
    rowBase,
    #{},
...
let inferPatCtorArgs = (ctor, curT, args, ctx, st, bindings, sp) => switch args {
  ...
  | [argPat, ...rest] => switch resolve(curT, st) {
      | TyFn(fromT, toT) => let? (subT, subBindings, st1) = inferPat(
          argPat,
          ctx,
          st
        ) in
...
let inferPatTupleFrom = (elems, ctx, st) => switch elems {
  | [] => Ok(([], #{}, st))
  | [ep, ...rest] => let? (t, bindings, st1) = inferPat(ep, ctx, st) in
    let? (restTs, restBindings, st2) = inferPatTupleFrom(rest, ctx, st1) in
    Ok(
      (Array.prepend(t, restTs), mergeBindingMaps(restBindings, bindings), st2)
    )
}
...
let inferSeqPatElems = (elem, elems, ctx, st) => switch elems {
  | [] => Ok((#{}, st))
  | [ep, ...rest] => let? (subT, subBindings, st1) = inferPat(ep, ctx, st) in
...
let inferSeqPat = (con, elems, restPat, ctx, st) =>
  ...
  switch restPat {
    ...
    | Some(r) => let? (subT, subBindings, st3) = inferPat(r, ctx, st2) in
...
let inferPat = (p, ctx, st) => switch p {
  ...
  | PBind(name, _) => let (t, st1) = freshVar(st) in
...
let inferOrPatAlts = (alts, i, t, bindings, ctx, st) => switch Array.get(i, alts) {
  | None => Ok(st)
  | Some(alt) => let? (altT, altBindings, st1) = inferPat(alt, ctx, st) in
...
let inferOrPat = (alts, sp, ctx, st) => switch alts {
  ...
  | [first, ...rest] => let? (t, bindings, st1) = inferPat(first, ctx, st) in
...
let patternBinds = p => switch p {
  ...
  | PArr(elems, rest, _) => Array.concat(
      elems |> Array.flatMap(patternBinds),
      patternBindsOpt(rest)
    )
  | PList(elems, rest, _) => Array.concat(
      elems |> Array.flatMap(patternBinds),
      patternBindsOpt(rest)
    )
...
let freeRefsInterpParts = (parts, bound, acc) => switch parts {
  ...
  | [IPExpr(ex), ...rest] => freeRefsInterpParts(
      rest,
      bound,
...
let freeRefs = (e, bound, acc) => switch e {
  ...
  | ELetIn(name, _, value, body, _) => let acc1 = freeRefs(value, bound, acc) in
    freeRefs(body, Set.add(name, bound), acc1)
  | ELetBind(param, _, value, body, _) => let acc1 = freeRefs(
      value,
      bound,
      acc
    ) in
  ...
  | ETernary(cond, thenE, elseE, _) => freeRefs(
      elseE,
      bound,
      freeRefs(thenE, bound, freeRefs(cond, bound, acc))
    )
  | EMatch(scrutinee, arms, _) => freeRefsArms(
      arms,
      bound,
      freeRefs(scrutinee, bound, acc)
    )
  | ERecord(fields, spread, _) => freeRefsFields(
      fields,
      bound,
  ...
  | [w, ...rest] => hasIndex(w, st)
      ? Set.has(w, st.onStack)
        ? visitNeighbors(
          ...
          {
            ...st,
            low: Map.set(v, min(lowOfV(v, st), indexOfV(w, st)), st.low)
          }
        )
      ...
      visitNeighbors(
        ...
        {
          ...st1,
          low: Map.set(v, min(lowOfV(v, st1), lowOfV(w, st1)), st1.low)
        }
      )
}
...
let connectAllFrom = (i, n, adj, st) =>
  gte(i, n)
    ...
    : connectAllFrom(
      ...
      n,
      adj,
      hasIndex(i, st) ? st : connect(i, adj, st)
    )
...
let seedBuiltinsFrom = (keys, builtins, env, st) => switch keys {
  ...
  | [n, ...rest] => switch Map.get(n, builtins) {
      | Some(t) => seedBuiltinsFrom(
          rest,
          builtins,
          Map.set(n, generalize(env, t, st), env),
          st
        )
...
let seedNsMembersFrom = (keys, members, env, st, acc) => switch keys {
  ...
  | [m, ...rest] => switch Map.get(m, members) {
      | Some(t) => seedNsMembersFrom(
          ...
          env,
          st,
          Map.set(m, generalize(env, t, st), acc)
        )
...
let aliasMapFrom = (stmts, acc) => switch stmts {
  | [] => acc
  | [s, ...rest] => switch s {
      | SType(name, params, _, Some(fields), _, _) => aliasMapFrom(
          rest,
          Map.set(name, { params: params, fields: fields }, acc)
        )
...
let registerUserCtorsFrom = (stmts, aliasMap, env, st) => switch stmts {
  | [] => (env, st)
  | [s, ...rest] => switch s {
      | SType(name, params, ctors, _, _, _) => let (env1, st1) = registerCtorsFrom(
          ctors,
          name,
          params,
          aliasMap,
...
let registerExternsFrom = (stmts, aliasMap, env, st) => switch stmts {
  | [] => (env, st)
  | [s, ...rest] => switch s {
      | SExtern(name, _, typeExpr, _, _, _, _) => let (t, _, st1) = typeExprToType(
          typeExpr,
          #{},
          st,
          aliasMap,
        ...
        registerExternsFrom(
          rest,
          aliasMap,
          Map.set(name, generalize(env, t, st1), env),
          st1
        )
  ...
  | [s, ...rest] => switch s {
      | SLet(name, _, _, _, _, _, _) => switch Map.get(name, bodyTypes) {
          | Some(t) => generalizeGroupFrom(
              rest,
              bodyTypes,
              Map.set(name, generalize(env, t, st), env),
              st
            )
...
let exportCtorsInto = (ctors, i, env, acc) => switch Array.get(i, ctors) {
  | None => acc
  | Some(c) => exportCtorsInto(
      ctors,
      add(i, 1),
      env,
      takeScheme(c.name, env, acc)
    )
}
let exportedSchemesFrom = (stmts, i, env, acc) => switch Array.get(i, stmts) {
  ...
  | Some(SExtern(name, _, _, _, _, true, _)) => exportedSchemesFrom(
      stmts,
      add(i, 1),
      env,
      takeScheme(name, env, acc)
    )
  | Some(SType(_, _, ctors, _, true, _)) => exportedSchemesFrom(
      stmts,
      add(i, 1),
      env,
