...
export const plainEnv: <A, B, C>(vars: A) => { vars: A; recs: Map<B, C> } = <A, B, C>(vars: A) =>
  tsEnv(vars, new Map<B, C>());
export const recsEnv: <A, B, C>(recs: A) => { vars: Map<B, C>; recs: A } = <A, B, C>(recs: A) =>
  tsEnv(new Map<B, C>(), recs);
...
const primitiveTs: (name: string) => string = (name: string) =>
  match(name)
    ...
    .with("unit", () => "undefined")
    .otherwise(() => name);
const namesOf: { (ts: Ty[]): (env: TsEnv) => string; (ts: Ty[], env: TsEnv): string } = _curry(
  2,
  (ts: Ty[], env: TsEnv) =>
    _Str_join(
      ", ",
...
const nominal: {
  (name: string): (args: Ty[]) => (env: TsEnv) => string;
  (name: string): (args: Ty[], env: TsEnv) => string;
  (name: string, args: Ty[]): (env: TsEnv) => string;
  (name: string, args: Ty[], env: TsEnv): string;
} = _curry(3, (name: string, args: Ty[], env: TsEnv) =>
  eq(length(args), 0) ? primitiveTs(name) : `${name}<${namesOf(args, env)}>`,
);
const tsRowFields: {
  (row: Row): (env: TsEnv) => [string[], Option<number>];
  (row: Row, env: TsEnv): [string[], Option<number>];
} = _curry(2, (row: Row, env: TsEnv) =>
  match(row)
...
const shapeFieldsFrom: {
  (row: Row): (vars: Map<number, string>) => Option<string[]>;
  (row: Row, vars: Map<number, string>): Option<string[]>;
} = _curry(2, (row: Row, vars: Map<number, string>) =>
  match(row)
...
const aliasNameFor: {
  (row: Row): (env: TsEnv) => Option<string>;
  (row: Row, env: TsEnv): Option<string>;
} = _curry(2, (row: Row, env: TsEnv) =>
...
const tsRow: { (row: Row): (env: TsEnv) => string; (row: Row, env: TsEnv): string } = _curry(
  2,
  (row: Row, env: TsEnv) =>
...
const tsArrow: {
  (fromT: Ty): (toT: Ty) => (env: TsEnv) => string;
  (fromT: Ty): (toT: Ty, env: TsEnv) => string;
  (fromT: Ty, toT: Ty): (env: TsEnv) => string;
  (fromT: Ty, toT: Ty, env: TsEnv): string;
} = _curry(3, (fromT: Ty, toT: Ty, env: TsEnv) =>
  isUnit(fromT) ? `() => ${tsOfRaw(toT, env)}` : tsArrowParams(fromT, toT, env, 0, [] as string[]),
);
const tsArrowParams: {
  (fromT: Ty): (toT: Ty) => (env: TsEnv) => (i: number) => (params: string[]) => string;
  (fromT: Ty): (toT: Ty) => (env: TsEnv) => (i: number, params: string[]) => string;
  (fromT: Ty): (toT: Ty) => (env: TsEnv, i: number) => (params: string[]) => string;
  (fromT: Ty): (toT: Ty, env: TsEnv) => (i: number) => (params: string[]) => string;
  (fromT: Ty, toT: Ty): (env: TsEnv) => (i: number) => (params: string[]) => string;
  ...
  return match(toT)
    .with(
      ...
      ({ from: nextFrom, to: nextTo }) => tsArrowParams(nextFrom, nextTo, env, add(i, 1), params1),
    )
    .otherwise(() => `(${_Str_join(", ", params1)}) => ${tsOfRaw(toT, env)}`);
});
export const tsOf: { (t: Ty): (env: TsEnv) => string; (t: Ty, env: TsEnv): string } = _curry(
  2,
  (t: Ty, env: TsEnv) => tsOfRaw(widenLits(t), env),
);
const tsOfRaw: { (t: Ty): (env: TsEnv) => string; (t: Ty, env: TsEnv): string } = _curry(
  2,
  (t: Ty, env: TsEnv) =>
    match(t)
