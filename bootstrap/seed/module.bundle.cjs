...
var frontend = _curry21(2, (src, plugins) => match20(lex(src)).with({ _tag: "Err" }, ({ error: e }) => Err9([stampStage("lex", e)])).with({ _tag: "Ok" }, ({ value: tokens }) => ((parsed) => match20(parsed.diagnostics).with((_v) => {
  const _g = _v;
  return _g.length === 0;
...
var typedProgramWith = _curry21(2, (src, opts) => _Result_flatMap8((stmts) => _Result_mapErr((e) => [stampType(e)], _Result_map7((r) => _tuple13(stmts, r), inferProgramTypesWith(stmts, builtins, namespaces, openMode(src, opts.open), opts.plugins))), frontend(src, opts.plugins)));
...
var defaultOpts2 = { open: false, runtime: true, docs: true, moduleExt: ".js", strictEntry: false, plugins: None22 };
...
var parseModule = _curry23(2, (src, plugins) => _Result_flatMap9((toks) => parseWith(toks, plugins), lex(src)));
...
var visit = _curry23(3, (path, acc, plugins) => match22(_Map_get12(path, acc.state)).with({ _tag: "Some", value: "done" }, () => Ok13(acc)).with({ _tag: "Some", value: "loading" }, () => Err11(mErr(`import cycle through '${path}'`))).otherwise(() => ((acc1) => match22(readFile(path)).with({ _tag: "Err" }, () => Err11(mErr(`cannot read module '${path}'`))).with({ _tag: "Ok" }, ({ value: src }) => match22(parseModule(src, plugins)).with({ _tag: "Err" }, ({ error: e }) => Err11(e)).with({ _tag: "Ok" }, ({ value: stmts }) => match22(visitAll(importFroms(stmts), path, acc1, plugins)).with({ _tag: "Err" }, ({ error: e }) => Err11(e)).with({ _tag: "Ok" }, ({ value: acc2 }) => Ok13({ state: _Map_set11(path, "done", acc2.state), order: _Array_append15({ path, src, stmts }, acc2.order) })).exhaustive()).exhaustive()).exhaustive())({ state: _Map_set11(path, "loading", acc.state), order: acc.order })));
var visitAll = _curry23(4, (froms, importer, acc, plugins) => match22(froms).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => Ok13(acc)).with((_v) => {
  const _g = _v;
  return _g.length >= 1;
}, ([from, ...rest]) => match22(visit(resolveImport2(importer, from), acc, plugins)).with({ _tag: "Err" }, ({ error: e }) => Err11(e)).with({ _tag: "Ok" }, ({ value: acc1 }) => visitAll(rest, importer, acc1, plugins)).exhaustive()).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var loadGraphWith = _curry23(2, (entry, plugins) => _Result_flatMap9((acc) => Ok13(acc.order), visit(absPath(entry), { state: new Map, order: [] }, plugins)));
var loadGraph = (entry) => loadGraphWith(entry, None22);
...
var qualScopeOf = _curry23(2, (stmts, quals) => ({ types: exportedTypeNames(stmts), aliases: scopeAliases(stmts, quals) }));
...
var buildModulesWith = _curry23(2, (entry, opts) => match22(loadGraphWith(entry, opts.plugins)).with({ _tag: "Err" }, ({ error: e }) => Err11([e])).with({ _tag: "Ok" }, ({ value: graph }) => ((recovered) => eq19(length16(recovered.errors), 0) ? _Result_mapErr2((e) => [e], compileGraphWith(graph, opts)) : Err11(recovered.errors))(compileGraphRecoveringWith(graph, opts))).exhaustive());
...
var compileOneTs = _curry23(3, (ctx, loaded, opts) => match22(resolveImportsFrom(ctx, loaded.stmts, 0, loaded.path, { imports: new Map, nsImports: new Map, reg: emptyReg, keys: new Map, quals: new Map }, false)).with({ _tag: "Err" }, ({ error: e }) => Err11(atPath(loaded.path, e))).with({ _tag: "Ok" }, ({ value: res }) => match22(checkWith(loaded.stmts, res.reg, res.quals)).with({ _tag: "Err" }, ({ error: e }) => Err11(atPath(loaded.path, e))).with({ _tag: "Ok" }, () => match22(inferProgramImportsTypes(loaded.stmts, builtins, namespaces, openMode(loaded.src, opts.open), res.imports, res.nsImports, res.quals, opts.plugins)).with({ _tag: "Err" }, ({ error: e }) => Err11(atPath(loaded.path, e))).with({ _tag: "Ok" }, ({ value: r }) => ((body) => ((lines) => ((ts) => Ok13({ exportsByPath: _Map_set11(loaded.path, exportedSchemes(loaded.stmts, r.env), ctx.exportsByPath), regByPath: _Map_set11(loaded.path, exportedRegistry(loaded.stmts), ctx.regByPath), keysByPath: _Map_set11(loaded.path, exportedCtorKeys(loaded.stmts), ctx.keysByPath), qualsByPath: _Map_set11(loaded.path, qualScopeOf(loaded.stmts, res.quals), ctx.qualsByPath), aliases: mergeMap(r.aliases, ctx.aliases), typeOwner: ctx.typeOwner, dupNames: ctx.dupNames, runtimeImport: ctx.runtimeImport, externs: externBindingsInto(loaded.stmts, loaded.path, r.env, ctx.externs), outputs: [...ctx.outputs, { path: loaded.path, js: ts }] }))(eq19(length16(lines), 0) ? body : `${_Str_join9(`
`, lines)}
...
var emitDtsForFileWith = _curry23(3, (entry, runtimeImport, opts) => _Result_flatMap9((graph) => dtsAll({ exportsByPath: new Map, regByPath: new Map, keysByPath: new Map, qualsByPath: new Map, aliases: new Map, runtimeImport, target: absPath(entry), dts: "" }, graph, opts), loadGraphWith(entry, opts.plugins)));
var emitDtsForFile = _curry23(2, (entry, runtimeImport) => emitDtsForFileWith(entry, runtimeImport, defaultOpts2));
var buildModulesTsWith = _curry23(3, (entry, runtimeImport, opts) => match22(loadGraphWith(entry, opts.plugins)).with({ _tag: "Err" }, ({ error: e }) => Err11([e])).with({ _tag: "Ok" }, ({ value: graph }) => ((recovered) => eq19(length16(recovered.errors), 0) ? _Result_mapErr2((e) => [e], compileGraphTsWith(graph, runtimeImport, opts)) : Err11(recovered.errors))(compileGraphRecoveringWith(graph, { ...opts, strictEntry: false }))).exhaustive());
