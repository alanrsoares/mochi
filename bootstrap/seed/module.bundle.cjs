...
var shapeType = _curry18(2, (t, vars) => match17(widenLits(t)).with({ _tag: "TyRecord" }, ({ row }) => match17(shapeFieldsFrom(row, vars)).with({ _tag: "None" }, () => tsOf(t, plainEnv(vars))).with({ _tag: "Some" }, ({ value: fs }) => eq16(length13(fs), 0) ? "{}" : `{ ${_Str_join6("; ", _Array_sort(fs))} }`).exhaustive()).with((_v) => {
  const _g = _v;
