...
var registerExternsFrom = _curry14(4, (stmts, aliasMap, env, st) => match13(stmts).with((_v) => {
  ...
  const _g = _v;
  return _g.length >= 1;
}, ([s, ...rest]) => match13(s).with({ _tag: "SExtern" }, ({ name, params, typeExpr }) => (([vars, st0]) => (([t, _, st1]) => registerExternsFrom(rest, aliasMap, _Map_set6(name, generalize(env, t, st1, false), env), st1))(typeExprToType(typeExpr, vars, st0, aliasMap, _Set_fromArray4([]))))(reduce2(_curry14(2, ([vs, s], param) => (([v, s1]) => _tuple7(_Map_set6(param, v, vs), s1))(freshVar(s))), _tuple7(new Map, st), params))).otherwise(() => registerExternsFrom(rest, aliasMap, env, st))).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
...
var localLetsFrom = (e) => {
  const collect = _curry14(2, (current, acc) => match13(current).with({ _tag: "ELetIn" }, ({ name, nameSpan, annot, value, body, span }) => match13(value).with({ _tag: "ELambda" }, () => collect(body, _Array_append9(SLet(name, nameSpan, annot, value, false, None13, span), acc))).otherwise(() => acc)).otherwise(() => acc));
  return collect(e, []);
};
...
var keyedSlot = _curry15(2, (key, sub) => eq14(sub, key) ? key : `${key}: ${sub}`);
...
var ctorRefines = _curry15(5, (ctx, args, keys, member, i) => match14(_Array_get11(i, args)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: a }) => ((rest) => ((key) => match14(fieldRefine(ctx, a, `${member}[${jsStringLit(key)}]`)).with({ _tag: "Some" }, ({ value: sub }) => _Array_prepend7(`${jsStringLit(key)}: ${sub}`, rest)).with({ _tag: "None" }, () => rest).exhaustive())(_Option_unwrapOr7(`_${show6(i)}`, _Array_get11(i, keys))))(ctorRefines(ctx, args, keys, member, i + 1))).exhaustive());
var recordRefines = _curry15(4, (ctx, fields, base, i) => match14(_Array_get11(i, fields)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: f }) => ((rest) => match14(fieldRefine(ctx, f.pat, `${base}[${jsStringLit(f.label)}]`)).with({ _tag: "Some" }, ({ value: sub }) => _Array_prepend7(`${jsStringLit(f.label)}: ${sub}`, rest)).with({ _tag: "None" }, () => rest).exhaustive())(recordRefines(ctx, fields, base, i + 1))).exhaustive());
...
`, header)}
${body}`) ? ["_Curry"] : [], builtinTypeNamesFor(declared, wanted, body, 0))))(typeHeader))(eq16(length13(deps), 0) ? "" : `import { ${_Str_join7(", ", _Array_sort2(deps))} } from "${runtimeImport}";`))(filter5((d) => or8(and11(and11(and11(and11(and11(and11(and11(not10(eq16(d, "add")), not10(eq16(d, "sub"))), not10(eq16(d, "mul"))), not10(eq16(d, "div"))), not10(eq16(d, "lt"))), not10(eq16(d, "lte"))), not10(eq16(d, "gt"))), not10(eq16(d, "gte"))), _Str_contains(d, body)), deps));
});
...
import { _Array_append as _Array_append13, _Array_concat as _Array_concat8, _Array_get as _Array_get14, _Array_prepend as _Array_prepend10, _Map_get as _Map_get10, _Map_set as _Map_set9, _curry as _curry18, _tuple as _tuple11 } from "@mochi/compiler/runtime";
import { match as match17 } from "@onrails/pattern";
var def = _curry18(2, (name, span) => ({ name, defStart: span.start, defEnd: span.end, start: span.start, end: span.end, role: "def" }));
var use = _curry18(3, (name, span, env) => match17(_Map_get10(name, env)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: binding }) => [{ name, defStart: binding.start, defEnd: binding.end, start: span.start, end: span.end, role: "use" }]).exhaustive());
var bind = _curry18(3, (name, span, env) => _Map_set9(name, { name, start: span.start, end: span.end }, env));
var bindSpannedNames = _curry18(4, (names, spans, env, i) => match17(_tuple11(_Array_get14(i, names), _Array_get14(i, spans))).with((_v) => {
  const _g = _v;
...
var bindParam2 = _curry18(2, (param, env) => match17(param).with((_v) => {
  const _g = _v;
...
var bindParams = _curry18(3, (params, env, i) => match17(_Array_get14(i, params)).with({ _tag: "None" }, () => ({ env, occurrences: [] })).with({ _tag: "Some" }, ({ value: param }) => ((head) => ((tail) => ({ env: tail.env, occurrences: _Array_concat8(head.occurrences, tail.occurrences) }))(bindParams(params, head.env, i + 1)))(bindParam2(param, env))).exhaustive());
var walkExprs = _curry18(3, (exprs, env, i) => match17(_Array_get14(i, exprs)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: expr }) => _Array_concat8(walkExpr(expr, env), walkExprs(exprs, env, i + 1))).exhaustive());
var walkSeqs = _curry18(3, (elems, env, i) => match17(_Array_get14(i, elems)).with({ _tag: "None" }, () => []).with((_v) => {
  const _g = _v;
  return _g._tag === "Some" && _g.value._tag === "SEExpr";
...
var walkPattern = _curry18(2, (pat, env) => match17(pat).with({ _tag: "PBind" }, ({ name, span }) => ({ env: bind(name, span, env), occurrences: [def(name, span)] })).with({ _tag: "PAs" }, ({ pat: inner, name, nameSpan }) => ((innerResult) => ({ env: bind(name, nameSpan, innerResult.env), occurrences: _Array_append13(def(name, nameSpan), innerResult.occurrences) }))(walkPattern(inner, env))).with({ _tag: "PTuple" }, ({ elems }) => walkPatterns(elems, env, 0)).with({ _tag: "PRecord" }, ({ fields }) => walkPatFields(fields, env, 0)).with({ _tag: "PCtor" }, ({ args }) => walkPatterns(args, env, 0)).with({ _tag: "PArr" }, ({ elems, rest }) => ((result) => match17(rest).with({ _tag: "None" }, () => result).with({ _tag: "Some" }, ({ value: tail }) => ((tailResult) => ({ env: tailResult.env, occurrences: _Array_concat8(result.occurrences, tailResult.occurrences) }))(walkPattern(tail, result.env))).exhaustive())(walkPatterns(elems, env, 0))).with({ _tag: "PList" }, ({ elems, rest }) => ((result) => match17(rest).with({ _tag: "None" }, () => result).with({ _tag: "Some" }, ({ value: tail }) => ((tailResult) => ({ env: tailResult.env, occurrences: _Array_concat8(result.occurrences, tailResult.occurrences) }))(walkPattern(tail, result.env))).exhaustive())(walkPatterns(elems, env, 0))).with((_v) => {
  const _g = _v;
  return _g._tag === "POr" && _g.alts.length >= 1;
}, ({ alts: [first] }) => walkPattern(first, env)).otherwise(() => ({ env, occurrences: [] })));
var walkPatterns = _curry18(3, (patterns, env, i) => match17(_Array_get14(i, patterns)).with({ _tag: "None" }, () => ({ env, occurrences: [] })).with({ _tag: "Some" }, ({ value: pat }) => ((head) => ((tail) => ({ env: tail.env, occurrences: _Array_concat8(head.occurrences, tail.occurrences) }))(walkPatterns(patterns, head.env, i + 1)))(walkPattern(pat, env))).exhaustive());
var walkPatFields = _curry18(3, (fields, env, i) => match17(_Array_get14(i, fields)).with({ _tag: "None" }, () => ({ env, occurrences: [] })).with({ _tag: "Some" }, ({ value: field }) => ((head) => ((tail) => ({ env: tail.env, occurrences: _Array_concat8(head.occurrences, tail.occurrences) }))(walkPatFields(fields, head.env, i + 1)))(walkPattern(field.pat, env))).exhaustive());
var walkArms = _curry18(3, (arms, env, i) => match17(_Array_get14(i, arms)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: arm }) => ((pat) => ((guard) => _Array_concat8(pat.occurrences, _Array_concat8(guard, _Array_concat8(walkExpr(arm.body, pat.env), walkArms(arms, env, i + 1)))))(match17(arm.guard).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: expr }) => walkExpr(expr, pat.env)).exhaustive()))(walkPattern(arm.pattern, env))).exhaustive());
var walkFields = _curry18(3, (fields, env, i) => match17(_Array_get14(i, fields)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: field }) => _Array_concat8(walkExpr(field.value, env), walkFields(fields, env, i + 1))).exhaustive());
var walkEntries = _curry18(3, (entries, env, i) => match17(_Array_get14(i, entries)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: entry }) => _Array_concat8(walkExpr(entry.key, env), _Array_concat8(walkExpr(entry.value, env), walkEntries(entries, env, i + 1)))).exhaustive());
var walkLoopParams = _curry18(3, (params, env, i) => match17(_Array_get14(i, params)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: param }) => _Array_concat8(walkExpr(param.init, env), walkLoopParams(params, env, i + 1))).exhaustive());
var loopEnv = _curry18(3, (params, env, i) => match17(_Array_get14(i, params)).with({ _tag: "None" }, () => env).with({ _tag: "Some" }, ({ value: param }) => loopEnv(params, bind(param.name, param.nameSpan, env), i + 1)).exhaustive());
var loopDefs = _curry18(2, (params, i) => match17(_Array_get14(i, params)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: param }) => _Array_concat8([def(param.name, param.nameSpan)], loopDefs(params, i + 1))).exhaustive());
var walkExpr = _curry18(2, (expr, env) => match17(expr).with({ _tag: "ERef" }, ({ name, span }) => use(name, span, env)).with({ _tag: "ECall" }, ({ fn, args }) => _Array_concat8(walkExpr(fn, env), walkExprs(args, env, 0))).with({ _tag: "ELambda" }, ({ params, body }) => ((bound) => _Array_concat8(bound.occurrences, walkExpr(body, bound.env)))(bindParams(params, env, 0))).with({ _tag: "ELetIn" }, ({ name, nameSpan, value, body }) => ((recursive) => ((bodyEnv) => recursive ? _Array_concat8([def(name, nameSpan)], _Array_concat8(walkExpr(value, bodyEnv), walkExpr(body, bodyEnv))) : _Array_concat8(walkExpr(value, env), _Array_concat8([def(name, nameSpan)], walkExpr(body, bodyEnv))))(bind(name, nameSpan, env)))(match17(value).with({ _tag: "ELambda" }, () => true).otherwise(() => false))).with({ _tag: "ELetBind" }, ({ value, body }) => _Array_concat8(walkExpr(value, env), walkExpr(body, env))).with({ _tag: "EPipe" }, ({ left, right }) => _Array_concat8(walkExpr(left, env), walkExpr(right, env))).with({ _tag: "EDo" }, ({ exprs }) => walkExprs(exprs, env, 0)).with({ _tag: "ETernary" }, ({ cond, thenE, elseE }) => _Array_concat8(walkExpr(cond, env), _Array_concat8(walkExpr(thenE, env), walkExpr(elseE, env)))).with({ _tag: "EMatch" }, ({ scrutinee, arms }) => _Array_concat8(walkExpr(scrutinee, env), walkArms(arms, env, 0))).with({ _tag: "ERecord" }, ({ fields, spread }) => match17(spread).with({ _tag: "None" }, () => walkFields(fields, env, 0)).with({ _tag: "Some" }, ({ value: base }) => _Array_concat8(walkExpr(base, env), walkFields(fields, env, 0))).exhaustive()).with({ _tag: "EField" }, ({ target }) => walkExpr(target, env)).with({ _tag: "ETuple" }, ({ elements }) => walkExprs(elements, env, 0)).with({ _tag: "EArr" }, ({ elements }) => walkSeqs(elements, env, 0)).with({ _tag: "EList" }, ({ elements }) => walkSeqs(elements, env, 0)).with({ _tag: "ESet" }, ({ elements }) => walkSeqs(elements, env, 0)).with({ _tag: "EMap" }, ({ entries }) => walkEntries(entries, env, 0)).with({ _tag: "ELoop" }, ({ params, body }) => ((scoped) => _Array_concat8(walkLoopParams(params, env, 0), _Array_concat8(loopDefs(params, 0), walkExpr(body, scoped))))(loopEnv(params, env, 0))).with({ _tag: "ERecur" }, ({ args }) => walkExprs(args, env, 0)).with({ _tag: "EInterp" }, ({ parts }) => walkInterp(parts, env, 0)).otherwise(() => []));
var walkInterp = _curry18(3, (parts, env, i) => match17(_Array_get14(i, parts)).with({ _tag: "None" }, () => []).with((_v) => {
  const _g = _v;
...
var topEnv = _curry18(3, (stmts, env, i) => match17(_Array_get14(i, stmts)).with({ _tag: "None" }, () => env).with((_v) => {
  const _g = _v;
  return _g._tag === "Some" && _g.value._tag === "SLet";
...
var topDefs = _curry18(2, (stmts, i) => match17(_Array_get14(i, stmts)).with({ _tag: "None" }, () => []).with((_v) => {
  const _g = _v;
  return _g._tag === "Some" && _g.value._tag === "SLet";
...
var walkStmts = _curry18(3, (stmts, env, i) => match17(_Array_get14(i, stmts)).with({ _tag: "None" }, () => []).with((_v) => {
  const _g = _v;
  return _g._tag === "Some" && _g.value._tag === "SLet";
...
import { _Array_concat as _Array_concat9, _Array_contains as _Array_contains5, _Array_get as _Array_get15, _Array_prepend as _Array_prepend11, _Map_get as _Map_get11, _Map_getOr as _Map_getOr7, _Map_has as _Map_has5, _Map_keys as _Map_keys7, _Map_set as _Map_set10, _Result_map as _Result_map8, _Set_add as _Set_add7, _Set_fromArray as _Set_fromArray7, _Set_has as _Set_has7, _Set_toArray as _Set_toArray4, _Str_contains as _Str_contains2, _Str_endsWith as _Str_endsWith3, _Str_join as _Str_join8, _Str_length as _Str_length4, _Str_slice as _Str_slice4, _Str_startsWith as _Str_startsWith6, _curry as _curry20, eq as eq17, length as length14, map as map13, not as not11, or as or10 } from "@mochi/compiler/runtime";
import { match as match19 } from "@onrails/pattern";

import { Err as Err9, _Result_flatMap as _Result_flatMap8, _Result_map as _Result_map7, _Result_mapErr, _Str_get as _Str_get4, _Str_startsWith as _Str_startsWith5, _Str_trim, _curry as _curry19, _tuple as _tuple12, and as and12, map as map12, or as or9 } from "@mochi/compiler/runtime";
...
var openDirective = (src) => {
  ...
  return and12(_Str_startsWith5('"use open"', t), match18(afterBlanks(t, 10)).with({ _tag: "None" }, () => true).with({ _tag: "Some", value: `
` }, () => true).with({ _tag: "Some", value: "r" }, () => true).otherwise(() => false));
};
var openMode = _curry19(2, (src, requested) => or9(requested, openDirective(src)));
var typecheckWith = _curry19(2, (prog, open) => _Result_mapErr((e) => [e], _Result_map7((_) => prog, inferProgram(prog, builtins, namespaces, open))));
var frontend = (src) => match18(lex(src)).with({ _tag: "Err" }, ({ error: e }) => Err9([e])).with({ _tag: "Ok" }, ({ value: tokens }) => match18(parse(tokens)).with({ _tag: "Err" }, ({ error: e }) => Err9([e])).with({ _tag: "Ok" }, ({ value: stmts }) => checkAll(stmts)).exhaustive()).exhaustive();
var pipelineWith = _curry19(2, (src, open) => _Result_flatMap8((stmts) => typecheckWith(stmts, open), frontend(src)));
var typedProgramWith = _curry19(2, (src, opts) => _Result_flatMap8((stmts) => _Result_mapErr((e) => [e], _Result_map7((r) => _tuple12(stmts, r), inferProgramTypes(stmts, builtins, namespaces, openMode(src, opts.open)))), frontend(src)));
var inferTypesWith = _curry19(2, (src, opts) => _Result_flatMap8((stmts) => _Result_mapErr((e) => [e], _Result_map7((r) => ({ env: r.env, types: map12((hit) => ({ span: hit.span, ty: hit.ty, display: showType(widenLits(hit.ty)) }), r.types), aliases: r.aliases, letParams: r.letParams }), inferProgramTypes(stmts, builtins, namespaces, openMode(src, opts.open)))), frontend(src)));
var compileWith = _curry19(2, (src, opts) => _Result_map7((prog) => codegenWith(prog, new Map, opts.runtime, namespaceRuntime, preludeJsDefs, runtimeDeps, { ...jsGenOpts, docs: opts.docs, moduleExt: opts.moduleExt }), pipelineWith(src, openMode(src, opts.open))));
var noImportedKeys = new Map;
var compileTsWith = _curry19(3, (src, runtimeImport, opts) => _Result_flatMap8((stmts) => _Result_mapErr((e) => [e], _Result_map7((r) => emitTsModuleWith(stmts, r.env, r.types, r.letParams, r.aliases, noImportedKeys, [], namespaceRuntime, preludeJsDefs, runtimeDeps, runtimeImport, opts.docs), inferProgramTypes(stmts, builtins, namespaces, openMode(src, opts.open)))), frontend(src)));
var compileTs = _curry19(2, (src, runtimeImport) => compileTsWith(src, runtimeImport, defaultOpts));

var writtenQualsIn = _curry20(3, (te, local, acc) => match19(te).with({ _tag: "TyName" }, () => acc).with({ _tag: "TyLit" }, () => acc).with({ _tag: "TyArrow" }, ({ from, to }) => writtenQualsIn(to, local, writtenQualsIn(from, local, acc))).with({ _tag: "TyApp" }, ({ args }) => writtenQualsInAll(args, local, acc, 0)).with({ _tag: "TyTuple" }, ({ elems }) => writtenQualsInAll(elems, local, acc, 0)).with({ _tag: "TyList" }, ({ elem }) => writtenQualsIn(elem, local, acc)).with({ _tag: "TyUnion" }, ({ members }) => writtenQualsInAll(members, local, acc, 0)).with({ _tag: "TyQual" }, ({ alias, name, args }) => ((acc1) => writtenQualsInAll(args, local, acc1, 0))(or10(_Set_has7(name, local), _Map_has5(name, acc)) ? acc : _Map_set10(name, `${alias}.${name}`, acc))).exhaustive());
var writtenQualsInAll = _curry20(4, (tes, local, acc, i) => match19(_Array_get15(i, tes)).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, ({ value: te }) => writtenQualsInAll(tes, local, writtenQualsIn(te, local, acc), i + 1)).exhaustive());
var ctorQualsFrom = _curry20(4, (ctors, local, acc, i) => match19(_Array_get15(i, ctors)).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, ({ value: c }) => ctorQualsFrom(ctors, local, writtenQualsInAll(map13((f) => f.fieldType, c.fields), local, acc, 0), i + 1)).exhaustive());
var writtenQualsFrom = _curry20(4, (stmts, local, acc, i) => match19(_Array_get15(i, stmts)).with({ _tag: "None" }, () => acc).with((_v) => {
  const _g = _v;
  return _g._tag === "Some" && _g.value._tag === "SExtern";
}, ({ value: { typeExpr: te } }) => writtenQualsFrom(stmts, local, writtenQualsIn(te, local, acc), i + 1)).with((_v) => {
  const _g = _v;
  return _g._tag === "Some" && _g.value._tag === "SLet";
}, ({ value: { annot } }) => writtenQualsFrom(stmts, local, match19(annot).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, ({ value: te }) => writtenQualsIn(te, local, acc)).exhaustive(), i + 1)).with((_v) => {
  const _g = _v;
  return _g._tag === "Some" && _g.value._tag === "SType";
}, ({ value: { ctors, alias, aliasType } }) => ((acc1) => ((acc2) => ((acc3) => writtenQualsFrom(stmts, local, acc3, i + 1))(match19(aliasType).with({ _tag: "None" }, () => acc2).with({ _tag: "Some" }, ({ value: te }) => writtenQualsIn(te, local, acc2)).exhaustive()))(match19(alias).with({ _tag: "None" }, () => acc1).with({ _tag: "Some" }, ({ value: fields }) => writtenQualsInAll(map13((f) => f.fieldType, fields), local, acc1, 0)).exhaustive()))(ctorQualsFrom(ctors, local, acc, 0))).with({ _tag: "Some" }, () => writtenQualsFrom(stmts, local, acc, i + 1)).exhaustive());
var qualifyRow = _curry20(2, (row, qualify) => match19(row).with({ _tag: "RowEmpty" }, () => RowEmpty).with({ _tag: "RowVar" }, ({ id }) => RowVar(id)).with({ _tag: "RowExtend" }, ({ label, fieldType, optional, rest }) => RowExtend(label, qualifyTy(fieldType, qualify), optional, qualifyRow(rest, qualify))).exhaustive());
var qualifyTy = _curry20(2, (t, qualify) => match19(t).with({ _tag: "TyVar" }, ({ id }) => TyVar(id)).with({ _tag: "TyCon" }, ({ name, args }) => TyCon(_Map_getOr7(name, name, qualify), map13((a) => qualifyTy(a, qualify), args))).with({ _tag: "TyFn" }, ({ from, to }) => TyFn(qualifyTy(from, qualify), qualifyTy(to, qualify))).with({ _tag: "TyRecord" }, ({ row }) => TyRecord(qualifyRow(row, qualify))).with({ _tag: "TySingleton" }, ({ base, value }) => TySingleton(base, value)).with({ _tag: "TyOneOf" }, ({ members }) => TyOneOf(map13((m) => qualifyTy(m, qualify), members))).exhaustive());
var qualifyTe2 = _curry20(2, (te, qualify) => match19(te).with({ _tag: "TyName" }, ({ name, span }) => TyName(_Map_getOr7(name, name, qualify), span)).with({ _tag: "TyArrow" }, ({ from, to, span }) => TyArrow(qualifyTe2(from, qualify), qualifyTe2(to, qualify), span)).with({ _tag: "TyApp" }, ({ ctor, args, span }) => TyApp(_Map_getOr7(ctor, ctor, qualify), map13((a) => qualifyTe2(a, qualify), args), span)).with({ _tag: "TyTuple" }, ({ elems, span }) => TyTuple(map13((e) => qualifyTe2(e, qualify), elems), span)).with({ _tag: "TyList" }, ({ elem, span }) => TyList(qualifyTe2(elem, qualify), span)).with({ _tag: "TyQual" }, ({ alias, name, nameSpan, args, span }) => TyQual(alias, name, nameSpan, map13((a) => qualifyTe2(a, qualify), args), span)).with({ _tag: "TyLit" }, ({ value, span }) => TyLit(value, span)).with({ _tag: "TyUnion" }, ({ members, span }) => TyUnion(map13((m) => qualifyTe2(m, qualify), members), span)).exhaustive());
var qualifyField2 = _curry20(2, (f, qualify) => ({ name: f.name, fieldType: qualifyTe2(f.fieldType, qualify) }));
var qualifyCtor = _curry20(2, (c, qualify) => ({ name: c.name, fields: map13((f) => qualifyField2(f, qualify), c.fields), span: c.span }));
var qualifyAliasField = _curry20(2, (f, qualify) => ({ name: f.name, fieldType: qualifyTe2(f.fieldType, qualify), optional: f.optional }));
var typeDeclsFrom = _curry20(6, (stmts, aliases, recs, qualify, docs, i) => match19(_Array_get15(i, stmts)).with({ _tag: "None" }, () => []).with((_v) => {
  const _g = _v;
  return _g._tag === "Some" && _g.value._tag === "SType";
}, ({ value: { name, params, ctors, alias, aliasType, doc } }) => ((rest) => ((docComment) => match19(alias).with({ _tag: "Some" }, ({ value: fields }) => _Array_prepend11(`${docComment}${recordAliasDecl(name, params, map13((f) => qualifyAliasField(f, qualify), fields), aliases, withoutOwnShape(fields, params, aliases, recs))}`, rest)).with({ _tag: "None" }, () => match19(aliasType).with({ _tag: "Some" }, ({ value: te }) => _Array_prepend11(`${docComment}${aliasTsDecl(name, params, qualifyTe2(te, qualify), aliases, recs)}`, rest)).with({ _tag: "None" }, () => eq17(length14(ctors), 0) ? _Array_prepend11(`${docComment}${opaqueTypeDecl(name)}`, rest) : _Array_prepend11(`${docComment}${typeDecl(name, params, map13((c) => qualifyCtor(c, qualify), ctors), aliases, recs)}`, rest)).exhaustive()).exhaustive())(docs ? jsDoc(doc) : ""))(typeDeclsFrom(stmts, aliases, recs, qualify, docs, i + 1))).with({ _tag: "Some" }, () => typeDeclsFrom(stmts, aliases, recs, qualify, docs, i + 1)).exhaustive());
var bindingDeclsFrom = _curry20(6, (stmts, env, recs, qualify, docs, i) => match19(_Array_get15(i, stmts)).with({ _tag: "None" }, () => []).with((_v) => {
  const _g = _v;
  return _g._tag === "Some" && _g.value._tag === "SLet";
}, ({ value: { name, value, doc } }) => ((rest) => _Str_startsWith6("$", name) ? rest : match19(_Map_get11(name, env)).with({ _tag: "None" }, () => rest).with({ _tag: "Some" }, ({ value: sc }) => _Array_prepend11(`${docs ? jsDoc(doc) : ""}export declare const ${name}: ${bindingTsType({ vars: sc.vars, rvars: sc.rvars, ty: qualifyTy(sc.ty, qualify) }, value, recs)};`, rest)).exhaustive())(bindingDeclsFrom(stmts, env, recs, qualify, docs, i + 1))).with({ _tag: "Some" }, () => bindingDeclsFrom(stmts, env, recs, qualify, docs, i + 1)).exhaustive());
var builtinDeclsFor2 = _curry20(4, (names, aliases, recs, i) => match19(_Array_get15(i, builtinTypeDecls)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: bt }) => ((rest) => _Array_contains5(bt.name, names) ? _Array_prepend11(typeDecl(bt.name, bt.params, bt.ctors, aliases, recs), rest) : rest)(builtinDeclsFor2(names, aliases, recs, i + 1))).exhaustive());
...
var nsTypeImportsFrom = _curry20(4, (stmts, body, seen, i) => match19(_Array_get15(i, stmts)).with({ _tag: "None" }, () => []).with((_v) => {
  const _g = _v;
...
var emitDtsFromTypedWith = _curry20(6, (stmts, env, aliases, qualify, runtimeImport, docs) => {
  const recs = recordAliasIndex(aliases);
...
`;
  ...
  return eq17(length14(imports), 0) ? body : `${_Str_join8(`
`, imports)}
${body}`;
});
var addQuals = _curry20(5, (alias, names, local, acc, i) => match19(_Array_get15(i, names)).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, ({ value: name }) => addQuals(alias, names, local, or10(_Set_has7(name, local), _Map_has5(name, acc)) ? acc : _Map_set10(name, `${alias}.${name}`, acc), i + 1)).exhaustive());
var qualsFromAliases = _curry20(5, (aliases, quals, local, acc, i) => match19(_Array_get15(i, aliases)).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, ({ value: alias }) => match19(_Map_get11(alias, quals)).with({ _tag: "None" }, () => qualsFromAliases(aliases, quals, local, acc, i + 1)).with({ _tag: "Some" }, ({ value: scope }) => qualsFromAliases(aliases, quals, local, addQuals(alias, _Set_toArray4(scope.types), local, acc, 0), i + 1)).exhaustive()).exhaustive());
...
var emitDtsTextWith = _curry20(3, (src, runtimeImport, opts) => _Result_map8(([stmts, r]) => emitDtsFromTypedWith(stmts, r.env, r.aliases, new Map, runtimeImport, opts.docs), typedProgramWith(src, opts)));
...
var resolveImport2 = _curry21(2, resolveImport);
var mErr = (message) => ({ message, start: 0, end: 0 });
var recoveryScheme = { vars: [0], rvars: [], ty: tVar(0) };
var atPath = _curry21(2, (path, e) => ({ message: `module '${path}': ${e.message}`, start: e.start, end: e.end }));
var parseModule = ($x) => _Result_flatMap9(parse)(lex($x));
var importFromsFrom = _curry21(3, (stmts, i, acc) => match20(_Array_get16(i, stmts)).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, ({ value: s }) => match20(s).with({ _tag: "SImport" }, ({ from }) => importFromsFrom(stmts, i + 1, _Array_append14(from, acc))).with({ _tag: "SImportNs" }, ({ from }) => importFromsFrom(stmts, i + 1, _Array_append14(from, acc))).otherwise(() => importFromsFrom(stmts, i + 1, acc))).exhaustive());
var importFroms = (stmts) => importFromsFrom(stmts, 0, []);
var visit = _curry21(2, (path, acc) => match20(_Map_get12(path, acc.state)).with({ _tag: "Some", value: "done" }, () => Ok13(acc)).with({ _tag: "Some", value: "loading" }, () => Err11(mErr(`import cycle through '${path}'`))).otherwise(() => ((acc1) => match20(readFile(path)).with({ _tag: "Err" }, () => Err11(mErr(`cannot read module '${path}'`))).with({ _tag: "Ok" }, ({ value: src }) => match20(parseModule(src)).with({ _tag: "Err" }, ({ error: e }) => Err11(e)).with({ _tag: "Ok" }, ({ value: stmts }) => match20(visitAll(importFroms(stmts), path, acc1)).with({ _tag: "Err" }, ({ error: e }) => Err11(e)).with({ _tag: "Ok" }, ({ value: acc2 }) => Ok13({ state: _Map_set11(path, "done", acc2.state), order: _Array_append14({ path, src, stmts }, acc2.order) })).exhaustive()).exhaustive()).exhaustive())({ state: _Map_set11(path, "loading", acc.state), order: acc.order })));
var visitAll = _curry21(3, (froms, importer, acc) => match20(froms).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => Ok13(acc)).with((_v) => {
  const _g = _v;
  return _g.length >= 1;
}, ([from, ...rest]) => match20(visit(resolveImport2(importer, from), acc)).with({ _tag: "Err" }, ({ error: e }) => Err11(e)).with({ _tag: "Ok" }, ({ value: acc1 }) => visitAll(rest, importer, acc1)).exhaustive()).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
...
var mergeInto2 = _curry21(3, (keys, from, into) => match20(keys).with((_v) => _v.length === 0, () => into).with((_v) => _v.length >= 1, ([k, ...rest]) => mergeInto2(rest, from, match20(_Map_get12(k, from)).with({ _tag: "Some" }, ({ value: v }) => _Map_set11(k, v, into)).with({ _tag: "None" }, () => into).exhaustive())).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var mergeMap = _curry21(2, (from, into) => mergeInto2(_Map_keys8(from), from, into));
var exportedTypeNames = (stmts) => _Set_fromArray8(_Array_flatMap4((s) => match20(s).with({ _tag: "SType", exported: true }, ({ name }) => [name]).otherwise(() => []), stmts));
var aliasesOf = (stmts) => reduce5(_curry21(2, (acc, s) => match20(s).with((_v) => {
  const _g = _v;
  return _g._tag === "SType" && _g.alias._tag === "Some";
}, ({ name, params, alias: { value: fields } }) => _Map_set11(name, { params, fields, expr: None20 }, acc)).with((_v) => {
  const _g = _v;
  return _g._tag === "SType" && _g.aliasType._tag === "Some";
}, ({ name, params, aliasType: { value: te } }) => _Map_set11(name, { params, fields: [], expr: Some20(te) }, acc)).otherwise(() => acc)), new Map, stmts);
var qualScopeOf = (stmts) => ({ types: exportedTypeNames(stmts), aliases: aliasesOf(stmts) });
var withNamedCtor = _curry21(5, (name, info, depReg, depKeys, res) => ({ imports: res.imports, nsImports: res.nsImports, reg: { ctors: _Map_set11(name, info, res.reg.ctors), types: match20(_Map_get12(info.owner, depReg.types)).with({ _tag: "Some" }, ({ value: cs }) => _Map_set11(info.owner, cs, res.reg.types)).with({ _tag: "None" }, () => res.reg.types).exhaustive() }, keys: match20(_Map_get12(name, depKeys)).with({ _tag: "Some" }, ({ value: ks }) => _Map_set11(name, ks, res.keys)).with({ _tag: "None" }, () => res.keys).exhaustive(), quals: res.quals }));
var takeNamedCtor = _curry21(5, (name, span, depReg, depKeys, res) => match20(_Map_get12(name, depReg.ctors)).with({ _tag: "None" }, () => Ok13(res)).with({ _tag: "Some" }, ({ value: info }) => match20(_Map_get12(name, res.reg.ctors)).with({ _tag: "Some" }, ({ value: prior }) => not12(eq18(prior.owner, info.owner)) ? Err11({ message: `duplicate constructor '${name}'`, start: span.start, end: span.end }) : Ok13(withNamedCtor(name, info, depReg, depKeys, res))).with({ _tag: "None" }, () => Ok13(withNamedCtor(name, info, depReg, depKeys, res))).exhaustive()).exhaustive());
var prefixCtorsInto = _curry21(4, (keys, alias, from, into) => match20(keys).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => into).with((_v) => {
  const _g = _v;
  return _g.length >= 1;
}, ([k, ...rest]) => prefixCtorsInto(rest, alias, from, match20(_Map_get12(k, from)).with({ _tag: "Some" }, ({ value: v }) => _Map_set11(`${alias}.${k}`, v, into)).with({ _tag: "None" }, () => into).exhaustive())).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var resolveNames = _curry21(7, (names, from, depExports, depReg, depKeys, res, recovering) => match20(names).with((_v) => _v.length === 0, () => Ok13(res)).with((_v) => _v.length >= 1, ([n, ...rest]) => match20(_Map_get12(n.name, depExports)).with({ _tag: "None" }, () => recovering ? resolveNames(rest, from, depExports, depReg, depKeys, { imports: _Map_set11(n.name, recoveryScheme, res.imports), nsImports: res.nsImports, reg: res.reg, keys: res.keys, quals: res.quals }, recovering) : Err11({ message: `'${from}' has no export '${n.name}'`, start: n.span.start, end: n.span.end })).with({ _tag: "Some" }, ({ value: sc }) => match20(takeNamedCtor(n.name, n.span, depReg, depKeys, { imports: _Map_set11(n.name, sc, res.imports), nsImports: res.nsImports, reg: res.reg, keys: res.keys, quals: res.quals })).with({ _tag: "Err" }, ({ error: e }) => Err11(e)).with({ _tag: "Ok" }, ({ value: res1 }) => resolveNames(rest, from, depExports, depReg, depKeys, res1, recovering)).exhaustive()).exhaustive()).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var resolveImportsFrom = _curry21(6, (ctx, stmts, i, path, res, recovering) => match20(_Array_get16(i, stmts)).with({ _tag: "None" }, () => Ok13(res)).with((_v) => {
  const _g = _v;
  return _g._tag === "Some" && _g.value._tag === "SImport";
}, ({ value: { names, from } }) => ((dp) => ((depExports) => ((depReg) => ((depKeys) => match20(resolveNames(names, from, depExports, depReg, depKeys, res, recovering)).with({ _tag: "Err" }, ({ error: e }) => Err11(e)).with({ _tag: "Ok" }, ({ value: res1 }) => resolveImportsFrom(ctx, stmts, i + 1, path, res1, recovering)).exhaustive())(_Map_getOr8(new Map, dp, ctx.keysByPath)))(_Map_getOr8(emptyReg, dp, ctx.regByPath)))(_Map_getOr8(new Map, dp, ctx.exportsByPath)))(resolveImport2(path, from))).with((_v) => {
  const _g = _v;
  return _g._tag === "Some" && _g.value._tag === "SImportNs";
...
var compileAll = _curry21(3, (ctx, graph, opts) => match20(graph).with((_v) => {
  ...
  const _g = _v;
  return _g.length >= 1;
}, ([m, ...rest]) => match20(compileOne(ctx, m, false, eq18(length15(rest), 0), opts)).with({ _tag: "Err" }, ({ error: e }) => Err11(e)).with({ _tag: "Ok" }, ({ value: ctx1 }) => compileAll(ctx1, rest, opts)).exhaustive()).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
...
var recoverGraphFromWith = _curry21(3, (state, graph, opts) => compileAllRecovering(state.ctx, graph, state.errors, opts));
var recoverGraphFrom = _curry21(2, (state, graph) => recoverGraphFromWith(state, graph, defaultOpts2));
var compileGraphRecoveringWith = _curry21(2, (graph, opts) => {
...
var inferOne = _curry21(3, (ctx, loaded, opts) => match20(resolveImportsFrom(ctx, loaded.stmts, 0, loaded.path, { imports: new Map, nsImports: new Map, reg: emptyReg, keys: new Map, quals: new Map }, false)).with({ _tag: "Err" }, ({ error: e }) => Err11(atPath(loaded.path, e))).with({ _tag: "Ok" }, ({ value: res }) => match20(checkWith(loaded.stmts, res.reg, res.quals)).with({ _tag: "Err" }, ({ error: e }) => Err11(atPath(loaded.path, e))).with({ _tag: "Ok" }, () => match20(inferProgramImportsTypes(loaded.stmts, builtins, namespaces, openMode(loaded.src, opts.open), res.imports, res.nsImports, res.quals, None20)).with({ _tag: "Err" }, ({ error: e }) => Err11(atPath(loaded.path, e))).with({ _tag: "Ok" }, ({ value: r }) => Ok13({ exportsByPath: _Map_set11(loaded.path, exportedSchemes(loaded.stmts, r.env), ctx.exportsByPath), regByPath: _Map_set11(loaded.path, exportedRegistry(loaded.stmts), ctx.regByPath), keysByPath: _Map_set11(loaded.path, exportedCtorKeys(loaded.stmts), ctx.keysByPath), qualsByPath: _Map_set11(loaded.path, qualScopeOf(loaded.stmts), ctx.qualsByPath), aliases: mergeMap(r.aliases, ctx.aliases), outputs: [...ctx.outputs, { path: loaded.path, types: map14((hit) => ({ span: hit.span, ty: hit.ty, display: showType(widenLits(hit.ty)) }), r.types), aliases: mergeMap(r.aliases, ctx.aliases) }] })).exhaustive()).exhaustive()).exhaustive());
var inferAll = _curry21(3, (ctx, graph, opts) => match20(graph).with((_v) => _v.length === 0, () => Ok13(ctx)).with((_v) => _v.length >= 1, ([m, ...rest]) => match20(inferOne(ctx, m, opts)).with({ _tag: "Err" }, ({ error: e }) => Err11(e)).with({ _tag: "Ok" }, ({ value: ctx1 }) => inferAll(ctx1, rest, opts)).exhaustive()).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
...
var inferGraphTypesFromWith = _curry21(3, (state, graph, opts) => inferAll(state, graph, opts));
var inferGraphTypesFrom = _curry21(2, (state, graph) => inferGraphTypesFromWith(state, graph, defaultOpts2));
var inferGraphTypesWith = _curry21(2, (graph, opts) => _Result_flatMap9((state) => Ok13(state.outputs), inferGraphTypesFromWith(freshInferGraphState(), graph, opts)));
var inferGraphTypes = (graph) => inferGraphTypesWith(graph, defaultOpts2);
var buildModulesWith = _curry21(2, (entry, opts) => match20(loadGraph(entry)).with({ _tag: "Err" }, ({ error: e }) => Err11([e])).with({ _tag: "Ok" }, ({ value: graph }) => ((recovered) => eq18(length15(recovered.errors), 0) ? _Result_mapErr2((e) => [e], compileGraphWith(graph, opts)) : Err11(recovered.errors))(compileGraphRecoveringWith(graph, opts))).exhaustive());
var buildModules = (entry) => buildModulesWith(entry, defaultOpts2);
var relSpec2 = _curry21(2, relSpec);
var externDtsPath2 = _curry21(2, externDtsPath);
var isIdentChar = (c) => match20(_Str_codeAt7(0, c)).with({ _tag: "None" }, () => false).with({ _tag: "Some" }, ({ value: n }) => or11(or11(or11(or11(and13(n >= 48, n <= 57), and13(n >= 65, n <= 90)), and13(n >= 97, n <= 122)), eq18(n, 95)), eq18(n, 36))).exhaustive();
var endsAtBoundary = (part) => eq18(_Str_length5(part), 0) ? true : not12(isIdentChar(_Option_unwrapOr10("", _Str_get5(_Str_length5(part) - 1, part))));
var startsAtBoundary = (part) => eq18(_Str_length5(part), 0) ? true : not12(isIdentChar(_Option_unwrapOr10("", _Str_get5(0, part))));
var occursAsWordFrom = _curry21(2, (parts, i) => match20(_Array_get16(i, parts)).with({ _tag: "None" }, () => false).with({ _tag: "Some" }, ({ value: after }) => and13(_Option_mapOr(false, endsAtBoundary, _Array_get16(i - 1, parts)), startsAtBoundary(after)) ? true : occursAsWordFrom(parts, i + 1)).exhaustive());
var occursAsWord = _curry21(2, (name, text) => occursAsWordFrom(_Str_split4(name, text), 1));
var importedBinding = (spec) => {
  const parts = _Str_split4(" as ", spec);
  return _Str_trim2(_Option_unwrapOr10(spec, _Array_get16(length15(parts) - 1, parts)));
};
var bindingsInLine = _curry21(2, (line, acc) => match20(_Array_get16(1, _Str_split4("{", line))).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, ({ value: rest }) => match20(_Array_get16(0, _Str_split4("}", rest))).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, ({ value: names }) => reduce5(_curry21(2, (a, n) => _Set_add8(importedBinding(n), a)), acc, _Str_split4(",", names))).exhaustive()).exhaustive());
var valueImported = (ts) => reduce5(_curry21(2, (acc, line) => bindingsInLine(line, acc)), _Set_fromArray8([]), filter6(_Str_startsWith7("import {"), _Str_split4(`
`, ts)));
var ownTypesInto = _curry21(3, (stmts, path, acc) => reduce5(_curry21(2, (a, s) => match20(s).with({ _tag: "SType" }, ({ name }) => _Map_set11(name, path, a)).otherwise(() => a)), acc, stmts));
var typeOwnerOf = (graph) => reduce5(_curry21(2, (acc, m) => ownTypesInto(m.stmts, m.path, acc)), new Map, graph);
var localTypeNames = (stmts) => _Set_fromArray8(_Array_flatMap4((s) => match20(s).with({ _tag: "SType" }, ({ name }) => [name]).otherwise(() => []), stmts));
var groupByOwner = _curry21(2, (names, ctx) => reduce5(_curry21(2, (acc, name) => {
  const owner = _Map_getOr8("", name, ctx.typeOwner);
  return or11(or11(or11(eq18(owner, ctx.importer), _Set_has8(name, ctx.localTypes)), _Set_has8(name, ctx.bound)), not12(occursAsWord(name, ctx.ts))) ? acc : ((spec) => _Map_set11(spec, _Array_append14(name, _Map_getOr8([], spec, acc)), acc))(relSpec2(ctx.importer, owner));
}), new Map, names));
var crossModuleTypeImports = _curry21(4, (ts, importer, localTypes, typeOwner) => {
...
var externBindingsInto = _curry21(4, (stmts, path, env, acc) => reduce5(_curry21(2, (a, s) => match20(s).with({ _tag: "SExtern" }, ({ name, module: hostModule, imported, curried }) => _Str_startsWith7("mochi:", hostModule) ? a : match20(_Map_get12(name, env)).with({ _tag: "None" }, () => a).with({ _tag: "Some" }, ({ value: sc }) => ((dp) => _Map_set11(dp, _Array_append14({ imported, scheme: sc, curried }, _Map_getOr8([], dp, a)), a))(externDtsPath2(path, hostModule))).exhaustive()).otherwise(() => a)), acc, stmts));
var compileOneTs = _curry21(3, (ctx, loaded, opts) => match20(resolveImportsFrom(ctx, loaded.stmts, 0, loaded.path, { imports: new Map, nsImports: new Map, reg: emptyReg, keys: new Map, quals: new Map }, false)).with({ _tag: "Err" }, ({ error: e }) => Err11(atPath(loaded.path, e))).with({ _tag: "Ok" }, ({ value: res }) => match20(checkWith(loaded.stmts, res.reg, res.quals)).with({ _tag: "Err" }, ({ error: e }) => Err11(atPath(loaded.path, e))).with({ _tag: "Ok" }, () => match20(inferProgramImportsTypes(loaded.stmts, builtins, namespaces, openMode(loaded.src, opts.open), res.imports, res.nsImports, res.quals, None20)).with({ _tag: "Err" }, ({ error: e }) => Err11(atPath(loaded.path, e))).with({ _tag: "Ok" }, ({ value: r }) => ((body) => ((lines) => ((ts) => Ok13({ exportsByPath: _Map_set11(loaded.path, exportedSchemes(loaded.stmts, r.env), ctx.exportsByPath), regByPath: _Map_set11(loaded.path, exportedRegistry(loaded.stmts), ctx.regByPath), keysByPath: _Map_set11(loaded.path, exportedCtorKeys(loaded.stmts), ctx.keysByPath), qualsByPath: _Map_set11(loaded.path, qualScopeOf(loaded.stmts), ctx.qualsByPath), aliases: mergeMap(r.aliases, ctx.aliases), typeOwner: ctx.typeOwner, runtimeImport: ctx.runtimeImport, externs: externBindingsInto(loaded.stmts, loaded.path, r.env, ctx.externs), outputs: [...ctx.outputs, { path: loaded.path, js: ts }] }))(eq18(length15(lines), 0) ? body : `${_Str_join9(`
`, lines)}
...
var compileAllTs = _curry21(3, (ctx, graph, opts) => match20(graph).with((_v) => _v.length === 0, () => Ok13(_Array_concat10(ctx.outputs, externOutputs(ctx.externs)))).with((_v) => _v.length >= 1, ([m, ...rest]) => match20(compileOneTs(ctx, m, opts)).with({ _tag: "Err" }, ({ error: e }) => Err11(e)).with({ _tag: "Ok" }, ({ value: ctx1 }) => compileAllTs(ctx1, rest, opts)).exhaustive()).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var compileGraphTsWith = _curry21(3, (graph, runtimeImport, opts) => compileAllTs({ exportsByPath: new Map, regByPath: new Map, keysByPath: new Map, qualsByPath: new Map, aliases: new Map, typeOwner: typeOwnerOf(graph), runtimeImport, externs: new Map, outputs: [] }, graph, opts));
var compileGraphTs = _curry21(2, (graph, runtimeImport) => compileGraphTsWith(graph, runtimeImport, defaultOpts2));
var dtsOne = _curry21(3, (ctx, loaded, opts) => match20(resolveImportsFrom(ctx, loaded.stmts, 0, loaded.path, { imports: new Map, nsImports: new Map, reg: emptyReg, keys: new Map, quals: new Map }, false)).with({ _tag: "Err" }, ({ error: e }) => Err11(atPath(loaded.path, e))).with({ _tag: "Ok" }, ({ value: res }) => match20(checkWith(loaded.stmts, res.reg, res.quals)).with({ _tag: "Err" }, ({ error: e }) => Err11(atPath(loaded.path, e))).with({ _tag: "Ok" }, () => match20(inferProgramImportsTypes(loaded.stmts, builtins, namespaces, openMode(loaded.src, opts.open), res.imports, res.nsImports, res.quals, None20)).with({ _tag: "Err" }, ({ error: e }) => Err11(atPath(loaded.path, e))).with({ _tag: "Ok" }, ({ value: r }) => Ok13({ exportsByPath: _Map_set11(loaded.path, exportedSchemes(loaded.stmts, r.env), ctx.exportsByPath), regByPath: _Map_set11(loaded.path, exportedRegistry(loaded.stmts), ctx.regByPath), keysByPath: _Map_set11(loaded.path, exportedCtorKeys(loaded.stmts), ctx.keysByPath), qualsByPath: _Map_set11(loaded.path, qualScopeOf(loaded.stmts), ctx.qualsByPath), aliases: mergeMap(r.aliases, ctx.aliases), runtimeImport: ctx.runtimeImport, target: ctx.target, dts: eq18(loaded.path, ctx.target) ? emitDtsFromTypedWith(loaded.stmts, r.env, mergeMap(r.aliases, ctx.aliases), qualifierMapOf(res.quals, localTypeNames(loaded.stmts)), ctx.runtimeImport, opts.docs) : ctx.dts })).exhaustive()).exhaustive()).exhaustive());
var dtsAll = _curry21(3, (ctx, graph, opts) => match20(graph).with((_v) => _v.length === 0, () => Ok13(ctx.dts)).with((_v) => _v.length >= 1, ([m, ...rest]) => match20(dtsOne(ctx, m, opts)).with({ _tag: "Err" }, ({ error: e }) => Err11(e)).with({ _tag: "Ok" }, ({ value: ctx1 }) => dtsAll(ctx1, rest, opts)).exhaustive()).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
