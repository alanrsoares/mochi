...
import { Err as Err7, None as None9, Ok as Ok7, Some as Some9, _Array_append as _Array_append7, _Array_contains as _Array_contains2, _Array_flatMap as _Array_flatMap2, _Array_get as _Array_get7, _Array_head as _Array_head3, _Map_get as _Map_get4, _Map_getOr as _Map_getOr2, _Map_has as _Map_has2, _Map_keys as _Map_keys3, _Map_set as _Map_set3, _Option_isNone, _Option_isSome as _Option_isSome2, _Option_orElse, _Option_unwrapOr as _Option_unwrapOr6, _Result_flatMap as _Result_flatMap5, _Set_add, _Set_fromArray, _Set_has, _Str_codeAt as _Str_codeAt4, _Str_join as _Str_join4, _curry as _curry10, _done as _done5, _recur as _recur5, _tuple as _tuple4, and as and6, eq as eq9, filter as filter3, length as length8, map as map6, not as not6, or as or5, show as show5 } from "@mochi/compiler/runtime";
import { match as match9 } from "@onrails/pattern";

import { Err as Err6, Ok as Ok6, Some as Some7, _Array_get as _Array_get5, _Array_prepend as _Array_prepend3, _Map_has, _Map_set as _Map_set2, _Option_unwrapOr as _Option_unwrapOr4, _Result_flatMap as _Result_flatMap4, _Result_map as _Result_map4, _curry as _curry8, _done as _done4, _recur as _recur4, eq as eq7, filter, length as length6, map as map4, not as not4, show as show3 } from "@mochi/compiler/runtime";
...
var keysOfFrom = _curry8(2, (fields, i) => match7(_Array_get5(i, fields)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: f }) => _Array_prepend3(_Option_unwrapOr4(`_${show3(i)}`, f.name), keysOfFrom(fields, i + 1))).exhaustive());
...
var seedRegCtorsFrom = _curry8(4, (ctors, i, owner, acc) => match7(_Array_get5(i, ctors)).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, ({ value: c }) => seedRegCtorsFrom(ctors, i + 1, owner, _Map_has(c.name, acc) ? acc : _Map_set2(c.name, { owner, arity: length6(c.fields) }, acc))).exhaustive());
var seedRegDeclsFrom = _curry8(3, (decls, i, reg) => match7(_Array_get5(i, decls)).with({ _tag: "None" }, () => reg).with({ _tag: "Some" }, ({ value: bt }) => seedRegDeclsFrom(decls, i + 1, { ctors: seedRegCtorsFrom(bt.ctors, 0, bt.name, reg.ctors), types: _Map_set2(bt.name, map4((c) => c.name, bt.ctors), reg.types) })).exhaustive());
var ctorErr = _curry8(2, (message, sp) => ({ message, start: sp.start, end: sp.end }));
var ctorsInto = _curry8(5, (ctors, i, owner, sp, acc) => match7(_Array_get5(i, ctors)).with({ _tag: "None" }, () => Ok6(acc)).with({ _tag: "Some" }, ({ value: c }) => _Map_has(c.name, acc) ? Err6(ctorErr(`duplicate constructor '${c.name}'`, sp)) : ctorsInto(ctors, i + 1, owner, sp, _Map_set2(c.name, { owner, arity: length6(c.fields) }, acc))).exhaustive());
var buildLoop = _curry8(3, (stmts, i, reg) => match7(_Array_get5(i, stmts)).with({ _tag: "None" }, () => Ok6(reg)).with((_v) => {
  const _g = _v;
  return _g._tag === "Some" && _g.value._tag === "SType";
...
var MRecord = _curry9(2, (labels, pats) => ({ _tag: "MRecord", labels, pats }));
var MArr = _curry9(2, (elems, rest) => ({ _tag: "MArr", elems, rest }));
...
var USome = _curry9(2, (row, fuel) => ({ _tag: "USome", row, fuel }));
...
var isWildMP = (mp) => match8(mp).with({ _tag: "MWild" }, () => true).otherwise(() => false);
var explodePat = (p) => match8(p).with({ _tag: "PAs" }, ({ pat }) => explodePat(pat)).with({ _tag: "POr" }, ({ alts }) => _Array_flatMap(explodePat, alts)).otherwise(() => [p]);
var toMP = (p) => match8(p).with({ _tag: "PAs" }, ({ pat }) => toMP(pat)).with({ _tag: "PWild" }, () => MWild).with({ _tag: "PUnit" }, () => MWild).with({ _tag: "PBind" }, () => MWild).with({ _tag: "PLit" }, ({ value: v }) => MNum(v)).with({ _tag: "PBool" }, ({ value: v }) => MBool(v)).with({ _tag: "PStr" }, ({ value: v }) => MStr(v)).with({ _tag: "PTuple" }, ({ elems }) => MTuple(map5(toMP, elems))).with({ _tag: "PCtor" }, ({ ctor: name, args }) => MCtor(name, map5(toMP, args))).with({ _tag: "PRecord" }, ({ fields }) => MRecord(map5((f) => f.label, fields), map5((f) => toMP(f.pat), fields))).with({ _tag: "PArr" }, ({ elems, rest }) => MArr(map5(toMP, elems), _Option_isSome(rest))).with({ _tag: "PList" }, () => MOpaque).with({ _tag: "POr" }, () => MOpaque).exhaustive();
var headOf = (mp) => match8(mp).with({ _tag: "MWild" }, () => None8).with({ _tag: "MOpaque" }, () => None8).with({ _tag: "MCtor" }, ({ name: n }) => Some8(HCtor(n))).with({ _tag: "MBool" }, ({ value: v }) => Some8(HBool(v))).with({ _tag: "MNum" }, ({ value: v }) => Some8(HNum(v))).with({ _tag: "MStr" }, ({ value: v }) => Some8(HStr(v))).with({ _tag: "MTuple" }, ({ elems }) => Some8(HTuple(length7(elems)))).with({ _tag: "MRecord" }, () => Some8(HRecord)).with({ _tag: "MArr" }, ({ elems }) => Some8(HArr(length7(elems)))).exhaustive();
var colOf = (m) => _Array_flatMap((row) => match8(_Array_head2(row)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: hd }) => [hd]).exhaustive(), m);
var headsOf = (col) => _Array_flatMap((mp) => match8(headOf(mp)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: h }) => [h]).exhaustive(), col);
var addLabel = _curry9(2, (acc, l) => _Array_contains(l, acc) ? acc : _Array_append6(l, acc));
var labelsOfMP = _curry9(2, (acc, mp) => match8(mp).with({ _tag: "MRecord" }, ({ labels: ls }) => reduce(addLabel, acc, ls)).otherwise(() => acc));
var recordLabelsOf = (col) => reduce(labelsOfMP, [], col);
var indexOfLabel = _curry9(3, (l, labels, i) => match8(_Array_get6(i, labels)).with({ _tag: "None" }, () => 0 - 1).with({ _tag: "Some" }, ({ value: x }) => eq8(x, l) ? i : indexOfLabel(l, labels, i + 1)).exhaustive());
var fieldOf = _curry9(3, (l, labels, pats) => {
  const i = indexOfLabel(l, labels, 0);
  return i < 0 ? MWild : _Option_unwrapOr5(MWild, _Array_get6(i, pats));
});
var arrShapeStep = _curry9(2, (acc, mp) => match8(mp).with({ _tag: "MArr" }, ({ elems, rest }) => ((n) => rest ? { fixed: acc.fixed, restFrom: match8(acc.restFrom).with({ _tag: "None" }, () => Some8(n)).with({ _tag: "Some" }, ({ value: m }) => Some8(m < n ? m : n)).exhaustive() } : { fixed: _Array_contains(n, acc.fixed) ? acc.fixed : _Array_append6(n, acc.fixed), restFrom: acc.restFrom })(length7(elems))).otherwise(() => acc));
var arrShapeOf = (col) => reduce(arrShapeStep, { fixed: [], restFrom: None8 }, col);
var rangeCovered = _curry9(3, (shape, i, n) => i >= n ? true : and5(_Array_contains(i, shape.fixed), rangeCovered(shape, i + 1, n)));
var arrComplete = (shape) => match8(shape.restFrom).with({ _tag: "None" }, () => false).with({ _tag: "Some" }, ({ value: r }) => rangeCovered(shape, 0, r)).exhaustive();
var arrMissingLen = _curry9(2, (shape, n) => and5(not5(_Array_contains(n, shape.fixed)), match8(shape.restFrom).with({ _tag: "None" }, () => true).with({ _tag: "Some" }, ({ value: r }) => n < r).exhaustive()) ? n : arrMissingLen(shape, n + 1));
var rangeArr = _curry9(2, (i, top) => i > top ? [] : _Array_prepend4(i, rangeArr(i + 1, top)));
var arrLengths = (shape) => {
  const top = reduce(_curry9(2, (a, x) => x > a ? x : a), _Option_unwrapOr5(0, shape.restFrom), shape.fixed);
  return rangeArr(0, top);
};
var specializeRow = _curry9(3, (h, mp, labels) => match8(h).with({ _tag: "HCtor" }, ({ name }) => match8(mp).with({ _tag: "MCtor" }, ({ name: n, args }) => eq8(n, name) ? Some8(args) : None8).otherwise(() => None8)).with({ _tag: "HBool" }, ({ value: v }) => match8(mp).with({ _tag: "MBool" }, ({ value: b }) => eq8(b, v) ? Some8([]) : None8).otherwise(() => None8)).with({ _tag: "HNum" }, ({ value: v }) => match8(mp).with({ _tag: "MNum" }, ({ value: x }) => eq8(x, v) ? Some8([]) : None8).otherwise(() => None8)).with({ _tag: "HStr" }, ({ value: v }) => match8(mp).with({ _tag: "MStr" }, ({ value: x }) => eq8(x, v) ? Some8([]) : None8).otherwise(() => None8)).with({ _tag: "HTuple" }, () => match8(mp).with({ _tag: "MTuple" }, ({ elems }) => Some8(elems)).otherwise(() => None8)).with({ _tag: "HRecord" }, () => match8(mp).with({ _tag: "MRecord" }, ({ labels: ls, pats: ps }) => Some8(map5((l) => fieldOf(l, ls, ps), labels))).otherwise(() => None8)).with({ _tag: "HArr" }, ({ len }) => match8(mp).with({ _tag: "MArr" }, ({ elems, rest }) => ((k) => rest ? k <= len ? Some8(_Array_concat3(elems, mWilds(len - k))) : None8 : eq8(k, len) ? Some8(elems) : None8)(length7(elems))).otherwise(() => None8)).exhaustive());
var specializeOne = _curry9(4, (h, arity, labels, row) => match8(_Array_head2(row)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: hd }) => ((rest) => isWildMP(hd) ? [_Array_concat3(mWilds(arity), rest)] : match8(specializeRow(h, hd, labels)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: sub3 }) => [_Array_concat3(sub3, rest)]).exhaustive())(_Array_tail2(row))).exhaustive());
var specializeM = _curry9(4, (m, h, arity, labels) => _Array_flatMap((row) => specializeOne(h, arity, labels, row), m));
var defaultM = (m) => _Array_flatMap((row) => match8(_Array_head2(row)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: hd }) => isWildMP(hd) ? [_Array_tail2(row)] : []).exhaustive(), m);
var rebuild = _curry9(3, (h, args, labels) => match8(h).with({ _tag: "HCtor" }, ({ name }) => MCtor(name, args)).with({ _tag: "HTuple" }, () => MTuple(args)).with({ _tag: "HRecord" }, () => MRecord(labels, args)).with({ _tag: "HArr" }, () => MArr(args, false)).with({ _tag: "HBool" }, ({ value: v }) => MBool(v)).with({ _tag: "HNum" }, ({ value: v }) => MNum(v)).with({ _tag: "HStr" }, ({ value: v }) => MStr(v)).exhaustive());
var takenNums = (heads) => _Array_flatMap((h) => match8(h).with({ _tag: "HNum" }, ({ value: v }) => [v]).otherwise(() => []), heads);
var freshNum = _curry9(2, (taken, i) => _Array_contains(i, taken) ? freshNum(taken, i + 1) : i);
var takenStrs = (heads) => _Array_flatMap((h) => match8(h).with({ _tag: "HStr" }, ({ value: v }) => [v]).otherwise(() => []), heads);
var starsOf = (n) => n <= 0 ? "" : _Str_concat("*", starsOf(n - 1));
var freshStr = _curry9(2, (taken, i) => {
  const s = starsOf(i);
  return _Array_contains(s, taken) ? freshStr(taken, i + 1) : s;
});
var ctorNames = (heads) => _Array_flatMap((h) => match8(h).with({ _tag: "HCtor" }, ({ name: n }) => [n]).otherwise(() => []), heads);
var boolVals = (heads) => _Array_flatMap((h) => match8(h).with({ _tag: "HBool" }, ({ value: v }) => [v]).otherwise(() => []), heads);
var ctorInfoSuffixed = _curry9(3, (keys, reg, n) => match8(keys).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => None8).with((_v) => {
  const _g = _v;
...
var ctorInfoOf = _curry9(2, (reg, n) => match8(_Map_get3(n, reg.ctors)).with({ _tag: "Some" }, ({ value: info }) => Some8(info)).with({ _tag: "None" }, () => ctorInfoSuffixed(_Map_keys2(reg.ctors), reg, n)).exhaustive());
var arityOfCtor = _curry9(2, (reg, n) => match8(ctorInfoOf(reg, n)).with({ _tag: "None" }, () => 0).with({ _tag: "Some" }, ({ value: info }) => info.arity).exhaustive());
var ownerOfCtor = _curry9(2, (reg, n) => match8(ctorInfoOf(reg, n)).with({ _tag: "None" }, () => None8).with({ _tag: "Some" }, ({ value: info }) => Some8(info.owner)).exhaustive());
var allNamesIn = _curry9(2, (all, names) => reduce(_curry9(2, (acc, n) => and5(acc, _Array_contains(n, names))), true, all));
var useful = _curry9(4, (m, width, reg, fuel) => fuel <= 0 ? UFuel : eq8(width, 0) ? eq8(length7(m), 0) ? USome([], fuel - 1) : UNone(fuel - 1) : eq8(length7(m), 0) ? USome(mWilds(width), fuel - 1) : usefulSplit(m, width, reg, fuel - 1));
var usefulSplit = _curry9(4, (m, width, reg, fuel) => {
  const col = colOf(m);
  const heads = headsOf(col);
  return match8(_Array_head2(heads)).with({ _tag: "None" }, () => prependWitness(MWild, useful(defaultM(m), width - 1, reg, fuel))).with({ _tag: "Some" }, ({ value: h0 }) => usefulHead(m, col, heads, h0, width, reg, fuel)).exhaustive();
});
var prependWitness = _curry9(2, (mp, r) => match8(r).with({ _tag: "UFuel" }, () => UFuel).with({ _tag: "UNone" }, ({ fuel: f }) => UNone(f)).with({ _tag: "USome" }, ({ row, fuel: f }) => USome(_Array_prepend4(mp, row), f)).exhaustive());
var tryHeads = _curry9(8, (m, heads, arities, labels, width, reg, fuel, i) => match8(_Array_get6(i, heads)).with({ _tag: "None" }, () => UNone(fuel)).with({ _tag: "Some" }, ({ value: h }) => ((arity) => match8(useful(specializeM(m, h, arity, labels), arity + width - 1, reg, fuel)).with({ _tag: "UFuel" }, () => UFuel).with({ _tag: "UNone" }, ({ fuel: f2 }) => tryHeads(m, heads, arities, labels, width, reg, f2, i + 1)).with({ _tag: "USome" }, ({ row, fuel: f2 }) => USome(_Array_prepend4(rebuild(h, _Array_take(arity, row), labels), _Array_drop(arity, row)), f2)).exhaustive())(_Option_unwrapOr5(0, _Array_get6(i, arities)))).exhaustive());
var usefulHead = _curry9(7, (m, col, heads, h0, width, reg, fuel) => match8(h0).with({ _tag: "HTuple" }, ({ arity }) => tryHeads(m, [HTuple(arity)], [arity], [], width, reg, fuel, 0)).with({ _tag: "HRecord" }, () => ((labels) => tryHeads(m, [HRecord], [length7(labels)], labels, width, reg, fuel, 0))(recordLabelsOf(col))).with({ _tag: "HCtor" }, () => usefulCtor(m, heads, width, reg, fuel)).with({ _tag: "HBool" }, () => usefulBool(m, heads, width, reg, fuel)).with({ _tag: "HArr" }, () => usefulArr(m, col, width, reg, fuel)).with({ _tag: "HNum" }, () => prependWitness(MNum(freshNum(takenNums(heads), 0)), useful(defaultM(m), width - 1, reg, fuel))).with({ _tag: "HStr" }, () => prependWitness(MStr(freshStr(takenStrs(heads), 0)), useful(defaultM(m), width - 1, reg, fuel))).exhaustive());
var usefulCtor = _curry9(5, (m, heads, width, reg, fuel) => {
  const names = ctorNames(heads);
  const ownerOpt = match8(_Array_head2(names)).with({ _tag: "None" }, () => None8).with({ _tag: "Some" }, ({ value: n }) => ownerOfCtor(reg, n)).exhaustive();
  const all = match8(ownerOpt).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: o }) => _Map_getOr([], o, reg.types)).exhaustive();
  return and5(length7(all) > 0, allNamesIn(all, names)) ? tryHeads(m, map5((n) => HCtor(n), all), map5((n) => arityOfCtor(reg, n), all), [], width, reg, fuel, 0) : prependWitness(match8(_Array_head2(filter2((n) => not5(_Array_contains(n, names)), all))).with({ _tag: "None" }, () => MWild).with({ _tag: "Some" }, ({ value: n }) => MCtor(n, mWilds(arityOfCtor(reg, n)))).exhaustive(), useful(defaultM(m), width - 1, reg, fuel));
});
var usefulBool = _curry9(5, (m, heads, width, reg, fuel) => {
...
var usefulArr = _curry9(5, (m, col, width, reg, fuel) => {
...
var showFields = _curry9(3, (labels, pats, i) => match8(_Array_get6(i, labels)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: l }) => _Array_prepend4(`${l}: ${showWitness(_Option_unwrapOr5(MWild, _Array_get6(i, pats)))}`, showFields(labels, pats, i + 1))).exhaustive());
var showWitness = (mp) => match8(mp).with({ _tag: "MWild" }, () => "_").with({ _tag: "MOpaque" }, () => "_").with({ _tag: "MBool" }, ({ value: v }) => show4(v)).with({ _tag: "MNum" }, ({ value: v }) => show4(v)).with({ _tag: "MStr" }, ({ value: v }) => show4(v)).with({ _tag: "MCtor" }, ({ name: n, args }) => eq8(length7(args), 0) ? n : `${n}(${_Str_join3(", ", map5(showWitness, args))})`).with({ _tag: "MTuple" }, ({ elems }) => `(${_Str_join3(", ", map5(showWitness, elems))})`).with({ _tag: "MRecord" }, ({ labels, pats }) => `{ ${_Str_join3(", ", showFields(labels, pats, 0))} }`).with({ _tag: "MArr" }, ({ elems, rest }) => `[${_Str_join3(", ", _Array_concat3(map5(showWitness, elems), rest ? ["..."] : []))}]`).exhaustive();
var isWideWitnessM = (mp) => match8(mp).with({ _tag: "MWild" }, () => true).with({ _tag: "MCtor" }, ({ args }) => reduce(_curry9(2, (acc, a) => and5(acc, isWildMP(a))), true, args)).otherwise(() => false);
var checkExhaustiveM = _curry9(2, (patterns, reg) => {
  const rows = _Array_flatMap((p) => map5((alt) => [toMP(alt)], explodePat(p)), patterns);
  return match8(useful(rows, 1, reg, 20000)).with({ _tag: "UFuel" }, () => ExFuel).with({ _tag: "UNone" }, () => ExOk).with({ _tag: "USome" }, ({ row }) => ExWitness(_Option_unwrapOr5(MWild, _Array_head2(row)))).exhaustive();
});

var checkErr = _curry10(2, (message, sp) => ({ message, start: sp.start, end: sp.end }));
var firstSomeFrom = _curry10(3, (f, xs, i0) => {
  let i = i0;
  while (true) {
    const _step = match9(_Array_get7(i, xs)).with({ _tag: "None" }, () => _done5(None9)).with({ _tag: "Some" }, ({ value: x }) => match9(f(x)).with({ _tag: "Some" }, ({ value: e }) => _done5(Some9(e))).with({ _tag: "None" }, () => _recur5(i + 1)).exhaustive()).exhaustive();
    if (_step._tag === "recur") {
      i = _step.args[0];
      continue;
    }
...
var firstSome = _curry10(2, (f, xs) => firstSomeFrom(f, xs, 0));
var allOfFrom = _curry10(3, (f, xs, i0) => {
  let i = i0;
  while (true) {
    const _step = match9(_Array_get7(i, xs)).with({ _tag: "None" }, () => _done5(true)).with({ _tag: "Some" }, ({ value: x }) => f(x) ? _recur5(i + 1) : _done5(false)).exhaustive();
    if (_step._tag === "recur") {
      i = _step.args[0];
      continue;
    }
...
var allOf = _curry10(2, (f, xs) => allOfFrom(f, xs, 0));
var someOfFrom2 = _curry10(3, (f, xs, i0) => {
  let i = i0;
  while (true) {
    const _step = match9(_Array_get7(i, xs)).with({ _tag: "None" }, () => _done5(false)).with({ _tag: "Some" }, ({ value: x }) => f(x) ? _done5(true) : _recur5(i + 1)).exhaustive();
    if (_step._tag === "recur") {
      i = _step.args[0];
      continue;
    }
...
var someOf2 = _curry10(2, (f, xs) => someOfFrom2(f, xs, 0));
var exprSpan2 = (e) => match9(e).with({ _tag: "ENum" }, ({ span: sp }) => sp).with({ _tag: "EUnit" }, ({ span: sp }) => sp).with({ _tag: "EBool" }, ({ span: sp }) => sp).with({ _tag: "EStr" }, ({ span: sp }) => sp).with({ _tag: "ERef" }, ({ span: sp }) => sp).with({ _tag: "ECall" }, ({ span: sp }) => sp).with({ _tag: "ELambda" }, ({ span: sp }) => sp).with({ _tag: "ELetIn" }, ({ span: sp }) => sp).with({ _tag: "ELetBind" }, ({ span: sp }) => sp).with({ _tag: "EPipe" }, ({ span: sp }) => sp).with({ _tag: "EDo" }, ({ span: sp }) => sp).with({ _tag: "ETernary" }, ({ span: sp }) => sp).with({ _tag: "EMatch" }, ({ span: sp }) => sp).with({ _tag: "ELoop" }, ({ span: sp }) => sp).with({ _tag: "ERecur" }, ({ span: sp }) => sp).with({ _tag: "ERecord" }, ({ span: sp }) => sp).with({ _tag: "EField" }, ({ span: sp }) => sp).with({ _tag: "ETuple" }, ({ span: sp }) => sp).with({ _tag: "EArr" }, ({ span: sp }) => sp).with({ _tag: "EList" }, ({ span: sp }) => sp).with({ _tag: "ESet" }, ({ span: sp }) => sp).with({ _tag: "EMap" }, ({ span: sp }) => sp).with({ _tag: "EInterp" }, ({ span: sp }) => sp).exhaustive();
var patSpan2 = (p) => match9(p).with({ _tag: "PWild" }, ({ span: sp }) => sp).with({ _tag: "PUnit" }, ({ span: sp }) => sp).with({ _tag: "PBind" }, ({ span: sp }) => sp).with({ _tag: "PAs" }, ({ span: sp }) => sp).with({ _tag: "PLit" }, ({ span: sp }) => sp).with({ _tag: "PBool" }, ({ span: sp }) => sp).with({ _tag: "PStr" }, ({ span: sp }) => sp).with({ _tag: "PTuple" }, ({ span: sp }) => sp).with({ _tag: "PRecord" }, ({ span: sp }) => sp).with({ _tag: "PCtor" }, ({ span: sp }) => sp).with({ _tag: "PArr" }, ({ span: sp }) => sp).with({ _tag: "PList" }, ({ span: sp }) => sp).with({ _tag: "POr" }, ({ span: sp }) => sp).exhaustive();
var isCatchAll = (p) => match9(p).with({ _tag: "PWild" }, () => true).with({ _tag: "PUnit" }, () => true).with({ _tag: "PBind" }, () => true).with({ _tag: "PAs" }, ({ pat }) => isCatchAll(pat)).with({ _tag: "PRecord" }, ({ fields }) => allOf((f) => isCatchAll(f.pat), fields)).with({ _tag: "PTuple" }, ({ elems }) => allOf(isCatchAll, elems)).with({ _tag: "PArr" }, ({ elems, rest }) => and6(eq9(length8(elems), 0), _Option_isSome2(rest))).with({ _tag: "PList" }, ({ elems, rest }) => and6(eq9(length8(elems), 0), _Option_isSome2(rest))).otherwise(() => false);
var isPList = (p) => match9(p).with({ _tag: "PList" }, () => true).otherwise(() => false);
var isPCtor = (p) => match9(p).with({ _tag: "PCtor" }, () => true).otherwise(() => false);
var ctorNameOf = (p) => match9(p).with({ _tag: "PCtor" }, ({ ctor: name }) => name).otherwise(() => "");
var patCtorKey = _curry10(2, (ctor, ns) => match9(ns).with({ _tag: "Some" }, ({ value: alias }) => `${alias}.${ctor}`).with({ _tag: "None" }, () => ctor).exhaustive());
var seqElemsRest = (p) => match9(p).with({ _tag: "PArr" }, ({ elems, rest }) => Some9(_tuple4(elems, rest))).with({ _tag: "PList" }, ({ elems, rest }) => Some9(_tuple4(elems, rest))).otherwise(() => None9);
var checkPattern = _curry10(3, (p, reg, top) => match9(p).with({ _tag: "PAs" }, ({ pat }) => checkPattern(pat, reg, top)).with({ _tag: "PCtor" }, ({ ctor, args, ns, span: sp }) => ((key) => match9(_Map_get4(key, reg.ctors)).with({ _tag: "None" }, () => Some9(checkErr(`unknown constructor '${key}'`, sp))).with({ _tag: "Some" }, ({ value: info }) => eq9(length8(args), info.arity) ? firstSome((a) => checkPattern(a, reg, false), args) : Some9(checkErr(`constructor '${ctor}' expects ${show5(info.arity)} arg(s), got ${show5(length8(args))}`, sp))).exhaustive())(patCtorKey(ctor, ns))).with({ _tag: "PRecord" }, ({ fields }) => firstSome((f) => checkPattern(f.pat, reg, false), fields)).with({ _tag: "PTuple" }, ({ elems }) => firstSome((el) => checkPattern(el, reg, false), elems)).with({ _tag: "PArr" }, ({ elems, rest }) => _Option_orElse(match9(rest).with({ _tag: "Some" }, ({ value: r }) => checkPattern(r, reg, false)).with({ _tag: "None" }, () => None9).exhaustive(), firstSome((el) => checkPattern(el, reg, false), elems))).with({ _tag: "PList" }, ({ elems, rest, span: sp }) => top ? _Option_orElse(match9(rest).with({ _tag: "Some" }, ({ value: r }) => checkPattern(r, reg, false)).with({ _tag: "None" }, () => None9).exhaustive(), firstSome((el) => checkPattern(el, reg, false), elems)) : Some9(checkErr("lazy-List pattern cannot nest inside another pattern (matching pulls from the sequence)", sp))).with({ _tag: "POr" }, ({ alts, span: sp }) => checkOrPattern(alts, sp, reg)).otherwise(() => None9));
var binderPathsArgs = _curry10(4, (args, i, at, acc) => match9(_Array_get7(i, args)).with({ _tag: "None" }, () => Ok7(acc)).with({ _tag: "Some" }, ({ value: a }) => _Result_flatMap5((acc2) => binderPathsArgs(args, i + 1, at, acc2), binderPaths(a, `${at}.a${show5(i)}`, acc))).exhaustive());
var binderPathsFields = _curry10(4, (fields, i, at, acc) => match9(_Array_get7(i, fields)).with({ _tag: "None" }, () => Ok7(acc)).with({ _tag: "Some" }, ({ value: f }) => _Result_flatMap5((acc2) => binderPathsFields(fields, i + 1, at, acc2), binderPaths(f.pat, `${at}.${f.label}`, acc))).exhaustive());
var binderPathsElems = _curry10(4, (elems, i, at, acc) => match9(_Array_get7(i, elems)).with({ _tag: "None" }, () => Ok7(acc)).with({ _tag: "Some" }, ({ value: e }) => _Result_flatMap5((acc2) => binderPathsElems(elems, i + 1, at, acc2), binderPaths(e, `${at}.t${show5(i)}`, acc))).exhaustive());
var binderPaths = _curry10(3, (p, at, acc) => match9(p).with({ _tag: "PAs" }, ({ pat, name, nameSpan: nameSp }) => _Result_flatMap5((acc1) => _Map_has2(name, acc1) ? Err7(checkErr(`pattern binds '${name}' more than once`, nameSp)) : Ok7(_Map_set3(name, at, acc1)), binderPaths(pat, at, acc))).with({ _tag: "PBind" }, ({ name, span: sp }) => _Map_has2(name, acc) ? Err7(checkErr(`pattern binds '${name}' more than once`, sp)) : Ok7(_Map_set3(name, at, acc))).with({ _tag: "PCtor" }, ({ args }) => binderPathsArgs(args, 0, at, acc)).with({ _tag: "PRecord" }, ({ fields }) => binderPathsFields(fields, 0, at, acc)).with({ _tag: "PTuple" }, ({ elems }) => binderPathsElems(elems, 0, at, acc)).otherwise(() => Ok7(acc)));
var altMapsFrom = _curry10(4, (alts, i, reg, acc) => match9(_Array_get7(i, alts)).with({ _tag: "None" }, () => Ok7(acc)).with({ _tag: "Some" }, ({ value: alt }) => isCatchAll(alt) ? Err7(checkErr("an or-pattern alternative can't be a catch-all (`_` or a bare binding)", patSpan2(alt))) : _Option_isSome2(seqElemsRest(alt)) ? Err7(checkErr("array/list patterns can't appear as an or-pattern alternative", patSpan2(alt))) : match9(checkPattern(alt, reg, false)).with({ _tag: "Some" }, ({ value: e }) => Err7(e)).with({ _tag: "None" }, () => _Result_flatMap5((m) => altMapsFrom(alts, i + 1, reg, _Array_append7(m, acc)), binderPaths(alt, "", new Map))).exhaustive()).exhaustive());
var missingNameErr = _curry10(2, (name, sp) => checkErr(`or-pattern alternatives must bind the same names ('${name}' is missing in an alternative)`, sp));
var consistentBindsFrom = _curry10(4, (maps, i, ref, sp) => match9(_Array_get7(i, maps)).with({ _tag: "None" }, () => None9).with({ _tag: "Some" }, ({ value: m }) => _Option_orElse(consistentBindsFrom(maps, i + 1, ref, sp), _Option_orElse(firstSome((name) => _Map_has2(name, ref) ? eq9(_Map_getOr2("", name, ref), _Map_getOr2("", name, m)) ? None9 : Some9(checkErr(`or-pattern binds '${name}' at a differing position across alternatives`, sp)) : Some9(missingNameErr(name, sp)), _Map_keys3(m)), firstSome((name) => _Map_has2(name, m) ? None9 : Some9(missingNameErr(name, sp)), _Map_keys3(ref))))).exhaustive());
var checkOrPattern = _curry10(3, (alts, sp, reg) => match9(altMapsFrom(alts, 0, reg, [])).with({ _tag: "Err" }, ({ error: e }) => Some9(e)).with({ _tag: "Ok" }, ({ value: maps }) => match9(_Array_head3(maps)).with({ _tag: "None" }, () => None9).with({ _tag: "Some" }, ({ value: ref }) => consistentBindsFrom(maps, 1, ref, sp)).exhaustive()).exhaustive());
var armUnguardedCatchAll = (a) => and6(isCatchAll(a.pattern), _Option_isNone(a.guard));
var guardErrs = _curry10(2, (arms, listSwitch) => firstSome((a) => match9(a.guard).with({ _tag: "None" }, () => None9).with({ _tag: "Some" }, ({ value: g }) => or5(isPList(a.pattern), listSwitch) ? Some9(checkErr("`when` guards are unsupported in a lazy-List switch (matching pulls from the sequence)", exprSpan2(g))) : None9).exhaustive(), arms));
var firstCatchIdx = _curry10(2, (arms, i0) => {
  let i = i0;
  while (true) {
    const _step = match9(_Array_get7(i, arms)).with({ _tag: "None" }, () => _done5(None9)).with({ _tag: "Some" }, ({ value: a }) => armUnguardedCatchAll(a) ? _done5(Some9(i)) : _recur5(i + 1)).exhaustive();
    if (_step._tag === "recur") {
      i = _step.args[0];
      continue;
    }
...
var unreachableAfterCatch = (arms) => match9(firstCatchIdx(arms, 0)).with({ _tag: "None" }, () => None9).with({ _tag: "Some" }, ({ value: i }) => match9(_Array_get7(i + 1, arms)).with({ _tag: "None" }, () => None9).with({ _tag: "Some" }, ({ value: a }) => Some9(checkErr("unreachable arm: a catch-all arm above it matches first", patSpan2(a.pattern)))).exhaustive()).exhaustive();
var SeqNotSeq = { _tag: "SeqNotSeq" };
var SeqTotal = { _tag: "SeqTotal" };
var SeqFail = (e) => ({ _tag: "SeqFail", e });
var checkSeqExhaustive = _curry10(2, (arms, mSpan) => {
  const seqs = map6((a) => a.pattern, filter3((a) => and6(_Option_isNone(a.guard), _Option_isSome2(seqElemsRest(a.pattern))), arms));
  return eq9(length8(seqs), 0) ? SeqNotSeq : ((hasEmpty) => ((hasCons) => and6(hasEmpty, hasCons) ? SeqTotal : SeqFail(checkErr("non-exhaustive list switch: cover `[]` and `[x, ...xs]` (or add `_`)", mSpan)))(someOf2((p) => match9(seqElemsRest(p)).with((_v) => {
    const _g = _v;
    return _g._tag === "Some";
  }, ({ value: [elems, rest] }) => and6(eq9(length8(elems), 1), _Option_isSome2(rest))).with({ _tag: "None" }, () => false).exhaustive(), seqs)))(someOf2((p) => match9(seqElemsRest(p)).with((_v) => {
    const _g = _v;
    return _g._tag === "Some";
  }, ({ value: [elems, rest] }) => and6(eq9(length8(elems), 0), _Option_isNone(rest))).with({ _tag: "None" }, () => false).exhaustive(), seqs));
});
var ctorLoop = _curry10(5, (arms, i, reg, owner, covered) => match9(_Array_get7(i, arms)).with({ _tag: "None" }, () => Ok7(_tuple4(owner, covered))).with({ _tag: "Some" }, ({ value: a }) => match9(a.pattern).with({ _tag: "PCtor" }, ({ ctor, args, ns, span: sp }) => ((key) => match9(_Map_get4(key, reg.ctors)).with({ _tag: "None" }, () => Err7(checkErr(`unknown constructor '${key}'`, sp))).with({ _tag: "Some" }, ({ value: info }) => not6(eq9(length8(args), info.arity)) ? Err7(checkErr(`constructor '${ctor}' expects ${show5(info.arity)} arg(s), got ${show5(length8(args))}`, sp)) : match9(owner).with((_v) => {
  const _g = _v;
  return _g._tag === "Some" && (({ value: own }) => not6(eq9(own, info.owner)))(_g);
}, ({ value: own }) => Err7(checkErr(`switch mixes variants of '${own}' and '${info.owner}'`, sp))).otherwise(() => ((covered2) => ctorLoop(arms, i + 1, reg, Some9(info.owner), covered2))(and6(allOf(isCatchAll, args), _Option_isNone(a.guard)) ? _Set_add(ctor, covered) : covered))).exhaustive())(patCtorKey(ctor, ns))).otherwise(() => ctorLoop(arms, i + 1, reg, owner, covered))).exhaustive());
var seqVerdict = _curry10(2, (arms, mSpan) => match9(checkSeqExhaustive(arms, mSpan)).with({ _tag: "SeqTotal" }, () => None9).with({ _tag: "SeqFail" }, ({ e }) => Some9(e)).with({ _tag: "SeqNotSeq" }, () => None9).exhaustive());
...
var matrixVerdict = _curry10(5, (arms, leaves, ownerOpt, mSpan, reg) => match9(checkExhaustiveM(unguardedPatterns(arms), reg)).with({ _tag: "ExOk" }, () => None9).with({ _tag: "ExFuel" }, () => Some9(checkErr("switch too complex to prove exhaustive \u2014 add a `_` catch-all arm", mSpan))).with({ _tag: "ExWitness" }, ({ witness: w }) => ((own) => ((named) => ((absent) => and6(and6(isWideWitnessM(w), not6(eq9(own, ""))), length8(absent) > 0) ? Some9(checkErr(`non-exhaustive switch on '${own}': missing ${_Str_join4(", ", absent)}`, mSpan)) : Some9(checkErr(`non-exhaustive switch: '${showWitness(w)}' is not matched`, mSpan)))(filter3((c) => not6(_Set_has(c, named)), _Map_getOr2([], own, reg.types))))(namedUnguarded(leaves)))(_Option_unwrapOr6("", ownerOpt))).exhaustive());
var leavesOfArm = (a) => match9(a.pattern).with({ _tag: "POr" }, ({ alts }) => map6((alt) => ({ pattern: alt, guard: a.guard }), alts)).otherwise(() => [{ pattern: a.pattern, guard: a.guard }]);
var checkMatch = _curry10(3, (arms, mSpan, reg) => match9(firstSome((a) => checkPattern(a.pattern, reg, true), arms)).with({ _tag: "Some" }, ({ value: e }) => Some9(e)).with({ _tag: "None" }, () => ((listSwitch) => match9(guardErrs(arms, listSwitch)).with({ _tag: "Some" }, ({ value: e }) => Some9(e)).with({ _tag: "None" }, () => match9(unreachableAfterCatch(arms)).with({ _tag: "Some" }, ({ value: e }) => Some9(e)).with({ _tag: "None" }, () => ((hasCatchAll) => ((leaves) => ((ctorArms) => someOf2((a) => isPList(a.pattern), arms) ? hasCatchAll ? None9 : seqVerdict(arms, mSpan) : match9(ctorLoop(ctorArms, 0, reg, None9, _Set_fromArray([]))).with({ _tag: "Err" }, ({ error: e }) => Some9(e)).with((_v) => {
  const _g = _v;
  return _g._tag === "Ok";
...
var checkExpr = _curry10(2, (e, reg) => match9(e).with({ _tag: "ENum" }, () => None9).with({ _tag: "EUnit" }, () => None9).with({ _tag: "EBool" }, () => None9).with({ _tag: "EStr" }, () => None9).with({ _tag: "ERef" }, () => None9).with({ _tag: "ECall" }, ({ fn, args }) => _Option_orElse(firstSome((a) => checkExpr(a, reg), args), checkExpr(fn, reg))).with({ _tag: "ELambda" }, ({ body }) => checkExpr(body, reg)).with({ _tag: "ELetIn" }, ({ value, body }) => _Option_orElse(checkExpr(body, reg), checkExpr(value, reg))).with({ _tag: "ELetBind" }, ({ value, body }) => _Option_orElse(checkExpr(body, reg), checkExpr(value, reg))).with({ _tag: "EPipe" }, ({ left, right }) => _Option_orElse(checkExpr(right, reg), checkExpr(left, reg))).with({ _tag: "EDo" }, ({ exprs }) => firstSome((x) => checkExpr(x, reg), exprs)).with({ _tag: "ETernary" }, ({ cond, thenE, elseE }) => _Option_orElse(checkExpr(elseE, reg), _Option_orElse(checkExpr(thenE, reg), checkExpr(cond, reg)))).with({ _tag: "EMatch" }, ({ scrutinee, arms, span: sp }) => _Option_orElse(checkMatch(arms, sp, reg), _Option_orElse(firstSome((a) => _Option_orElse(checkExpr(a.body, reg), match9(a.guard).with({ _tag: "Some" }, ({ value: g }) => checkExpr(g, reg)).with({ _tag: "None" }, () => None9).exhaustive()), arms), checkExpr(scrutinee, reg)))).with({ _tag: "ERecord" }, ({ fields, spread }) => _Option_orElse(firstSome((f) => checkExpr(f.value, reg), fields), match9(spread).with({ _tag: "Some" }, ({ value: s }) => checkExpr(s, reg)).with({ _tag: "None" }, () => None9).exhaustive())).with({ _tag: "EField" }, ({ target }) => checkExpr(target, reg)).with({ _tag: "ELoop" }, ({ params, body }) => _Option_orElse(checkExpr(body, reg), firstSome((p) => checkExpr(p.init, reg), params))).with({ _tag: "ERecur" }, ({ args }) => firstSome((a) => checkExpr(a, reg), args)).with({ _tag: "ETuple" }, ({ elements }) => firstSome((el) => checkExpr(el, reg), elements)).with({ _tag: "EArr" }, ({ elements }) => firstSome((el) => checkExpr(match9(el).with({ _tag: "SEExpr" }, ({ expr: e2 }) => e2).with({ _tag: "SESpread" }, ({ expr: e2 }) => e2).exhaustive(), reg), elements)).with({ _tag: "EList" }, ({ elements }) => firstSome((el) => checkExpr(match9(el).with({ _tag: "SEExpr" }, ({ expr: e2 }) => e2).with({ _tag: "SESpread" }, ({ expr: e2 }) => e2).exhaustive(), reg), elements)).with({ _tag: "ESet" }, ({ elements }) => firstSome((el) => checkExpr(match9(el).with({ _tag: "SEExpr" }, ({ expr: e2 }) => e2).with({ _tag: "SESpread" }, ({ expr: e2 }) => e2).exhaustive(), reg), elements)).with({ _tag: "EMap" }, ({ entries }) => firstSome((en) => _Option_orElse(checkExpr(en.value, reg), checkExpr(en.key, reg)), entries)).with({ _tag: "EInterp" }, ({ parts }) => firstSome((p) => match9(p).with({ _tag: "IPLit" }, () => None9).with({ _tag: "IPExpr" }, ({ expr: ex }) => checkExpr(ex, reg)).exhaustive(), parts)).exhaustive());
var reservedNames = ["Array", "List", "Set", "Map", "Option", "Result", "Str"];
var redeclarableTypes = ["Option", "Result"];
var reservedErr = _curry10(2, (name, sp) => checkErr(`'${name}' is a reserved collection namespace and cannot be bound`, sp));
var checkReservedNames = (stmts) => firstSome((s) => match9(s).with({ _tag: "SType" }, ({ name, span: sp }) => _Array_contains2(name, redeclarableTypes) ? None9 : _Array_contains2(name, reservedNames) ? Some9(reservedErr(name, sp)) : None9).with({ _tag: "SLet" }, ({ name, span: sp }) => _Array_contains2(name, reservedNames) ? Some9(reservedErr(name, sp)) : None9).with({ _tag: "SExtern" }, ({ name, span: sp }) => _Array_contains2(name, reservedNames) ? Some9(reservedErr(name, sp)) : None9).with({ _tag: "SImport" }, ({ names }) => firstSome((n) => _Array_contains2(n.name, reservedNames) ? Some9(checkErr(`'${n.name}' is a reserved collection namespace and cannot be imported`, n.span)) : None9, names)).with({ _tag: "SImportNs" }, ({ alias }) => _Array_contains2(alias.name, reservedNames) ? Some9(checkErr(`'${alias.name}' is a reserved collection namespace and cannot be imported`, alias.span)) : None9).with({ _tag: "SError" }, () => None9).with({ _tag: "SExpr" }, () => None9).exhaustive(), stmts);
var isUpperStart = (s) => match9(_Str_codeAt4(0, s)).with({ _tag: "Some" }, ({ value: c }) => and6(c >= 65, c <= 90)).with({ _tag: "None" }, () => false).exhaustive();
var strayTypeVar = _curry10(2, (params, te) => match9(te).with({ _tag: "TyName" }, ({ name, span: sp }) => or5(isUpperStart(name), or5(_Array_contains2(name, primTypeNames), _Array_contains2(name, params))) ? None9 : Some9(_tuple4(name, sp))).with({ _tag: "TyArrow" }, ({ from, to }) => _Option_orElse(strayTypeVar(params, to), strayTypeVar(params, from))).with({ _tag: "TyApp" }, ({ args }) => firstSome(strayTypeVar(params), args)).with({ _tag: "TyTuple" }, ({ elems }) => firstSome(strayTypeVar(params), elems)).with({ _tag: "TyList" }, ({ elem }) => strayTypeVar(params, elem)).with({ _tag: "TyQual" }, ({ args }) => firstSome(strayTypeVar(params), args)).with({ _tag: "TyLit" }, () => None9).with({ _tag: "TyUnion" }, ({ members }) => firstSome(strayTypeVar(params), members)).exhaustive());
var checkCtorFieldVars = (stmts) => firstSome((s) => match9(s).with({ _tag: "SType" }, ({ name, params, ctors }) => firstSome((c) => firstSome((f) => match9(strayTypeVar(params, f.fieldType)).with((_v) => {
  const _g = _v;
  return _g._tag === "Some";
}, ({ value: [vn, vsp] }) => Some9(checkErr(`unknown type parameter '${vn}' in constructor '${c.name}' \u2014 declare it: type ${name} ${_Str_join4(" ", _Array_append7(vn, params))} = ...`, vsp))).with({ _tag: "None" }, () => None9).exhaustive(), c.fields), ctors)).otherwise(() => None9), stmts);
var qualRefsFrom = (te) => match9(te).with({ _tag: "TyName" }, () => []).with({ _tag: "TyArrow" }, ({ from, to }) => [...qualRefsFrom(from), ...qualRefsFrom(to)]).with({ _tag: "TyApp" }, ({ args }) => _Array_flatMap2(qualRefsFrom, args)).with({ _tag: "TyTuple" }, ({ elems }) => _Array_flatMap2(qualRefsFrom, elems)).with({ _tag: "TyList" }, ({ elem }) => qualRefsFrom(elem)).with({ _tag: "TyQual" }, ({ alias, name, nameSpan, args, span: sp }) => [{ alias, name, nameSpan, qualSpan: sp }, ..._Array_flatMap2(qualRefsFrom, args)]).with({ _tag: "TyLit" }, () => []).with({ _tag: "TyUnion" }, ({ members }) => _Array_flatMap2(qualRefsFrom, members)).exhaustive();
var letInAnnots = (e) => match9(e).with({ _tag: "ENum" }, () => []).with({ _tag: "EUnit" }, () => []).with({ _tag: "EBool" }, () => []).with({ _tag: "EStr" }, () => []).with({ _tag: "ERef" }, () => []).with({ _tag: "ECall" }, ({ fn, args }) => [...letInAnnots(fn), ..._Array_flatMap2(letInAnnots, args)]).with({ _tag: "ELambda" }, ({ params, body }) => [...letInAnnots(body), ..._Array_flatMap2((p) => match9(p).with((_v) => {
  const _g = _v;
...
}, ({ defaultValue: { value: d } }) => letInAnnots(d)).otherwise(() => []), params)]).with({ _tag: "ELetIn" }, ({ annot, value, body }) => [...letInAnnots(value), ...letInAnnots(body), ...match9(annot).with({ _tag: "Some" }, ({ value: te }) => [te]).with({ _tag: "None" }, () => []).exhaustive()]).with({ _tag: "ELetBind" }, ({ value, body }) => [...letInAnnots(value), ...letInAnnots(body)]).with({ _tag: "EPipe" }, ({ left, right }) => [...letInAnnots(left), ...letInAnnots(right)]).with({ _tag: "EDo" }, ({ exprs }) => _Array_flatMap2(letInAnnots, exprs)).with({ _tag: "ETernary" }, ({ cond, thenE, elseE }) => [...letInAnnots(cond), ...letInAnnots(thenE), ...letInAnnots(elseE)]).with({ _tag: "EMatch" }, ({ scrutinee, arms }) => [...letInAnnots(scrutinee), ..._Array_flatMap2((a) => [...match9(a.guard).with({ _tag: "Some" }, ({ value: g }) => letInAnnots(g)).with({ _tag: "None" }, () => []).exhaustive(), ...letInAnnots(a.body)], arms)]).with({ _tag: "ERecord" }, ({ fields, spread }) => [...match9(spread).with({ _tag: "Some" }, ({ value: sp }) => letInAnnots(sp)).with({ _tag: "None" }, () => []).exhaustive(), ..._Array_flatMap2((f) => letInAnnots(f.value), fields)]).with({ _tag: "EField" }, ({ target }) => letInAnnots(target)).with({ _tag: "ELoop" }, ({ params, body }) => [..._Array_flatMap2((prm) => letInAnnots(prm.init), params), ...letInAnnots(body)]).with({ _tag: "ERecur" }, ({ args }) => _Array_flatMap2(letInAnnots, args)).with({ _tag: "ETuple" }, ({ elements }) => _Array_flatMap2(letInAnnots, elements)).with({ _tag: "EArr" }, ({ elements }) => _Array_flatMap2(seqElemAnnots, elements)).with({ _tag: "EList" }, ({ elements }) => _Array_flatMap2(seqElemAnnots, elements)).with({ _tag: "ESet" }, ({ elements }) => _Array_flatMap2(seqElemAnnots, elements)).with({ _tag: "EMap" }, ({ entries }) => _Array_flatMap2((en) => [...letInAnnots(en.key), ...letInAnnots(en.value)], entries)).with({ _tag: "EInterp" }, ({ parts }) => _Array_flatMap2((prt) => match9(prt).with({ _tag: "IPLit" }, () => []).with({ _tag: "IPExpr" }, ({ expr: ex }) => letInAnnots(ex)).exhaustive(), parts)).exhaustive();
var seqElemAnnots = (el) => match9(el).with({ _tag: "SEExpr" }, ({ expr: e }) => letInAnnots(e)).with({ _tag: "SESpread" }, ({ expr: e }) => letInAnnots(e)).exhaustive();
var writtenTypeExprs = (stmts) => _Array_flatMap2((s) => match9(s).with({ _tag: "SExtern" }, ({ typeExpr: te }) => [te]).with({ _tag: "SLet" }, ({ annot, value }) => [...match9(annot).with({ _tag: "Some" }, ({ value: te }) => [te]).with({ _tag: "None" }, () => []).exhaustive(), ...letInAnnots(value)]).with({ _tag: "SExpr" }, ({ value }) => letInAnnots(value)).with({ _tag: "SType" }, ({ ctors, alias, aliasType }) => [..._Array_flatMap2((c) => map6((f) => f.fieldType, c.fields), ctors), ...match9(alias).with({ _tag: "Some" }, ({ value: fields }) => map6((f) => f.fieldType, fields)).with({ _tag: "None" }, () => []).exhaustive(), ...match9(aliasType).with({ _tag: "Some" }, ({ value: te }) => [te]).with({ _tag: "None" }, () => []).exhaustive()]).otherwise(() => []), stmts);
var emptyQuals = new Map;
var checkQualifiedTypeNames = _curry10(2, (stmts, quals) => {
  const nsAliases = _Set_fromArray(_Array_flatMap2((s) => match9(s).with({ _tag: "SImportNs" }, ({ alias }) => [alias.name]).otherwise(() => []), stmts));
  return firstSome((q) => _Set_has(q.alias, nsAliases) ? match9(_Map_get4(q.alias, quals)).with({ _tag: "None" }, () => None9).with({ _tag: "Some" }, ({ value: dep }) => _Set_has(q.name, dep.types) ? None9 : Some9(checkErr(`module alias '${q.alias}' has no exported type '${q.name}' \u2014 export it from the imported module ('export type ${q.name} = \u2026')`, q.nameSpan))).exhaustive() : Some9(checkErr(`unknown module alias '${q.alias}' in type '${q.alias}.${q.name}' \u2014 a qualified type name needs a matching 'import * as ${q.alias} from "\u2026"'`, q.qualSpan)), _Array_flatMap2(qualRefsFrom, writtenTypeExprs(stmts)));
});
var mergeMissing = _curry10(3, (keys, from, into) => match9(keys).with((_v) => _v.length === 0, () => into).with((_v) => _v.length >= 1, ([k, ...rest]) => match9(_Map_get4(k, from)).with({ _tag: "Some" }, ({ value: v }) => mergeMissing(rest, from, _Map_has2(k, into) ? into : _Map_set3(k, v, into))).with({ _tag: "None" }, () => mergeMissing(rest, from, into)).exhaustive()).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var checkWith = _curry10(3, (stmts, imported, quals) => match9(checkReservedNames(stmts)).with({ _tag: "Some" }, ({ value: e }) => Err7(e)).with({ _tag: "None" }, () => match9(checkCtorFieldVars(stmts)).with({ _tag: "Some" }, ({ value: e }) => Err7(e)).with({ _tag: "None" }, () => match9(checkQualifiedTypeNames(stmts, quals)).with({ _tag: "Some" }, ({ value: e }) => Err7(e)).with({ _tag: "None" }, () => _Result_flatMap5((reg0) => ((reg) => match9(firstSome((s) => match9(s).with({ _tag: "SLet" }, ({ value }) => checkExpr(value, reg)).with({ _tag: "SExpr" }, ({ value }) => checkExpr(value, reg)).otherwise(() => None9), stmts)).with({ _tag: "Some" }, ({ value: e }) => Err7(e)).with({ _tag: "None" }, () => Ok7(stmts)).exhaustive())({ ctors: mergeMissing(_Map_keys3(imported.ctors), imported.ctors, reg0.ctors), types: mergeMissing(_Map_keys3(imported.types), imported.types, reg0.types) }), buildRegistry(stmts))).exhaustive()).exhaustive()).exhaustive());
var check = (stmts) => checkWith(stmts, { ctors: new Map, types: new Map }, emptyQuals);

import { Err as Err8, None as None12, Ok as Ok8, Some as Some12, _Array_append as _Array_append9, _Array_concat as _Array_concat4, _Array_flatMap as _Array_flatMap3, _Array_get as _Array_get9, _Array_head as _Array_head4, _Array_prepend as _Array_prepend6, _Array_reverse, _Map_delete, _Map_get as _Map_get6, _Map_getOr as _Map_getOr5, _Map_has as _Map_has4, _Map_keys as _Map_keys4, _Map_set as _Map_set6, _Option_map, _Result_flatMap as _Result_flatMap6, _Result_map as _Result_map5, _Set_add as _Set_add4, _Set_fromArray as _Set_fromArray4, _Set_has as _Set_has4, _Set_size, _Set_toArray as _Set_toArray2, _Str_startsWith as _Str_startsWith2, _curry as _curry13, _done as _done7, _recur as _recur7, _tuple as _tuple6, and as and8, eq as eq12, map as map8, not as not7, or as or6, reduce as reduce2 } from "@mochi/compiler/runtime";
import { match as match12 } from "@onrails/pattern";

import { _Array_contains as _Array_contains3, _Array_prepend as _Array_prepend5, _Map_get as _Map_get5, _Map_getOr as _Map_getOr3, _Map_set as _Map_set4, _Map_values, _Set_add as _Set_add2, _Set_diff, _Set_fromArray as _Set_fromArray2, _Set_has as _Set_has2, _Set_toArray, _Str_codeAt as _Str_codeAt5, _curry as _curry11, _tuple as _tuple5, and as and7, map as map7 } from "@mochi/compiler/runtime";
...
var primType = (name) => match10(name).with("float", () => tNumber).with("int", () => tNumber).with("string", () => tString).with("bool", () => tBool).otherwise(() => tPrim(name));
var emptyVarSets = { tv: _Set_fromArray2([]), rv: _Set_fromArray2([]) };
var diffVarSets = _curry11(2, (a, b) => ({ tv: _Set_diff(a.tv, b.tv), rv: _Set_diff(a.rv, b.rv) }));
var collect = _curry11(2, (t, acc) => match10(t).with({ _tag: "TyVar" }, ({ id }) => ({ tv: _Set_add2(id, acc.tv), rv: acc.rv })).with({ _tag: "TyCon" }, ({ args }) => collectArgs(args, acc)).with({ _tag: "TyFn" }, ({ from: fromT, to: toT }) => collect(toT, collect(fromT, acc))).with({ _tag: "TyRecord" }, ({ row }) => collectRow(row, acc)).with({ _tag: "TySingleton" }, () => acc).with({ _tag: "TyOneOf" }, ({ members }) => collectArgs(members, acc)).exhaustive());
var collectArgs = _curry11(2, (args, acc) => match10(args).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => acc).with((_v) => {
  const _g = _v;
...
var collectRow = _curry11(2, (row, acc) => match10(row).with({ _tag: "RowVar" }, ({ id }) => ({ tv: acc.tv, rv: _Set_add2(id, acc.rv) })).with({ _tag: "RowExtend" }, ({ fieldType, rest }) => collectRow(rest, collect(fieldType, acc))).with({ _tag: "RowEmpty" }, () => acc).exhaustive());
var freeInType = (t) => collect(t, emptyVarSets);
var collectFree = _curry11(4, (t, bound, st, acc) => match10(t).with({ _tag: "TyVar" }, ({ id }) => _Set_has2(id, bound.tv) ? acc : match10(_Map_get5(id, st.tv)).with({ _tag: "Some" }, ({ value: next }) => collectFree(next, bound, st, acc)).with({ _tag: "None" }, () => ({ tv: _Set_add2(id, acc.tv), rv: acc.rv })).exhaustive()).with({ _tag: "TyCon" }, ({ args }) => collectFreeArgs(args, bound, st, acc)).with({ _tag: "TyFn" }, ({ from: fromT, to: toT }) => collectFree(toT, bound, st, collectFree(fromT, bound, st, acc))).with({ _tag: "TyRecord" }, ({ row }) => collectFreeRow(row, bound, st, acc)).with({ _tag: "TySingleton" }, () => acc).with({ _tag: "TyOneOf" }, ({ members }) => collectFreeArgs(members, bound, st, acc)).exhaustive());
var collectFreeArgs = _curry11(4, (args, bound, st, acc) => match10(args).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => acc).with((_v) => {
  const _g = _v;
...
var collectFreeRow = _curry11(4, (row, bound, st, acc) => match10(row).with({ _tag: "RowVar" }, ({ id }) => _Set_has2(id, bound.rv) ? acc : match10(_Map_get5(id, st.rv)).with({ _tag: "Some" }, ({ value: next }) => collectFreeRow(next, bound, st, acc)).with({ _tag: "None" }, () => ({ tv: acc.tv, rv: _Set_add2(id, acc.rv) })).exhaustive()).with({ _tag: "RowExtend" }, ({ fieldType, rest }) => collectFreeRow(rest, bound, st, collectFree(fieldType, bound, st, acc))).with({ _tag: "RowEmpty" }, () => acc).exhaustive());
var freeInScheme = _curry11(3, (sc, st, acc) => collectFree(sc.ty, { tv: _Set_fromArray2(sc.vars), rv: _Set_fromArray2(sc.rvars) }, st, acc));
var freeInEnvFrom = _curry11(3, (schemes, st, acc) => match10(schemes).with((_v) => _v.length === 0, () => acc).with((_v) => _v.length >= 1, ([sc, ...rest]) => freeInEnvFrom(rest, st, freeInScheme(sc, st, acc))).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var freeInEnv = _curry11(2, (env, st) => freeInEnvFrom(_Map_values(env), st, emptyVarSets));
var generalize = _curry11(4, (env, t, st, widen) => {
...
var widenLits = (t) => match10(t).with({ _tag: "TySingleton", base: "string" }, () => tString).with({ _tag: "TySingleton" }, () => tNumber).with({ _tag: "TyOneOf" }, ({ members }) => tUnion(map7((m) => match10(m).with({ _tag: "TySingleton" }, () => m).otherwise(() => widenLits(m)), members))).with({ _tag: "TyCon" }, ({ name, args }) => tCon(name, map7(widenLits, args))).with({ _tag: "TyFn" }, ({ from: fromT, to: toT }) => tArrow(widenLits(fromT), widenLits(toT))).with({ _tag: "TyRecord" }, ({ row }) => tRecord(widenRow(row))).with({ _tag: "TyVar" }, () => t).exhaustive();
var widenRow = (row) => match10(row).with({ _tag: "RowEmpty" }, () => row).with({ _tag: "RowVar" }, () => row).with({ _tag: "RowExtend" }, ({ label, fieldType, optional, rest }) => rField(label, widenLits(fieldType), widenRow(rest), optional)).exhaustive();
var instMapFrom = _curry11(3, (vars, acc, st) => match10(vars).with((_v) => _v.length === 0, () => _tuple5(acc, st)).with((_v) => _v.length >= 1, ([v, ...rest]) => (([fv, st1]) => instMapFrom(rest, _Map_set4(v, fv, acc), st1))(freshVar(st))).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var instRowMapFrom = _curry11(3, (vars, acc, st) => match10(vars).with((_v) => _v.length === 0, () => _tuple5(acc, st)).with((_v) => _v.length >= 1, ([v, ...rest]) => (([fr, st1]) => instRowMapFrom(rest, _Map_set4(v, fr, acc), st1))(freshRowVar(st))).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var instSub = _curry11(3, (t, tmap, rmap) => match10(t).with({ _tag: "TyVar" }, ({ id }) => _Map_getOr3(t, id, tmap)).with({ _tag: "TyCon" }, ({ name, args }) => tCon(name, map7((a) => instSub(a, tmap, rmap), args))).with({ _tag: "TyFn" }, ({ from: fromT, to: toT }) => tArrow(instSub(fromT, tmap, rmap), instSub(toT, tmap, rmap))).with({ _tag: "TyRecord" }, ({ row }) => tRecord(instSubRow(row, tmap, rmap))).with({ _tag: "TySingleton" }, ({ base, value }) => TySingleton(base, value)).with({ _tag: "TyOneOf" }, ({ members }) => tUnion(map7((m) => instSub(m, tmap, rmap), members))).exhaustive());
var instSubRow = _curry11(3, (row, tmap, rmap) => match10(row).with({ _tag: "RowVar" }, ({ id }) => _Map_getOr3(row, id, rmap)).with({ _tag: "RowExtend" }, ({ label, fieldType, optional, rest }) => rField(label, instSub(fieldType, tmap, rmap), instSubRow(rest, tmap, rmap), optional)).with({ _tag: "RowEmpty" }, () => row).exhaustive());
var instantiate = _curry11(2, (sc, st) => (([tmap, st1]) => (([rmap, st2]) => _tuple5(instSub(sc.ty, tmap, rmap), st2))(instRowMapFrom(sc.rvars, new Map, st1)))(instMapFrom(sc.vars, new Map, st)));
var isUpperStart2 = (s) => match10(_Str_codeAt5(0, s)).with({ _tag: "Some" }, ({ value: c }) => and7(c >= 65, c <= 90)).with({ _tag: "None" }, () => false).exhaustive();
var typeExprListToType = _curry11(5, (tes, vars, st, aliases, expanding) => match10(tes).with((_v) => {
  const _g = _v;
  return _g.length === 0;
...
var aliasFieldsFrom = _curry11(5, (fields, vars, st, aliases, expanding) => match10(fields).with((_v) => {
  const _g = _v;
  return _g.length === 0;
...
var aliasRow = _curry11(6, (name, info, args, st, aliases, expanding) => _Set_has2(name, expanding) ? _tuple5(tCon(name, args), st) : match10(info.expr).with({ _tag: "Some" }, ({ value: te }) => (([local, st1]) => (([t, _, st2]) => _tuple5(t, st2))(typeExprToType(te, local, st1, aliases, _Set_add2(name, expanding))))(aliasLocalVarsFrom(info.params, args, st))).with({ _tag: "None" }, () => (([local, st1]) => {
...
var pvarsFrom = _curry11(2, (params, st) => match10(params).with((_v) => _v.length === 0, () => _tuple5(new Map, [], st)).with((_v) => _v.length >= 1, ([p, ...rest]) => (([v, st1]) => (([restMap, restVars, st2]) => _tuple5(_Map_set4(p, v, restMap), _Array_prepend5(v, restVars), st2))(pvarsFrom(rest, st1)))(freshVar(st))).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var ctorFieldsArrowFrom = _curry11(5, (fields, pvars, st, aliases, result) => match10(fields).with((_v) => {
  const _g = _v;
  return _g.length === 0;
...
var ctorScheme = _curry11(5, (typeName, params, c, st, aliases) => (([pvars, pvarTypes, st1]) => {
...
import { _Array_append as _Array_append8, _Array_drop as _Array_drop2, _Array_get as _Array_get8, _Array_take as _Array_take2, _Map_getOr as _Map_getOr4, _Map_has as _Map_has3, _Map_set as _Map_set5, _Set_add as _Set_add3, _Set_diff as _Set_diff2, _Set_fromArray as _Set_fromArray3, _Set_has as _Set_has3, _curry as _curry12, _done as _done6, _recur as _recur6, eq as eq11, length as length9, min } from "@mochi/compiler/runtime";
import { match as match11 } from "@onrails/pattern";
var hasIndex = _curry12(2, (v, st) => _Map_has3(v, st.index));
var indexOfV = _curry12(2, (v, st) => _Map_getOr4(-1, v, st.index));
var lowOfV = _curry12(2, (v, st) => _Map_getOr4(-1, v, st.low));
var neighborsOf = _curry12(2, (v, adj) => match11(_Array_get8(v, adj)).with({ _tag: "Some" }, ({ value: ws }) => ws).with({ _tag: "None" }, () => []).exhaustive());
var indexOfFrom = _curry12(3, (v, xs, i) => {
  let j = i;
  while (true) {
    const _step = match11(_Array_get8(j, xs)).with({ _tag: "None" }, () => _done6(-1)).with({ _tag: "Some" }, ({ value: x }) => eq11(x, v) ? _done6(j) : _recur6(j + 1)).exhaustive();
    if (_step._tag === "recur") {
      j = _step.args[0];
      continue;
    }
...
var visitNeighbors = _curry12(4, (v, ws, adj, st) => {
  let remaining = ws;
  let current = st;
  while (true) {
    const _step = match11(remaining).with((_v) => {
      const _g = _v;
      return _g.length === 0;
...
var connect = _curry12(3, (v, adj, st) => {
  ...
  return eq11(lowOfV(v, st2), indexOfV(v, st2)) ? ((start) => ((comp) => ({ ...st2, onStack: _Set_diff2(st2.onStack, _Set_fromArray3(comp)), stack: _Array_take2(start, st2.stack), sccs: _Array_append8(comp, st2.sccs) }))(_Array_drop2(start, st2.stack)))(indexOfFrom(v, st2.stack, 0)) : st2;
});
var connectAllFrom = _curry12(4, (i, n, adj, st) => {
  let j = i;
  let current = st;
  while (true) {
    if (j >= n) {
...
var setLetBindMonad = _curry13(2, ($receiver, $value) => $receiver["monad"] = $value);
var exprSpan3 = (e) => match12(e).with({ _tag: "ENum" }, ({ span: sp }) => sp).with({ _tag: "EUnit" }, ({ span: sp }) => sp).with({ _tag: "EBool" }, ({ span: sp }) => sp).with({ _tag: "EStr" }, ({ span: sp }) => sp).with({ _tag: "ERef" }, ({ span: sp }) => sp).with({ _tag: "ECall" }, ({ span: sp }) => sp).with({ _tag: "ELambda" }, ({ span: sp }) => sp).with({ _tag: "ELetIn" }, ({ span: sp }) => sp).with({ _tag: "ELetBind" }, ({ span: sp }) => sp).with({ _tag: "EPipe" }, ({ span: sp }) => sp).with({ _tag: "EDo" }, ({ span: sp }) => sp).with({ _tag: "ETernary" }, ({ span: sp }) => sp).with({ _tag: "EMatch" }, ({ span: sp }) => sp).with({ _tag: "ELoop" }, ({ span: sp }) => sp).with({ _tag: "ERecur" }, ({ span: sp }) => sp).with({ _tag: "ERecord" }, ({ span: sp }) => sp).with({ _tag: "EField" }, ({ span: sp }) => sp).with({ _tag: "ETuple" }, ({ span: sp }) => sp).with({ _tag: "EArr" }, ({ span: sp }) => sp).with({ _tag: "EList" }, ({ span: sp }) => sp).with({ _tag: "ESet" }, ({ span: sp }) => sp).with({ _tag: "EMap" }, ({ span: sp }) => sp).with({ _tag: "EInterp" }, ({ span: sp }) => sp).exhaustive();
var patSpan3 = (p) => match12(p).with({ _tag: "PWild" }, ({ span: sp }) => sp).with({ _tag: "PUnit" }, ({ span: sp }) => sp).with({ _tag: "PBind" }, ({ span: sp }) => sp).with({ _tag: "PAs" }, ({ span: sp }) => sp).with({ _tag: "PLit" }, ({ span: sp }) => sp).with({ _tag: "PBool" }, ({ span: sp }) => sp).with({ _tag: "PStr" }, ({ span: sp }) => sp).with({ _tag: "PTuple" }, ({ span: sp }) => sp).with({ _tag: "PRecord" }, ({ span: sp }) => sp).with({ _tag: "PCtor" }, ({ span: sp }) => sp).with({ _tag: "PArr" }, ({ span: sp }) => sp).with({ _tag: "PList" }, ({ span: sp }) => sp).with({ _tag: "POr" }, ({ span: sp }) => sp).exhaustive();
var annotSpan = (t) => match12(t).with({ _tag: "TyName" }, ({ span: sp }) => sp).with({ _tag: "TyArrow" }, ({ span: sp }) => sp).with({ _tag: "TyApp" }, ({ span: sp }) => sp).with({ _tag: "TyTuple" }, ({ span: sp }) => sp).with({ _tag: "TyList" }, ({ span: sp }) => sp).with({ _tag: "TyQual" }, ({ span: sp }) => sp).with({ _tag: "TyLit" }, ({ span: sp }) => sp).with({ _tag: "TyUnion" }, ({ span: sp }) => sp).exhaustive();
var typeErr = _curry13(2, (msg, sp) => ({ message: msg, start: sp.start, end: sp.end }));
var u = _curry13(4, (a, b, st, sp) => match12(unify(a, b, st)).with({ _tag: "Ok" }, ({ value: newSt }) => Ok8(newSt)).with({ _tag: "Err" }, ({ error: e }) => Err8(typeErr(e.message, sp))).exhaustive());
var checkFits = _curry13(4, (actual, expected, st, sp) => match12(fits(actual, expected, st)).with({ _tag: "Ok" }, ({ value: newSt }) => Ok8(newSt)).with({ _tag: "Err" }, ({ error: e }) => Err8(typeErr(e.message, sp))).exhaustive());
var bindParamNamesFrom = _curry13(3, (names, env, st) => match12(names).with((_v) => _v.length === 0, () => _tuple6([], env, st)).with((_v) => _v.length >= 1, ([n, ...rest]) => (([t, st1]) => (([restTs, env2, st2]) => _tuple6(_Array_prepend6(t, restTs), env2, st2))(bindParamNamesFrom(rest, _Map_set6(n, mono(t), env), st1)))(freshVar(st))).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var bindParamFieldsFrom = _curry13(4, (fields, env, row, st) => match12(fields).with((_v) => {
  const _g = _v;
  return _g.length === 0;
...
var constrainParamAnnotsFrom = _curry13(5, (ctx, params, paramTypes, vars, st) => match12(params).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => Ok8(_tuple6(vars, st))).with((_v) => {
  const _g = _v;
  return _g.length >= 1;
}, ([param, ...rest]) => match12(paramTypes).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => Ok8(_tuple6(vars, st))).with((_v) => {
  const _g = _v;
  return _g.length >= 1;
}, ([paramT, ...restTypes]) => match12(param).with((_v) => {
  const _g = _v;
...
var arrowChain = _curry13(2, (paramTypes, resultT) => match12(paramTypes).with((_v) => {
  const _g = _v;
  return _g.length === 0;
...
var ctxWithEnv = _curry13(2, (ctx, env) => ({ env, open: ctx.open, ns: ctx.ns, aliasMap: ctx.aliasMap, plugins: ctx.plugins, loopStack: ctx.loopStack, letOwner: ctx.letOwner }));
var ctxWithLets = _curry13(3, (ctx, env, letOwner) => ({ env, open: ctx.open, ns: ctx.ns, aliasMap: ctx.aliasMap, plugins: ctx.plugins, loopStack: ctx.loopStack, letOwner }));
var ctxWithLoop = _curry13(4, (ctx, env, frame, letOwner) => ({ env, open: ctx.open, ns: ctx.ns, aliasMap: ctx.aliasMap, plugins: ctx.plugins, loopStack: _Array_prepend6(frame, ctx.loopStack), letOwner }));
var inferLoopParamsFrom = _curry13(7, (ctx, params, i, envAcc, frameAcc, ownerAcc, st) => match12(_Array_get9(i, params)).with({ _tag: "None" }, () => Ok8(_tuple6(frameAcc, envAcc, ownerAcc, st))).with({ _tag: "Some" }, ({ value: p }) => _Result_flatMap6(([t, st1]) => ((sp) => inferLoopParamsFrom(ctx, params, i + 1, _Map_set6(p.name, mono(t), envAcc), _Array_append9(t, frameAcc), _Map_set6(p.name, sp, ownerAcc), noteLet(sp, st1)))(exprSpan3(p.init)), inferExpr(ctx, p.init, st))).exhaustive());
var unifyRecurArgsFrom = _curry13(5, (ctx, args, frame, i, st) => match12(_Array_get9(i, args)).with({ _tag: "None" }, () => Ok8(st)).with({ _tag: "Some" }, ({ value: a }) => _Result_flatMap6(([at, st1]) => match12(_Array_get9(i, frame)).with({ _tag: "None" }, () => unifyRecurArgsFrom(ctx, args, frame, i + 1, st1)).with({ _tag: "Some" }, ({ value: pt }) => _Result_flatMap6((st2) => unifyRecurArgsFrom(ctx, args, frame, i + 1, st2), u(at, pt, st1, exprSpan3(a)))).exhaustive(), inferExpr(ctx, a, st))).exhaustive());
var inferRecur = _curry13(4, (ctx, args, sp, st) => match12(ctx.loopStack).with((_v) => {
  const _g = _v;
  return _g.length === 0;
...
var rowHasOptional = (row) => match12(row).with({ _tag: "RowExtend" }, ({ optional, rest }) => or6(optional, rowHasOptional(rest))).otherwise(() => false);
var domainNeedsFits = _curry13(2, (t, st) => match12(zonk(t, st)).with({ _tag: "TyRecord" }, ({ row }) => rowHasOptional(row)).otherwise(() => false));
var rowAllOptional = (row) => match12(row).with({ _tag: "RowExtend" }, ({ optional, rest }) => and8(optional, rowAllOptional(rest))).otherwise(() => true);
var domainIsOmittableRecord = _curry13(2, (t, st) => match12(zonk(t, st)).with({ _tag: "TyRecord" }, ({ row }) => rowAllOptional(row)).otherwise(() => false));
var isLabeledParam2 = (p) => match12(p).with({ _tag: "LPLabeled" }, () => true).with({ _tag: "LPSpanned" }, ({ param: inner }) => isLabeledParam2(inner)).otherwise(() => false);
var splitLamParams = _curry13(3, (params, positional, labeled) => match12(params).with((_v) => {
  const _g = _v;
  return _g.length === 0;
...
var labFieldsFrom = _curry13(5, (ctx, labs, env, vars, st) => match12(labs).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => Ok8(_tuple6([], st))).with((_v) => {
  const _g = _v;
  return _g.length >= 1;
}, ([lab, ...rest]) => match12(lab).with({ _tag: "LPSpanned" }, ({ param: inner }) => labFieldsFrom(ctx, [inner, ...rest], env, vars, st)).with({ _tag: "LPLabeled" }, ({ name, annot, optional, defaultValue }) => (([fieldT, vars1, st1]) => _Result_flatMap6(([fieldT1, st2]) => ((bodyT) => ((omittable) => _Result_flatMap6(([fields, stN]) => Ok8(_tuple6(_Array_prepend6({ name, fieldType: fieldT1, omittable, bodyType: bodyT }, fields), stN)), labFieldsFrom(ctx, rest, env, vars1, st2)))(or6(optional, match12(defaultValue).with({ _tag: "Some" }, () => true).with({ _tag: "None" }, () => false).exhaustive())))(match12(defaultValue).with({ _tag: "Some" }, () => fieldT1).with({ _tag: "None" }, () => optional ? tCon("Option", [fieldT1]) : fieldT1).exhaustive()), match12(defaultValue).with({ _tag: "None" }, () => Ok8(_tuple6(fieldT, st1))).with({ _tag: "Some" }, ({ value: d }) => _Result_flatMap6(([dt, s2]) => match12(annot).with({ _tag: "Some" }, () => _Result_flatMap6((s3) => Ok8(_tuple6(fieldT, s3)), checkFits(dt, fieldT, s2, exprSpan3(d)))).with({ _tag: "None" }, () => ((widened) => _Result_flatMap6((s3) => Ok8(_tuple6(widened, s3)), u(fieldT, widened, s2, exprSpan3(d))))(widenLits(zonk(dt, s2)))).exhaustive(), inferExpr(ctxWithEnv(ctx, env), d, st1))).exhaustive()))(match12(annot).with({ _tag: "Some" }, ({ value: te }) => typeExprToType(te, vars, st, ctx.aliasMap, _Set_fromArray4([]))).with({ _tag: "None" }, () => (([t, s1]) => _tuple6(t, vars, s1))(freshVar(st))).exhaustive())).otherwise(() => labFieldsFrom(ctx, rest, env, vars, st))).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var rowOfLabFields = (fields) => match12(fields).with((_v) => _v.length === 0, () => RowEmpty).with((_v) => _v.length >= 1, ([f, ...rest]) => rField(f.name, f.fieldType, rowOfLabFields(rest), f.omittable)).otherwise(() => {
  throw new Error("non-exhaustive match");
});
var envWithLabFields = _curry13(2, (fields, env) => match12(fields).with((_v) => _v.length === 0, () => env).with((_v) => _v.length >= 1, ([f, ...rest]) => envWithLabFields(rest, _Map_set6(f.name, mono(f.bodyType), env))).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var inferCallArgs = _curry13(5, (ctx, fnT, args, st, callSpan) => match12(args).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => Ok8(_tuple6(fnT, st))).with((_v) => {
  const _g = _v;
  return _g.length >= 1;
}, ([arg, ...rest]) => _Result_flatMap6(([argT, st1]) => match12(resolve(fnT, st1)).with({ _tag: "TyFn" }, ({ from: fromT, to: toT }) => domainNeedsFits(fromT, st1) ? _Result_flatMap6((st2) => inferCallArgs(ctx, toT, rest, st2, callSpan), checkFits(argT, fromT, st1, exprSpan3(arg))) : (([resultT, st2]) => _Result_flatMap6((st3) => inferCallArgs(ctx, resultT, rest, st3, callSpan), u(fnT, tArrow(argT, resultT), st2, exprSpan3(arg))))(freshVar(st1))).otherwise(() => (([resultT, st2]) => _Result_flatMap6((st3) => inferCallArgs(ctx, resultT, rest, st3, callSpan), u(fnT, tArrow(argT, resultT), st2, exprSpan3(arg))))(freshVar(st1))), inferExpr(ctx, arg, st))).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var inferNormalCall = _curry13(4, (ctx, fn, args, st) => _Result_flatMap6(([fnT, st1]) => match12(args).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => match12(resolve(fnT, st1)).with({ _tag: "TyFn" }, ({ from: fromT, to: toT }) => domainIsOmittableRecord(fromT, st1) ? _Result_flatMap6((st2) => Ok8(_tuple6(toT, st2)), checkFits(tRecord(RowEmpty), fromT, st1, exprSpan3(fn))) : (([resultT, st2]) => _Result_flatMap6((st3) => Ok8(_tuple6(resultT, st3)), u(fnT, tArrow(tUnit, resultT), st2, exprSpan3(fn))))(freshVar(st1))).otherwise(() => (([resultT, st2]) => _Result_flatMap6((st3) => Ok8(_tuple6(resultT, st3)), u(fnT, tArrow(tUnit, resultT), st2, exprSpan3(fn))))(freshVar(st1)))).otherwise(() => inferCallArgs(ctx, fnT, args, st1, exprSpan3(fn))), inferExpr(ctx, fn, st)));
var inferTernary = _curry13(5, (ctx, cond, thenE, elseE, st) => _Result_flatMap6(([condT, st1]) => _Result_flatMap6((st2) => _Result_flatMap6(([thenT, st3]) => _Result_flatMap6(([elseT, st4]) => _Result_flatMap6((st5) => Ok8(_tuple6(thenT, st5)), u(thenT, elseT, st4, exprSpan3(elseE))), inferExpr(ctx, elseE, st3)), inferExpr(ctx, thenE, st2)), u(condT, tBool, st1, exprSpan3(cond))), inferExpr(ctx, cond, st)));
var inferBindBody = _curry13(7, (ctx, param, paramSpan, body, payloadT, mkBody, st) => (([paramT, bodyEnv, st1]) => _Result_flatMap6((st2) => _Result_flatMap6(([bodyT, st3]) => (([resT, st4]) => {
...
var inferTwoSlotBind = _curry13(8, (ctx, param, paramSpan, value, body, valT, ctor, st) => (([payloadT, st1]) => (([errT, st2]) => _Result_flatMap6((st3) => inferBindBody(ctx, param, paramSpan, body, payloadT, (resT) => tCon(ctor, [resT, errT]), st3), u(valT, tCon(ctor, [payloadT, errT]), st2, exprSpan3(value))))(freshVar(st1)))(freshVar(st)));
var inferQuestionBind = _curry13(8, (ctx, bind, param, paramSpan, value, body, valT, st) => match12(resolve(valT, st)).with({ _tag: "TyVar" }, () => (($written) => inferTwoSlotBind(ctx, param, paramSpan, value, body, valT, "Result", st))(setLetBindMonad(bind, "Result"))).with({ _tag: "TyCon" }, ({ name }) => eq12(name, "Option") ? (($written) => (([payloadT, st1]) => _Result_flatMap6((st2) => inferBindBody(ctx, param, paramSpan, body, payloadT, (resT) => tCon("Option", [resT]), st2), u(valT, tCon("Option", [payloadT]), st1, exprSpan3(value))))(freshVar(st)))(setLetBindMonad(bind, "Option")) : eq12(name, "Result") ? (($written) => inferTwoSlotBind(ctx, param, paramSpan, value, body, valT, "Result", st))(setLetBindMonad(bind, "Result")) : Err8(typeErr(`let? requires Option or Result, got ${showType(zonk(valT, st))}`, exprSpan3(value)))).otherwise(() => Err8(typeErr(`let? requires Option or Result, got ${showType(zonk(valT, st))}`, exprSpan3(value)))));
var inferLetBind = _curry13(8, (ctx, bind, param, paramSpan, monad, value, body, st) => _Result_flatMap6(([valT, st1]) => eq12(monad, "Task") ? inferTwoSlotBind(ctx, param, paramSpan, value, body, valT, "Task", st1) : inferQuestionBind(ctx, bind, param, paramSpan, value, body, valT, st1), inferExpr(ctx, value, st)));
var inferRecordRow = _curry13(3, (ctx, fields, st) => match12(fields).with((_v) => {
  const _g = _v;
  return _g.length === 0;
...
var rWithTail = _curry13(2, (row, tail) => match12(row).with({ _tag: "RowEmpty" }, () => tail).with({ _tag: "RowVar" }, ({ id }) => rVar(id)).with({ _tag: "RowExtend" }, ({ label, fieldType, optional, rest }) => rField(label, fieldType, rWithTail(rest, tail), optional)).exhaustive());
var lookupField = _curry13(2, (row, name) => match12(row).with({ _tag: "RowExtend" }, ({ label, fieldType, optional, rest }) => eq12(label, name) ? Some12(_tuple6(fieldType, optional)) : lookupField(rest, name)).otherwise(() => None12));
var rowEndsEmpty = (row) => match12(row).with({ _tag: "RowEmpty" }, () => true).with({ _tag: "RowExtend" }, ({ rest }) => rowEndsEmpty(rest)).with({ _tag: "RowVar" }, () => false).exhaustive();
var inferFieldAccess = _curry13(5, (ctx, target, name, sp, st) => _Result_flatMap6(([targetT, st1]) => ((zonked) => match12(zonked).with({ _tag: "TyRecord" }, ({ row }) => match12(lookupField(row, name)).with((_v) => {
  const _g = _v;
  return _g._tag === "Some";
...
var inferDuckField = _curry13(4, (targetT, name, sp, st) => (([fieldT, st2]) => (([restRow, st3]) => _Result_flatMap6((st4) => Ok8(_tuple6(fieldT, st4)), u(targetT, tRecord(rExtend(name, fieldT, restRow)), st3, sp)))(freshRowVar(st2)))(freshVar(st)));
var inferNsField = _curry13(5, (ctx, tname, name, sp, st) => match12(_Map_get6(name, _Map_getOr5(new Map, tname, ctx.ns))).with({ _tag: "Some" }, ({ value: sc }) => (([t, st1]) => Ok8(_tuple6(t, st1)))(instantiate(sc, st))).with({ _tag: "None" }, () => Err8(typeErr(`'${tname}' has no member '${name}'`, sp))).exhaustive());
var inferInterpParts = _curry13(3, (ctx, parts, st) => match12(parts).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => Ok8(st)).with((_v) => {
  const _g = _v;
...
var inferTupleElems = _curry13(3, (ctx, elements, st) => match12(elements).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => Ok8(_tuple6([], st))).with((_v) => {
  const _g = _v;
...
var seqElemExpr2 = (el) => match12(el).with({ _tag: "SEExpr" }, ({ expr: e }) => e).with({ _tag: "SESpread" }, ({ expr: e }) => e).exhaustive();
var inferSeqSlotsElems = _curry13(5, (ctx, con, elem, elements, st) => match12(elements).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => Ok8(st)).with((_v) => {
  const _g = _v;
  return _g.length >= 1;
}, ([slot, ...rest]) => ((ex) => _Result_flatMap6(([et, st1]) => ((want) => _Result_flatMap6((st2) => inferSeqSlotsElems(ctx, con, elem, rest, st2), u(want, et, st1, exprSpan3(ex))))(match12(slot).with({ _tag: "SEExpr" }, () => elem).with({ _tag: "SESpread" }, () => tCon(con, [elem])).exhaustive()), inferExpr(ctx, ex, st)))(seqElemExpr2(slot))).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var inferSeqSlots = _curry13(4, (ctx, con, elements, st) => (([elem, st1]) => _Result_flatMap6((st2) => Ok8(_tuple6(tCon(con, [elem]), st2)), inferSeqSlotsElems(ctx, con, elem, elements, st1)))(freshVar(st)));
var inferMapEntries = _curry13(5, (ctx, k, v, entries, st) => match12(entries).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => Ok8(st)).with((_v) => {
  const _g = _v;
...
var inferMapExpr = _curry13(3, (ctx, entries, st) => (([k, st1]) => (([v, st2]) => _Result_flatMap6((st3) => Ok8(_tuple6(tCon("Map", [k, v]), st3)), inferMapEntries(ctx, k, v, entries, st2)))(freshVar(st1)))(freshVar(st)));
var mergeBindingMapsFrom = _curry13(3, (keys, src, dest) => match12(keys).with((_v) => _v.length === 0, () => dest).with((_v) => _v.length >= 1, ([k, ...rest]) => match12(_Map_get6(k, src)).with({ _tag: "Some" }, ({ value: v }) => mergeBindingMapsFrom(rest, src, _Map_set6(k, v, dest))).with({ _tag: "None" }, () => mergeBindingMapsFrom(rest, src, dest)).exhaustive()).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var mergeBindingMaps = _curry13(2, (dest, src) => mergeBindingMapsFrom(_Map_keys4(src), src, dest));
var mergeEnvBindingsFrom = _curry13(3, (keys, bindings, env) => match12(keys).with((_v) => _v.length === 0, () => env).with((_v) => _v.length >= 1, ([k, ...rest]) => match12(_Map_get6(k, bindings)).with({ _tag: "Some" }, ({ value: t }) => mergeEnvBindingsFrom(rest, bindings, _Map_set6(k, mono(t), env))).with({ _tag: "None" }, () => mergeEnvBindingsFrom(rest, bindings, env)).exhaustive()).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var mergeEnvBindings = _curry13(2, (bindings, env) => mergeEnvBindingsFrom(_Map_keys4(bindings), bindings, env));
var inferArms = _curry13(5, (ctx, scrutT, resultT, arms, st) => match12(arms).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => Ok8(st)).with((_v) => {
  const _g = _v;
  return _g.length >= 1;
}, ([arm, ...rest]) => _Result_flatMap6(([patT, bindings, st1]) => _Result_flatMap6((st2) => ((armCtx) => _Result_flatMap6((st3) => _Result_flatMap6(([bodyT, st4]) => _Result_flatMap6((st5) => inferArms(ctx, scrutT, resultT, rest, st5), u(resultT, bodyT, st4, exprSpan3(arm.body))), inferExpr(armCtx, arm.body, st3)), match12(arm.guard).with({ _tag: "None" }, () => Ok8(st2)).with({ _tag: "Some" }, ({ value: g }) => _Result_flatMap6(([guardT, stg]) => u(tBool, guardT, stg, exprSpan3(g)), inferExpr(armCtx, g, st2))).exhaustive()))(ctxWithEnv(ctx, mergeEnvBindings(bindings, ctx.env))), u(scrutT, patT, st1, patSpan3(arm.pattern))), inferPat(ctx, arm.pattern, st))).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var inferMatch = _curry13(4, (ctx, scrutinee, arms, st) => _Result_flatMap6(([scrutT, st1]) => (([resultT, st2]) => _Result_flatMap6((st3) => Ok8(_tuple6(resultT, st3)), inferArms(ctx, scrutT, resultT, arms, st2)))(freshVar(st1)), inferExpr(ctx, scrutinee, st)));
var inferExpr = _curry13(3, (ctx, e, st) => _Result_flatMap6(([t, st1]) => Ok8(_tuple6(t, recordAt(exprSpan3(e), t, st1))), inferExprRaw(ctx, e, st)));
var inferExprRaw = _curry13(3, (ctx, e, st) => match12(e).with({ _tag: "ENum" }, () => Ok8(_tuple6(tNumber, st))).with({ _tag: "EUnit" }, () => Ok8(_tuple6(tUnit, st))).with({ _tag: "EBool" }, () => Ok8(_tuple6(tBool, st))).with({ _tag: "EStr" }, ({ value }) => Ok8(_tuple6(tLit(value), st))).with({ _tag: "ERef" }, ({ name, span: sp }) => match12(_Map_get6(name, ctx.env)).with({ _tag: "Some" }, ({ value: sc }) => (([t, st1]) => Ok8(_tuple6(t, match12(_Map_get6(name, ctx.letOwner)).with({ _tag: "Some" }, ({ value: vsp }) => noteUse(vsp, t, st1)).with({ _tag: "None" }, () => st1).exhaustive())))(instantiate(sc, st))).with({ _tag: "None" }, () => ctx.open ? (([t, st1]) => Ok8(_tuple6(t, st1)))(freshVar(st)) : Err8(typeErr(`unbound variable '${name}'`, sp))).exhaustive()).with({ _tag: "ELambda" }, ({ params, body }) => (([posParams, labParams]) => (([paramTypes, bodyEnv, st1]) => _Result_flatMap6(([annotVars, st2]) => _Result_flatMap6(([labFields, st3]) => ((allTypes) => _Result_flatMap6(([bodyT, st4]) => Ok8(_tuple6(arrowChain(allTypes, bodyT), st4)), inferExpr(ctxWithEnv(ctx, envWithLabFields(labFields, bodyEnv)), body, st3)))(match12(labParams).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => paramTypes).otherwise(() => _Array_append9(tRecord(rowOfLabFields(labFields)), paramTypes))), labFieldsFrom(ctx, labParams, bodyEnv, annotVars, st2)), constrainParamAnnotsFrom(ctx, posParams, paramTypes, new Map, st1)))(bindParamsFrom(posParams, ctx.env, st)))(splitLamParams(params, [], []))).with({ _tag: "ELetIn" }, ({ name, nameSpan: _nameSpan, annot, value, body, span: _span }) => match12(value).with({ _tag: "ELambda" }, () => ((lets) => ((idxOf) => ((tail) => _Result_flatMap6(([localCtx, localSt]) => inferExpr(localCtx, tail, localSt), processGroupsFrom(ctx, stronglyConnected(adjOf(lets, idxOf)), lets, st)))(localTail(e)))(idxOfMap(lets)))(localLetsFrom(e))).otherwise(() => _Result_flatMap6(([valT, st1]) => _Result_flatMap6(([pinned, st2]) => ((widen) => ((sc) => ((vsp) => (($ctx) => inferExpr($ctx, body, noteLet(vsp, st2)))(ctxWithLets(ctx, _Map_set6(name, sc, ctx.env), _Map_set6(name, vsp, ctx.letOwner))))(exprSpan3(value)))(generalize(ctx.env, pinned, st2, widen)))(match12(annot).with({ _tag: "Some" }, () => false).with({ _tag: "None" }, () => true).exhaustive()), match12(annot).with({ _tag: "Some" }, ({ value: te }) => (([at, _, stA]) => _Result_map5((stB) => _tuple6(at, stB), checkFits(valT, at, stA, annotSpan(te))))(typeExprToType(te, new Map, st1, ctx.aliasMap, _Set_fromArray4([])))).with({ _tag: "None" }, () => Ok8(_tuple6(valT, st1))).exhaustive()), inferExpr(ctx, value, st)))).with({ _tag: "ELetBind" }, ({ param, paramSpan, monad, value, body }) => inferLetBind(ctx, e, param, paramSpan, monad, value, body, st)).with({ _tag: "ECall" }, ({ fn, args, origin }) => ((api) => _Result_flatMap6((claimed) => match12(claimed).with({ _tag: "Some" }, ({ value: r }) => Ok8(r)).with({ _tag: "None" }, () => inferNormalCall(ctx, fn, args, st)).exhaustive(), runInferCallHooks(inferCallHooksOf(ctx.plugins), fn, args, origin, st, api)))({ inferExpr: _curry13(2, (e2, st0) => inferExpr(ctx, e2, st0)), unify: u })).with({ _tag: "EPipe", fast: true }, ({ left, right, span: sp }) => match12(right).with({ _tag: "ECall" }, ({ fn: rfn, args: rargs, origin }) => inferExpr(ctx, ECall(rfn, _Array_prepend6(left, rargs), origin, sp), st)).otherwise(() => inferExpr(ctx, ECall(right, [left], None12, sp), st))).with({ _tag: "EPipe" }, ({ left, right, span: sp }) => inferExpr(ctx, ECall(right, [left], None12, sp), st)).with({ _tag: "EDo" }, ({ exprs }) => inferDo(ctx, exprs, st)).with({ _tag: "ETernary" }, ({ cond, thenE, elseE }) => inferTernary(ctx, cond, thenE, elseE, st)).with({ _tag: "ERecord" }, ({ fields, spread, span: sp }) => match12(spread).with({ _tag: "None" }, () => _Result_flatMap6(([row, st1]) => Ok8(_tuple6(tRecord(row), st1)), inferRecordRow(ctx, fields, st))).with({ _tag: "Some" }, ({ value: spreadExpr }) => _Result_flatMap6(([row, st1]) => _Result_flatMap6(([baseT, st2]) => (([tailVar, st3]) => _Result_flatMap6((st4) => Ok8(_tuple6(baseT, st4)), u(baseT, tRecord(rWithTail(row, tailVar)), st3, sp)))(freshRowVar(st2)), inferExpr(ctx, spreadExpr, st1)), inferRecordRow(ctx, fields, st))).exhaustive()).with({ _tag: "EField" }, ({ target, name, span: sp }) => match12(target).with({ _tag: "ERef" }, ({ name: tname }) => and8(_Map_has4(tname, ctx.ns), not7(_Map_has4(tname, ctx.env))) ? inferNsField(ctx, tname, name, sp, st) : inferFieldAccess(ctx, target, name, sp, st)).otherwise(() => inferFieldAccess(ctx, target, name, sp, st))).with({ _tag: "ETuple" }, ({ elements }) => _Result_flatMap6(([elems, st1]) => Ok8(_tuple6(tTuple(elems), st1)), inferTupleElems(ctx, elements, st))).with({ _tag: "EArr" }, ({ elements }) => inferSeqSlots(ctx, "Array", elements, st)).with({ _tag: "EList" }, ({ elements }) => inferSeqSlots(ctx, "List", elements, st)).with({ _tag: "ESet" }, ({ elements }) => inferSeqSlots(ctx, "Set", elements, st)).with({ _tag: "EMap" }, ({ entries }) => inferMapExpr(ctx, entries, st)).with({ _tag: "EMatch" }, ({ scrutinee, arms }) => inferMatch(ctx, scrutinee, arms, st)).with({ _tag: "ELoop" }, ({ params, body }) => _Result_flatMap6(([frame, bodyEnv, bodyOwner, st1]) => inferExpr(ctxWithLoop(ctx, bodyEnv, frame, bodyOwner), body, st1), inferLoopParamsFrom(ctx, params, 0, ctx.env, [], ctx.letOwner, st))).with({ _tag: "ERecur" }, ({ args, span: sp }) => inferRecur(ctx, args, sp, st)).with({ _tag: "EInterp" }, ({ parts }) => _Result_flatMap6((st1) => Ok8(_tuple6(tString, st1)), inferInterpParts(ctx, parts, st))).exhaustive());
var inferDo = _curry13(3, (ctx, exprs, st) => match12(exprs).with((_v) => {
  const _g = _v;
  return _g.length === 0;
...
var inferPatRecordFrom = _curry13(5, (ctx, fields, row, bindings, st) => match12(fields).with((_v) => {
  const _g = _v;
  return _g.length === 0;
...
var inferPatRecord = _curry13(3, (ctx, fields, st) => (([rowBase, st1]) => _Result_flatMap6(([row, bindings, st2]) => Ok8(_tuple6(tRecord(row), bindings, st2)), inferPatRecordFrom(ctx, fields, rowBase, new Map, st1)))(freshRowVar(st)));
var inferPatCtorArgs = _curry13(7, (ctx, ctor, curT, args, st, bindings, sp) => match12(args).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => Ok8(_tuple6(curT, bindings, st))).with((_v) => {
  const _g = _v;
  return _g.length >= 1;
}, ([argPat, ...rest]) => match12(resolve(curT, st)).with({ _tag: "TyFn" }, ({ from: fromT, to: toT }) => _Result_flatMap6(([subT, subBindings, st1]) => _Result_flatMap6((st2) => inferPatCtorArgs(ctx, ctor, toT, rest, st2, mergeBindingMaps(bindings, subBindings), sp), u(fromT, subT, st1, patSpan3(argPat))), inferPat(ctx, argPat, st))).otherwise(() => Err8(typeErr(`constructor '${ctor}' applied to too many arguments`, sp)))).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var inferPatTupleFrom = _curry13(3, (ctx, elems, st) => match12(elems).with((_v) => {
  const _g = _v;
  return _g.length === 0;
...
var inferPatTuple = _curry13(3, (ctx, elems, st) => _Result_flatMap6(([elemTs, bindings, st1]) => Ok8(_tuple6(tTuple(elemTs), bindings, st1)), inferPatTupleFrom(ctx, elems, st)));
var inferSeqPatElems = _curry13(4, (ctx, elem, elems, st) => match12(elems).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => Ok8(_tuple6(new Map, st))).with((_v) => {
  const _g = _v;
...
var inferSeqPat = _curry13(5, (ctx, con, elems, restPat, st) => (([elem, st1]) => {
  const seqT = tCon(con, [elem]);
  return _Result_flatMap6(([bindings, st2]) => match12(restPat).with({ _tag: "None" }, () => Ok8(_tuple6(seqT, bindings, st2))).with({ _tag: "Some" }, ({ value: r }) => _Result_flatMap6(([subT, subBindings, st3]) => _Result_flatMap6((st4) => Ok8(_tuple6(seqT, mergeBindingMaps(bindings, subBindings), st4)), u(subT, seqT, st3, patSpan3(r))), inferPat(ctx, r, st2))).exhaustive(), inferSeqPatElems(ctx, elem, elems, st1));
})(freshVar(st)));
var inferPat = _curry13(3, (ctx, p, st) => _Result_flatMap6(([t, bindings, st1]) => Ok8(_tuple6(t, bindings, recordAt(patSpan3(p), t, st1))), inferPatRaw(ctx, p, st)));
var inferPatRaw = _curry13(3, (ctx, p, st) => match12(p).with({ _tag: "PAs" }, ({ pat, name }) => _Result_flatMap6(([t, bindings, st1]) => Ok8(_tuple6(t, _Map_set6(name, t, bindings), st1)), inferPat(ctx, pat, st))).with({ _tag: "PWild" }, () => (([t, st1]) => Ok8(_tuple6(t, new Map, st1)))(freshVar(st))).with({ _tag: "PUnit" }, () => Ok8(_tuple6(tUnit, new Map, st))).with({ _tag: "PLit" }, () => Ok8(_tuple6(tNumber, new Map, st))).with({ _tag: "PBool" }, () => Ok8(_tuple6(tBool, new Map, st))).with({ _tag: "PStr" }, ({ value }) => Ok8(_tuple6(tLit(value), new Map, st))).with({ _tag: "PBind" }, ({ name }) => (([t, st1]) => Ok8(_tuple6(t, _Map_set6(name, t, new Map), st1)))(freshVar(st))).with({ _tag: "PRecord" }, ({ fields }) => inferPatRecord(ctx, fields, st)).with({ _tag: "PCtor" }, ({ ctor, args, ns, span: sp }) => match12(ns).with({ _tag: "Some" }, ({ value: alias }) => match12(_Map_get6(ctor, _Map_getOr5(new Map, alias, ctx.ns))).with({ _tag: "None" }, () => Err8(typeErr(`'${alias}' has no member '${ctor}'`, sp))).with({ _tag: "Some" }, ({ value: sc }) => (([curT, st1]) => inferPatCtorArgs(ctx, ctor, curT, args, st1, new Map, sp))(instantiate(sc, st))).exhaustive()).with({ _tag: "None" }, () => match12(_Map_get6(ctor, ctx.env)).with({ _tag: "None" }, () => Err8(typeErr(`unknown constructor '${ctor}'`, sp))).with({ _tag: "Some" }, ({ value: sc }) => (([curT, st1]) => inferPatCtorArgs(ctx, ctor, curT, args, st1, new Map, sp))(instantiate(sc, st))).exhaustive()).exhaustive()).with({ _tag: "PTuple" }, ({ elems }) => inferPatTuple(ctx, elems, st)).with({ _tag: "PArr" }, ({ elems, rest }) => inferSeqPat(ctx, "Array", elems, rest, st)).with({ _tag: "PList" }, ({ elems, rest }) => inferSeqPat(ctx, "List", elems, rest, st)).with({ _tag: "POr" }, ({ alts, span: sp }) => inferOrPat(ctx, alts, sp, st)).exhaustive());
var unifyOrPatBinding = _curry13(5, (name, altBindings, bindings, st, sp) => match12(_Map_get6(name, bindings)).with({ _tag: "None" }, () => Ok8(st)).with({ _tag: "Some" }, ({ value: prevT }) => match12(_Map_get6(name, altBindings)).with({ _tag: "None" }, () => Ok8(st)).with({ _tag: "Some" }, ({ value: ty }) => u(prevT, ty, st, sp)).exhaustive()).exhaustive());
var unifyOrPatBindings = _curry13(5, (names, altBindings, bindings, st, sp) => match12(names).with((_v) => _v.length === 0, () => Ok8(st)).with((_v) => _v.length >= 1, ([name, ...rest]) => _Result_flatMap6((st1) => unifyOrPatBindings(rest, altBindings, bindings, st1, sp), unifyOrPatBinding(name, altBindings, bindings, st, sp))).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var inferOrPatAlts = _curry13(6, (ctx, alts, i, t, bindings, st) => match12(_Array_get9(i, alts)).with({ _tag: "None" }, () => Ok8(st)).with({ _tag: "Some" }, ({ value: alt }) => _Result_flatMap6(([altT, altBindings, st1]) => _Result_flatMap6((st2) => _Result_flatMap6((st3) => inferOrPatAlts(ctx, alts, i + 1, t, bindings, st3), unifyOrPatBindings(_Map_keys4(altBindings), altBindings, bindings, st2, patSpan3(alt))), u(t, altT, st1, patSpan3(alt))), inferPat(ctx, alt, st))).exhaustive());
var inferOrPat = _curry13(4, (ctx, alts, sp, st) => match12(alts).with((_v) => {
  const _g = _v;
  return _g.length === 0;
...
var patternBindsOpt = (rest) => match12(rest).with({ _tag: "Some" }, ({ value: r }) => patternBinds(r)).with({ _tag: "None" }, () => []).exhaustive();
var patternBinds = (p) => match12(p).with({ _tag: "PAs" }, ({ pat, name }) => _Array_append9(name, patternBinds(pat))).with({ _tag: "PBind" }, ({ name }) => [name]).with({ _tag: "PRecord" }, ({ fields }) => _Array_flatMap3((f) => patternBinds(f.pat), fields)).with({ _tag: "PCtor" }, ({ args }) => _Array_flatMap3(patternBinds, args)).with({ _tag: "PTuple" }, ({ elems }) => _Array_flatMap3(patternBinds, elems)).with({ _tag: "PArr" }, ({ elems, rest }) => _Array_concat4(_Array_flatMap3(patternBinds, elems), patternBindsOpt(rest))).with({ _tag: "PList" }, ({ elems, rest }) => _Array_concat4(_Array_flatMap3(patternBinds, elems), patternBindsOpt(rest))).with({ _tag: "POr" }, ({ alts }) => match12(_Array_head4(alts)).with({ _tag: "Some" }, ({ value: first }) => patternBinds(first)).with({ _tag: "None" }, () => []).exhaustive()).otherwise(() => []);
var addAllFrom = _curry13(2, (names, set) => match12(names).with((_v) => _v.length === 0, () => set).with((_v) => _v.length >= 1, ([n, ...rest]) => addAllFrom(rest, _Set_add4(n, set))).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var paramBound = _curry13(2, (p, bound) => match12(p).with({ _tag: "LPSpanned" }, ({ param: inner }) => paramBound(inner, bound)).with({ _tag: "LPName" }, ({ name }) => _Set_add4(name, bound)).with({ _tag: "LPTuple" }, ({ names }) => addAllFrom(names, bound)).with({ _tag: "LPRecord" }, ({ fields }) => addAllFrom(fields, bound)).with({ _tag: "LPLabeled" }, ({ name }) => _Set_add4(name, bound)).exhaustive());
var lambdaBound = _curry13(2, (params, bound) => match12(params).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => bound).with((_v) => {
  const _g = _v;
...
var labeledDefaultRefs = _curry13(3, (params, bound, acc) => match12(params).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => acc).with((_v) => {
  const _g = _v;
  return _g.length >= 1;
}, ([p, ...rest]) => match12(p).with({ _tag: "LPSpanned" }, ({ param: inner }) => labeledDefaultRefs([inner, ...rest], bound, acc)).with((_v) => {
  const _g = _v;
  return _g._tag === "LPLabeled" && _g.defaultValue._tag === "Some";
...
var loopBound = _curry13(2, (params, bound) => reduce2(_curry13(2, (b, p) => _Set_add4(p.name, b)), bound, params));
var loopInitRefsFrom = _curry13(4, (params, i, bound, acc) => match12(_Array_get9(i, params)).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, ({ value: p }) => loopInitRefsFrom(params, i + 1, bound, freeRefs(p.init, bound, acc))).exhaustive());
var freeRefsList = _curry13(3, (es, bound, acc) => match12(es).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => acc).with((_v) => {
  const _g = _v;
...
var freeRefsFields = _curry13(3, (fields, bound, acc) => match12(fields).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => acc).with((_v) => {
  const _g = _v;
...
var freeRefsEntries = _curry13(3, (entries, bound, acc) => match12(entries).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => acc).with((_v) => {
  const _g = _v;
...
var freeRefsInterpParts = _curry13(3, (parts, bound, acc) => match12(parts).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => acc).with((_v) => {
  const _g = _v;
...
var freeRefsArms = _curry13(3, (arms, bound, acc) => match12(arms).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => acc).with((_v) => {
  const _g = _v;
  return _g.length >= 1;
}, ([arm, ...rest]) => ((armBound) => ((acc1) => freeRefsArms(rest, bound, freeRefs(arm.body, armBound, acc1)))(match12(arm.guard).with({ _tag: "Some" }, ({ value: g }) => freeRefs(g, armBound, acc)).with({ _tag: "None" }, () => acc).exhaustive()))(addAllFrom(patternBinds(arm.pattern), bound))).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var freeRefs = _curry13(3, (e, bound, acc) => match12(e).with({ _tag: "ENum" }, () => acc).with({ _tag: "EUnit" }, () => acc).with({ _tag: "EBool" }, () => acc).with({ _tag: "EStr" }, () => acc).with({ _tag: "ERef" }, ({ name }) => _Set_has4(name, bound) ? acc : _Set_add4(name, acc)).with({ _tag: "ECall" }, ({ fn, args }) => freeRefsList(args, bound, freeRefs(fn, bound, acc))).with({ _tag: "ELambda" }, ({ params, body }) => freeRefs(body, lambdaBound(params, bound), labeledDefaultRefs(params, bound, acc))).with({ _tag: "ELetIn" }, ({ name, value, body }) => ((valueBound) => ((acc1) => freeRefs(body, _Set_add4(name, bound), acc1))(freeRefs(value, valueBound, acc)))(match12(value).with({ _tag: "ELambda" }, () => _Set_add4(name, bound)).otherwise(() => bound))).with({ _tag: "ELetBind" }, ({ param, value, body }) => ((acc1) => freeRefs(body, paramBound(param, bound), acc1))(freeRefs(value, bound, acc))).with({ _tag: "EPipe" }, ({ left, right }) => freeRefs(right, bound, freeRefs(left, bound, acc))).with({ _tag: "EDo" }, ({ exprs }) => freeRefsList(exprs, bound, acc)).with({ _tag: "ETernary" }, ({ cond, thenE, elseE }) => freeRefs(elseE, bound, freeRefs(thenE, bound, freeRefs(cond, bound, acc)))).with({ _tag: "EMatch" }, ({ scrutinee, arms }) => freeRefsArms(arms, bound, freeRefs(scrutinee, bound, acc))).with({ _tag: "ELoop" }, ({ params, body }) => freeRefs(body, loopBound(params, bound), loopInitRefsFrom(params, 0, bound, acc))).with({ _tag: "ERecur" }, ({ args }) => freeRefsList(args, bound, acc)).with({ _tag: "ERecord" }, ({ fields, spread }) => freeRefsFields(fields, bound, match12(spread).with({ _tag: "Some" }, ({ value: s }) => freeRefs(s, bound, acc)).with({ _tag: "None" }, () => acc).exhaustive())).with({ _tag: "EField" }, ({ target }) => freeRefs(target, bound, acc)).with({ _tag: "ETuple" }, ({ elements }) => freeRefsList(elements, bound, acc)).with({ _tag: "EArr" }, ({ elements }) => freeRefsList(map8(seqElemExpr2, elements), bound, acc)).with({ _tag: "EList" }, ({ elements }) => freeRefsList(map8(seqElemExpr2, elements), bound, acc)).with({ _tag: "ESet" }, ({ elements }) => freeRefsList(map8(seqElemExpr2, elements), bound, acc)).with({ _tag: "EMap" }, ({ entries }) => freeRefsEntries(entries, bound, acc)).with({ _tag: "EInterp" }, ({ parts }) => freeRefsInterpParts(parts, bound, acc)).exhaustive());
var seedBuiltinsFrom = _curry13(4, (keys, builtins, env, st) => match12(keys).with((_v) => _v.length === 0, () => env).with((_v) => _v.length >= 1, ([n, ...rest]) => match12(_Map_get6(n, builtins)).with({ _tag: "Some" }, ({ value: t }) => seedBuiltinsFrom(rest, builtins, _Map_set6(n, generalize(env, t, st, true), env), st)).with({ _tag: "None" }, () => seedBuiltinsFrom(rest, builtins, env, st)).exhaustive()).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var seedBuiltins = _curry13(3, (builtins, env, st) => seedBuiltinsFrom(_Map_keys4(builtins), builtins, env, st));
var seedNsMembersFrom = _curry13(5, (keys, members, env, st, acc) => match12(keys).with((_v) => _v.length === 0, () => acc).with((_v) => _v.length >= 1, ([m, ...rest]) => match12(_Map_get6(m, members)).with({ _tag: "Some" }, ({ value: t }) => seedNsMembersFrom(rest, members, env, st, _Map_set6(m, generalize(env, t, st, true), acc))).with({ _tag: "None" }, () => seedNsMembersFrom(rest, members, env, st, acc)).exhaustive()).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var seedNsFrom = _curry13(5, (nsNames, namespaces, env, st, acc) => match12(nsNames).with((_v) => _v.length === 0, () => acc).with((_v) => _v.length >= 1, ([nsName, ...rest]) => match12(_Map_get6(nsName, namespaces)).with({ _tag: "Some" }, ({ value: members }) => seedNsFrom(rest, namespaces, env, st, _Map_set6(nsName, seedNsMembersFrom(_Map_keys4(members), members, env, st, new Map), acc))).with({ _tag: "None" }, () => seedNsFrom(rest, namespaces, env, st, acc)).exhaustive()).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var seedNs = _curry13(3, (namespaces, env, st) => seedNsFrom(_Map_keys4(namespaces), namespaces, env, st, new Map));
var seedNsImportsFrom = _curry13(3, (aliases, nsImports, ns) => match12(aliases).with((_v) => _v.length === 0, () => ns).with((_v) => _v.length >= 1, ([alias, ...rest]) => match12(_Map_get6(alias, nsImports)).with({ _tag: "Some" }, ({ value: members }) => seedNsImportsFrom(rest, nsImports, _Map_set6(alias, members, ns))).with({ _tag: "None" }, () => seedNsImportsFrom(rest, nsImports, ns)).exhaustive()).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var seedNsImports = _curry13(2, (nsImports, ns) => seedNsImportsFrom(_Map_keys4(nsImports), nsImports, ns));
var aliasMapFrom = _curry13(2, (stmts, acc) => match12(stmts).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => acc).with((_v) => {
  const _g = _v;
  return _g.length >= 1;
}, ([s, ...rest]) => match12(s).with((_v) => {
  const _g = _v;
  return _g._tag === "SType" && _g.alias._tag === "Some";
}, ({ name, params, alias: { value: fields } }) => aliasMapFrom(rest, _Map_set6(name, { params, fields, expr: None12 }, acc))).with((_v) => {
  const _g = _v;
  return _g._tag === "SType" && _g.aliasType._tag === "Some";
}, ({ name, params, aliasType: { value: te } }) => aliasMapFrom(rest, _Map_set6(name, { params, fields: [], expr: Some12(te) }, acc))).otherwise(() => aliasMapFrom(rest, acc))).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var registerCtorsFrom = _curry13(6, (ctors, typeName, params, aliasMap, env, st) => match12(ctors).with((_v) => _v.length === 0, () => _tuple6(env, st)).with((_v) => _v.length >= 1, ([c, ...rest]) => (([sc, st1]) => registerCtorsFrom(rest, typeName, params, aliasMap, _Map_set6(c.name, sc, env), st1))(ctorScheme(typeName, params, c, st, aliasMap))).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var registerUserCtorsFrom = _curry13(4, (stmts, aliasMap, env, st) => match12(stmts).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => _tuple6(env, st)).with((_v) => {
  const _g = _v;
  return _g.length >= 1;
}, ([s, ...rest]) => match12(s).with({ _tag: "SType" }, ({ name, params, ctors }) => (([env1, st1]) => registerUserCtorsFrom(rest, aliasMap, env1, st1))(registerCtorsFrom(ctors, name, params, aliasMap, env, st))).otherwise(() => registerUserCtorsFrom(rest, aliasMap, env, st))).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var registerBuiltinCtorGroup = _curry13(6, (ctors, typeName, params, aliasMap, env, st) => match12(ctors).with((_v) => _v.length === 0, () => _tuple6(env, st)).with((_v) => _v.length >= 1, ([c, ...rest]) => _Map_has4(c.name, env) ? registerBuiltinCtorGroup(rest, typeName, params, aliasMap, env, st) : (([sc, st1]) => registerBuiltinCtorGroup(rest, typeName, params, aliasMap, _Map_set6(c.name, sc, env), st1))(ctorScheme(typeName, params, c, st, aliasMap))).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var registerBuiltinCtorsFrom = _curry13(4, (decls, aliasMap, env, st) => match12(decls).with((_v) => _v.length === 0, () => _tuple6(env, st)).with((_v) => _v.length >= 1, ([d, ...rest]) => (([env1, st1]) => registerBuiltinCtorsFrom(rest, aliasMap, env1, st1))(registerBuiltinCtorGroup(d.ctors, d.name, d.params, aliasMap, env, st))).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var registerExternsFrom = _curry13(4, (stmts, aliasMap, env, st) => match12(stmts).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => _tuple6(env, st)).with((_v) => {
  const _g = _v;
  return _g.length >= 1;
}, ([s, ...rest]) => match12(s).with({ _tag: "SExtern" }, ({ name, params, typeExpr }) => (([vars, st0]) => (([t, _, st1]) => registerExternsFrom(rest, aliasMap, _Map_set6(name, generalize(env, t, st1, false), env), st1))(typeExprToType(typeExpr, vars, st0, aliasMap, _Set_fromArray4([]))))(reduce2(_curry13(2, ([vs, s2], param) => (([v, s1]) => _tuple6(_Map_set6(param, v, vs), s1))(freshVar(s2))), _tuple6(new Map, st), params))).otherwise(() => registerExternsFrom(rest, aliasMap, env, st))).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var letsOfFrom = (stmts) => match12(stmts).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => []).with((_v) => {
  const _g = _v;
  return _g.length >= 1;
}, ([s, ...rest]) => match12(s).with({ _tag: "SLet" }, () => _Array_prepend6(s, letsOfFrom(rest))).otherwise(() => letsOfFrom(rest))).otherwise(() => {
  throw new Error("non-exhaustive match");
});
var localLetsFrom = (e) => {
  const collect2 = _curry13(2, (current, acc) => match12(current).with({ _tag: "ELetIn" }, ({ name, nameSpan, annot, value, body, span }) => match12(value).with({ _tag: "ELambda" }, () => collect2(body, _Array_append9(SLet(name, nameSpan, annot, value, false, None12, span), acc))).otherwise(() => acc)).otherwise(() => acc));
  return collect2(e, []);
};
var localTail = (e) => match12(e).with((_v) => {
  const _g = _v;
  return _g._tag === "ELetIn" && _g.value._tag === "ELambda";
}, ({ body }) => localTail(body)).otherwise(() => e);
var idxOfFrom = _curry13(3, (lets, i0, acc0) => {
  let i = i0;
  let acc = acc0;
  while (true) {
    const _step = match12(_Array_get9(i, lets)).with({ _tag: "None" }, () => _done7(acc)).with((_v) => {
      const _g = _v;
...
var depsOf = _curry13(2, (letStmt, idxOf) => match12(letStmt).with({ _tag: "SLet" }, ({ value }) => _Array_flatMap3((r) => match12(_Map_get6(r, idxOf)).with({ _tag: "Some" }, ({ value: j }) => [j]).with({ _tag: "None" }, () => []).exhaustive(), _Set_toArray2(freeRefs(value, _Set_fromArray4([]), _Set_fromArray4([]))))).otherwise(() => []));
var adjOf = _curry13(2, (lets, idxOf) => map8((s) => depsOf(s, idxOf), lets));
var groupOfFrom = _curry13(2, (idxs, lets) => match12(idxs).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => []).with((_v) => {
  const _g = _v;
  return _g.length >= 1;
}, ([i, ...rest]) => match12(_Array_get9(i, lets)).with({ _tag: "Some" }, ({ value: s }) => _Array_prepend6(s, groupOfFrom(rest, lets))).with({ _tag: "None" }, () => groupOfFrom(rest, lets)).exhaustive()).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var preBindGroupFrom = _curry13(3, (group, env, st) => match12(group).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => _tuple6(env, st)).with((_v) => {
  const _g = _v;
  return _g.length >= 1;
}, ([s, ...rest]) => match12(s).with({ _tag: "SLet" }, ({ name }) => (([v, st1]) => preBindGroupFrom(rest, _Map_set6(name, mono(v), env), st1))(freshVar(st))).otherwise(() => preBindGroupFrom(rest, env, st))).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var inferGroupFrom = _curry13(3, (ctx, group, st) => match12(group).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => Ok8(_tuple6(new Map, st))).with((_v) => {
  const _g = _v;
  return _g.length >= 1;
}, ([s, ...rest]) => match12(s).with({ _tag: "SLet" }, ({ name, annot, value, span }) => _Result_flatMap6(([t, st1]) => match12(_Map_get6(name, ctx.env)).with({ _tag: "Some" }, ({ value: selfSc }) => _Result_flatMap6((st2) => _Result_flatMap6(([pinned, st3]) => _Result_flatMap6(([restTypes, st4]) => Ok8(_tuple6(_Map_set6(name, pinned, restTypes), st4)), inferGroupFrom(ctx, rest, st3)), match12(annot).with({ _tag: "Some" }, ({ value: te }) => (([at, _, stA]) => _Result_map5((stB) => _tuple6(at, stB), checkFits(t, at, stA, annotSpan(te))))(typeExprToType(te, new Map, st2, ctx.aliasMap, _Set_fromArray4([])))).with({ _tag: "None" }, () => Ok8(_tuple6(t, st2))).exhaustive()), u(selfSc.ty, t, st1, span))).with({ _tag: "None" }, () => Err8(typeErr(`internal: missing self-binding for '${name}'`, span))).exhaustive(), inferExpr(ctx, value, st))).otherwise(() => inferGroupFrom(ctx, rest, st))).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var dropGroupFrom = _curry13(2, (group, env) => match12(group).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => env).with((_v) => {
  const _g = _v;
  return _g.length >= 1;
}, ([s, ...rest]) => match12(s).with({ _tag: "SLet" }, ({ name }) => dropGroupFrom(rest, _Map_delete(name, env))).otherwise(() => dropGroupFrom(rest, env))).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var generalizeGroupFrom = _curry13(4, (group, bodyTypes, env, st) => match12(group).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => env).with((_v) => {
  const _g = _v;
  return _g.length >= 1;
}, ([s, ...rest]) => match12(s).with({ _tag: "SLet" }, ({ name, annot }) => match12(_Map_get6(name, bodyTypes)).with({ _tag: "Some" }, ({ value: t }) => ((widen) => generalizeGroupFrom(rest, bodyTypes, _Map_set6(name, generalize(env, t, st, widen), env), st))(match12(annot).with({ _tag: "None" }, () => true).with({ _tag: "Some" }, () => false).exhaustive())).with({ _tag: "None" }, () => generalizeGroupFrom(rest, bodyTypes, env, st)).exhaustive()).otherwise(() => generalizeGroupFrom(rest, bodyTypes, env, st))).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var noteGroupLets = _curry13(3, (group, letOwner, st) => match12(group).with((_v) => {
  const _g = _v;
  return _g.length === 0;
...
var processGroupsFrom = _curry13(4, (ctx, sccs, lets, st) => match12(sccs).with((_v) => {
  const _g = _v;
  return _g.length === 0;
...
var inferExprStmtsFrom = _curry13(3, (ctx, stmts, st) => match12(stmts).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => Ok8(st)).with((_v) => {
  const _g = _v;
  return _g.length >= 1;
}, ([s, ...rest]) => match12(s).with({ _tag: "SExpr" }, ({ value, span }) => _Result_flatMap6(([t, st1]) => _Result_flatMap6((st2) => inferExprStmtsFrom(ctx, rest, st2), u(t, tUnit, st1, span)), inferExpr(ctx, value, st))).otherwise(() => inferExprStmtsFrom(ctx, rest, st))).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var seedImportsFrom = _curry13(3, (keys, imports, env) => match12(keys).with((_v) => _v.length === 0, () => env).with((_v) => _v.length >= 1, ([k, ...rest]) => match12(_Map_get6(k, imports)).with({ _tag: "Some" }, ({ value: sc }) => seedImportsFrom(rest, imports, _Map_set6(k, sc, env))).with({ _tag: "None" }, () => seedImportsFrom(rest, imports, env)).exhaustive()).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var qualifyTe = _curry13(3, (te, alias, from) => match12(te).with({ _tag: "TyName" }, ({ name, span: sp }) => _Map_has4(name, from) ? TyQual(alias, name, sp, [], sp) : te).with({ _tag: "TyApp" }, ({ ctor, args, span: sp }) => ((args1) => _Map_has4(ctor, from) ? TyQual(alias, ctor, sp, args1, sp) : TyApp(ctor, args1, sp))(map8((a) => qualifyTe(a, alias, from), args))).with({ _tag: "TyArrow" }, ({ from: fromTe, to: toTe, span: sp }) => TyArrow(qualifyTe(fromTe, alias, from), qualifyTe(toTe, alias, from), sp)).with({ _tag: "TyTuple" }, ({ elems, span: sp }) => TyTuple(map8((e) => qualifyTe(e, alias, from), elems), sp)).with({ _tag: "TyList" }, ({ elem, span: sp }) => TyList(qualifyTe(elem, alias, from), sp)).with({ _tag: "TyUnion" }, ({ members, span: sp }) => TyUnion(map8((m) => qualifyTe(m, alias, from), members), sp)).otherwise(() => te));
var qualifyField = _curry13(3, (fld, alias, from) => ({ name: fld.name, fieldType: qualifyTe(fld.fieldType, alias, from), optional: fld.optional }));
var qualifyInfo = _curry13(3, (info, alias, from) => ({ params: info.params, fields: map8((f) => qualifyField(f, alias, from), info.fields), expr: _Option_map((te) => qualifyTe(te, alias, from), info.expr) }));
var qualAliasSeedFrom = _curry13(4, (names, alias, from, acc) => match12(names).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => acc).with((_v) => {
  const _g = _v;
  return _g.length >= 1;
}, ([n, ...rest]) => qualAliasSeedFrom(rest, alias, from, match12(_Map_get6(n, from)).with({ _tag: "Some" }, ({ value: info }) => _Map_set6(`${alias}.${n}`, qualifyInfo(info, alias, from), acc)).with({ _tag: "None" }, () => acc).exhaustive())).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var qualAliasSeed = _curry13(3, (stmts, quals, acc) => match12(stmts).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => acc).with((_v) => {
  const _g = _v;
  return _g.length >= 1;
}, ([s, ...rest]) => qualAliasSeed(rest, quals, match12(s).with({ _tag: "SImportNs" }, ({ alias }) => match12(_Map_get6(alias.name, quals)).with({ _tag: "Some" }, ({ value: dep }) => qualAliasSeedFrom(_Map_keys4(dep.aliases), alias.name, dep.aliases, acc)).with({ _tag: "None" }, () => acc).exhaustive()).otherwise(() => acc))).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var zonkRecorded = _curry13(2, (recorded, st) => map8((r) => ({ span: r.span, ty: zonk(r.ty, st) }), _Array_reverse(recorded)));
var isConcrete = (t) => {
  const f = freeInType(t);
  return and8(eq12(_Set_size(f.tv), 0), eq12(_Set_size(f.rv), 0));
};
var allSameConcreteFrom = _curry13(3, (shown, uses, i) => match12(_Array_get9(i, uses)).with({ _tag: "None" }, () => true).with({ _tag: "Some" }, ({ value: t }) => and8(isConcrete(t), eq12(showType(t), shown)) ? allSameConcreteFrom(shown, uses, i + 1) : false).exhaustive());
var allSameConcrete = _curry13(2, (shown, uses) => allSameConcreteFrom(shown, uses, 0));
var resolveLetParamsFrom = _curry13(2, (keys, st) => match12(keys).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => []).with((_v) => {
  const _g = _v;
  return _g.length >= 1;
}, ([k, ...rest]) => ((tail) => ((uses) => match12(_Array_get9(0, uses)).with({ _tag: "None" }, () => tail).with({ _tag: "Some" }, ({ value: first }) => allSameConcrete(showType(first), uses) ? match12(_Map_get6(k, st.letSpans)).with({ _tag: "Some" }, ({ value: span }) => _Array_prepend6({ span, ty: first }, tail)).with({ _tag: "None" }, () => tail).exhaustive() : tail).exhaustive())(map8((t) => zonk(t, st), _Map_getOr5([], k, st.letUses))))(resolveLetParamsFrom(rest, st))).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var resolveLetParams = (st) => resolveLetParamsFrom(_Map_keys4(st.letSpans), st);
var runInferImports = _curry13(8, (stmts, builtins, namespaces, openMode, imports, nsImports, quals, pluginsOpt) => {
  ...
  return (([env1, st1]) => (([env2, st2]) => (([env3, st3]) => {
    ...
    return match12(processGroupsFrom({ env: env4, open: openMode, ns: ns0, aliasMap, plugins, loopStack: [], letOwner: new Map }, sccs, lets, st3)).with((_v) => {
      const _g = _v;
      return _g._tag === "Ok";
    }, ({ value: [finalCtx, st4] }) => match12(inferExprStmtsFrom(finalCtx, stmts, st4)).with({ _tag: "Ok" }, ({ value: st5 }) => Ok8({ env: finalCtx.env, types: zonkRecorded(st5.recorded, st5), aliases: aliasMap, letParams: resolveLetParams(st5) })).with({ _tag: "Err" }, ({ error: e }) => Err8(e)).exhaustive()).with({ _tag: "Err" }, ({ error: e }) => Err8(e)).exhaustive();
...
var inferProgramImports = _curry13(8, (stmts, builtins, namespaces, openMode, imports, nsImports, quals, pluginsOpt) => _Result_map5((r) => r.env, runInferImports(stmts, builtins, namespaces, openMode, imports, nsImports, quals, pluginsOpt)));
var emptyQuals2 = new Map;
var inferProgram = _curry13(4, (stmts, builtins, namespaces, openMode) => inferProgramImports(stmts, builtins, namespaces, openMode, new Map, new Map, emptyQuals2, None12));
var inferProgramImportsTypes = _curry13(8, (stmts, builtins, namespaces, openMode, imports, nsImports, quals, pluginsOpt) => runInferImports(stmts, builtins, namespaces, openMode, imports, nsImports, quals, pluginsOpt));
var inferProgramTypes = _curry13(4, (stmts, builtins, namespaces, openMode) => runInferImports(stmts, builtins, namespaces, openMode, new Map, new Map, emptyQuals2, None12));
var inferProgramWith = _curry13(5, (stmts, builtins, namespaces, openMode, pluginsOpt) => inferProgramImports(stmts, builtins, namespaces, openMode, new Map, new Map, emptyQuals2, pluginsOpt));
var takeScheme = _curry13(3, (name, env, acc) => match12(_Map_get6(name, env)).with({ _tag: "Some" }, ({ value: sc }) => _Map_set6(name, sc, acc)).with({ _tag: "None" }, () => acc).exhaustive());
var exportCtorsInto = _curry13(4, (ctors, i, env, acc) => match12(_Array_get9(i, ctors)).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, ({ value: c }) => exportCtorsInto(ctors, i + 1, env, takeScheme(c.name, env, acc))).exhaustive());
var exportedSchemesFrom = _curry13(4, (stmts, i0, env, acc0) => {
  let i = i0;
  let acc = acc0;
  while (true) {
    const _step = match12(_Array_get9(i, stmts)).with({ _tag: "None" }, () => _done7(acc)).with((_v) => {
      const _g = _v;
...
var exportedSchemes = _curry13(2, (stmts, env) => exportedSchemesFrom(stmts, 0, env, new Map));

import { None as None13, Some as Some13, _Array_append as _Array_append10, _Array_concat as _Array_concat5, _Array_get as _Array_get10, _Array_head as _Array_head5, _Array_prepend as _Array_prepend7, _Map_get as _Map_get7, _Map_getOr as _Map_getOr6, _Map_keys as _Map_keys5, _Option_contains as _Option_contains3, _Option_exists as _Option_exists4, _Option_isSome as _Option_isSome3, _Option_unwrapOr as _Option_unwrapOr7, _Set_add as _Set_add5, _Set_fromArray as _Set_fromArray5, _Set_has as _Set_has5, _Set_toArray as _Set_toArray3, _Set_union, _Str_chars, _Str_codeAt as _Str_codeAt6, _Str_concat as _Str_concat2, _Str_endsWith as _Str_endsWith2, _Str_join as _Str_join5, _Str_length as _Str_length3, _Str_replace, _Str_slice as _Str_slice3, _Str_split as _Str_split2, _Str_startsWith as _Str_startsWith3, _curry as _curry14, _done as _done8, _recur as _recur8, _tuple as _tuple7, and as and9, concat, eq as eq13, filter as filter4, length as length10, map as map9, not as not8, or as or7, reduce as reduce3, show as show6 } from "@mochi/compiler/runtime";
import { match as match13 } from "@onrails/pattern";
var jsGenOpts = { annotateLet: None13, annotateCtor: None13, annotateParams: None13, annotateEmpty: None13, annotateLetin: None13, annotateCall: None13, guardBaseType: None13, flattenPipe: false, tupleHelper: false, preserveInfix: false, preserveJsx: false, moduleExt: ".js", docs: true };
var hook1 = _curry14(2, (h, x) => match13(h).with({ _tag: "None" }, () => None13).with({ _tag: "Some" }, ({ value: f }) => f(x)).exhaustive());
var hook2 = _curry14(3, (h, x, y) => match13(h).with({ _tag: "None" }, () => None13).with({ _tag: "Some" }, ({ value: f }) => f(x, y)).exhaustive());
var emptyNsCtor = _curry14(2, (con, ann) => match13(ann).with({ _tag: "None" }, () => `new ${con}()`).with({ _tag: "Some" }, ({ value: t }) => `new ${t}()`).exhaustive());
var isIdentStart = (c) => or7(or7(or7(and9(c >= 65, c <= 90), and9(c >= 97, c <= 122)), eq13(c, 95)), eq13(c, 36));
var isIdentPart = (c) => or7(isIdentStart(c), and9(c >= 48, c <= 57));
var identPartsFrom = _curry14(2, (s, i) => match13(_Str_codeAt6(i, s)).with({ _tag: "None" }, () => true).with({ _tag: "Some" }, ({ value: c }) => and9(isIdentPart(c), identPartsFrom(s, i + 1))).exhaustive());
var isJsIdent = (s) => match13(_Str_codeAt6(0, s)).with({ _tag: "None" }, () => false).with({ _tag: "Some" }, ({ value: c }) => and9(isIdentStart(c), identPartsFrom(s, 1))).exhaustive();
var isUpperStart3 = (s) => _Option_exists4((n) => and9(n >= 65, n <= 90), _Str_codeAt6(0, s));
var isNullaryCtor = _curry14(2, (name, keys) => _Option_exists4((ks) => eq13(length10(ks), 0), _Map_get7(name, keys)));
var isCtorRef = (fn) => match13(fn).with({ _tag: "ERef" }, ({ name }) => isUpperStart3(name)).otherwise(() => false);
var suffixOr = _curry14(2, (name, ann) => match13(ann).with({ _tag: "None" }, () => name).with({ _tag: "Some" }, ({ value: t }) => `${name}: ${t}`).exhaustive());
var bareParamAnnots = { generics: "", params: [] };
var paramAnnotsFor = _curry14(3, (h, sp, arity) => match13(h).with({ _tag: "None" }, () => bareParamAnnots).with({ _tag: "Some" }, ({ value: f }) => f(sp, arity)).exhaustive());
var annotatedParams = _curry14(3, (cparams, annots, i) => match13(_Array_get10(i, cparams)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: p }) => _Array_prepend7(suffixOr(genParam(p), _Option_unwrapOr7(None13, _Array_get10(i, annots))), annotatedParams(cparams, annots, i + 1))).exhaustive());
var castOr = _curry14(2, (js, ann) => match13(ann).with({ _tag: "None" }, () => js).with({ _tag: "Some" }, ({ value: t }) => `(${js} as ${t})`).exhaustive());
var bindRuntime = (monad) => eq13(monad, "Option") ? "_Option_flatMap" : eq13(monad, "Result") ? "_Result_flatMap" : "_Task_andThen";
var allOfFrom2 = _curry14(3, (f, xs, i) => match13(_Array_get10(i, xs)).with({ _tag: "None" }, () => true).with({ _tag: "Some" }, ({ value: x }) => f(x) ? allOfFrom2(f, xs, i + 1) : false).exhaustive());
var allOf2 = _curry14(2, (f, xs) => allOfFrom2(f, xs, 0));
var someOfFrom3 = _curry14(3, (f, xs, i) => match13(_Array_get10(i, xs)).with({ _tag: "None" }, () => false).with({ _tag: "Some" }, ({ value: x }) => f(x) ? true : someOfFrom3(f, xs, i + 1)).exhaustive());
var someOf3 = _curry14(2, (f, xs) => someOfFrom3(f, xs, 0));
var escChar2 = (c) => match13(c).with("\\", () => "\\\\").with('"', () => "\\\"").with(`
`, () => "\\n").with("\t", () => "\\t").otherwise(() => c);
var jsStringLit = (s) => `"${_Str_join5("", map9(escChar2, _Str_chars(s)))}"`;
var escTemplateLoop = _curry14(3, (chars, i0, acc0) => {
  let i = i0;
  let acc = acc0;
  while (true) {
    const _step = match13(_Array_get10(i, chars)).with({ _tag: "None" }, () => _done8(acc)).with({ _tag: "Some", value: "\\" }, () => _recur8(i + 1, `${acc}\\\\`)).with({ _tag: "Some", value: "`" }, () => _recur8(i + 1, `${acc}\\\``)).with((_v) => {
      const _g = _v;
      return _g._tag === "Some" && _g.value === "$" && _Option_contains3("{", _Array_get10(i + 1, chars));
...
var keyAt = _curry14(3, (ctx, ctor, i) => match13(_Map_get7(ctor, ctx.keys)).with({ _tag: "Some" }, ({ value: ks }) => _Option_unwrapOr7(`_${show6(i)}`, _Array_get10(i, ks))).with({ _tag: "None" }, () => `_${show6(i)}`).exhaustive());
var nsRuntimeId = _curry14(3, (ctx, target, name) => match13(target).with({ _tag: "ERef" }, ({ name: refName }) => match13(_Map_get7(refName, ctx.ns)).with({ _tag: "Some" }, ({ value: members }) => _Map_get7(name, members)).with({ _tag: "None" }, () => None13).exhaustive()).otherwise(() => None13));
var emptyNsEmit = _curry14(3, (target, name, ann) => match13(target).with({ _tag: "ERef" }, ({ name: refName }) => eq13(name, "empty") ? eq13(refName, "Set") ? Some13(emptyNsCtor("Set", ann)) : eq13(refName, "Map") ? Some13(emptyNsCtor("Map", ann)) : eq13(refName, "List") ? Some13("_list(function* () {})") : None13 : None13).otherwise(() => None13));
var isLabeledParam3 = (p) => match13(p).with({ _tag: "LPLabeled" }, () => true).with({ _tag: "LPSpanned" }, ({ param: inner }) => isLabeledParam3(inner)).otherwise(() => false);
var splitLamParams2 = _curry14(3, (params, positional, labeled) => match13(params).with((_v) => {
  const _g = _v;
  return _g.length === 0;
...
var absorbParams = _curry14(4, (params, acc, fills, labN) => (([positional, labeled]) => {
  const acc1 = _Array_concat5(acc, positional);
  return match13(labeled).with((_v) => {
    const _g = _v;
    return _g.length === 0;
  }, () => _tuple7(acc1, fills, labN)).otherwise(() => ((labVar) => _tuple7(_Array_append10(LPName(labVar, None13), acc1), _Array_append10({ labVar, labs: labeled }, fills), labN + 1))(eq13(labN, 0) ? "$lab" : `$lab${show6(labN)}`));
})(splitLamParams2(params, [], [])));
var collapseLambdaFrom = _curry14(5, (params, body, acc, fills, labN) => (([acc1, fills1, labN1]) => match13(body).with({ _tag: "ELambda" }, ({ params: params2, body: body2 }) => collapseLambdaFrom(params2, body2, acc1, fills1, labN1)).otherwise(() => _tuple7(acc1, body, fills1)))(absorbParams(params, acc, fills, labN)));
var collapseLambda = _curry14(2, (params, body) => collapseLambdaFrom(params, body, [], [], 0));
var tsInfix = _curry14(3, (ctx, fn, args) => not8(ctx.preserveInfix) ? None13 : match13(_tuple7(fn, args)).with((_v) => {
  const _g = _v;
  return _g[0]._tag === "ERef" && _g[1].length === 2;
}, ([{ name }, [left, right]]) => match13(name).with("add", () => Some13(`(${genExpr(ctx, left)} + ${genExpr(ctx, right)})`)).with("sub", () => Some13(`(${genExpr(ctx, left)} - ${genExpr(ctx, right)})`)).with("mul", () => Some13(`(${genExpr(ctx, left)} * ${genExpr(ctx, right)})`)).with("div", () => Some13(`(${genExpr(ctx, left)} / ${genExpr(ctx, right)})`)).with("lt", () => Some13(`(${genExpr(ctx, left)} < ${genExpr(ctx, right)})`)).with("lte", () => Some13(`(${genExpr(ctx, left)} <= ${genExpr(ctx, right)})`)).with("gt", () => Some13(`(${genExpr(ctx, left)} > ${genExpr(ctx, right)})`)).with("gte", () => Some13(`(${genExpr(ctx, left)} >= ${genExpr(ctx, right)})`)).otherwise(() => None13)).otherwise(() => None13));
var jsxHasSpread = (children) => match13(children).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => false).with((_v) => {
  const _g = _v;
...
var jsxAttrs = _curry14(3, (ctx, fields, spread) => {
  const head = match13(spread).with({ _tag: "None" }, () => "").with({ _tag: "Some" }, ({ value }) => ` {...${genExpr(ctx, value)}}`).exhaustive();
  return `${head}${_Str_join5("", map9((f) => match13(f.value).with({ _tag: "EBool", value: true }, () => ` ${f.name}`).otherwise((value) => ` ${f.name}={${genExpr(ctx, value)}}`), fields))}`;
});
var tsxArgs = _curry14(2, (ctx, args) => match13(args).with((_v) => {
  const _g = _v;
  return _g.length === 3 && _g[1]._tag === "ERecord" && _g[2]._tag === "EArr";
}, ([tag, { fields, spread }, { elements: children }]) => jsxHasSpread(children) ? None13 : ((fragment) => ((name) => ((attrs) => and9(eq13(length10(children), 0), not8(fragment)) ? Some13(`<${name}${attrs} />`) : ((body) => fragment ? Some13(`<>${body}</>`) : Some13(`<${name}${attrs}>${body}</${name}>`))(_Str_join5("", map9((child) => match13(child).with({ _tag: "SEExpr" }, ({ expr: value }) => `{${genExpr(ctx, value)}}`).with({ _tag: "SESpread" }, () => "").exhaustive(), children))))(jsxAttrs(ctx, fields, spread)))(fragment ? "" : match13(tag).with({ _tag: "EStr" }, ({ value }) => value).otherwise(() => genMember(ctx, tag))))(match13(tag).with({ _tag: "EStr", value: "Fragment" }, () => true).otherwise(() => false))).otherwise(() => None13));
var tsxCall = _curry14(4, (ctx, fn, args, origin) => not8(ctx.preserveJsx) ? None13 : match13(origin).with({ _tag: "Some", value: "jsx" }, () => match13(fn).with({ _tag: "ERef", name: "h" }, () => tsxArgs(ctx, args)).otherwise(() => None13)).otherwise(() => None13));
var genExpr = _curry14(2, (ctx, e) => match13(e).with({ _tag: "ENum" }, ({ raw }) => raw).with({ _tag: "EUnit" }, () => "undefined").with({ _tag: "EBool" }, ({ value }) => value ? "true" : "false").with({ _tag: "EStr" }, ({ value }) => jsStringLit(value)).with({ _tag: "ERef" }, ({ name }) => castOr(name, isNullaryCtor(name, ctx.keys) ? hook1(ctx.annotateEmpty, e) : None13)).with({ _tag: "ECall" }, ({ fn, args, origin }) => match13(tsxCall(ctx, fn, args, origin)).with({ _tag: "Some" }, ({ value: jsx }) => jsx).with({ _tag: "None" }, () => match13(tsInfix(ctx, fn, args)).with({ _tag: "Some" }, ({ value: infix }) => infix).with({ _tag: "None" }, () => ((inner) => castOr(inner, isCtorRef(fn) ? hook1(ctx.annotateCall, e) : None13))(`${genCallee(ctx, fn)}(${_Str_join5(", ", map9((a) => genExpr(ctx, a), args))})`)).exhaustive()).exhaustive()).with({ _tag: "ELambda" }, ({ params, body, span: sp }) => (([cparams, cbody, fills]) => {
...
})(collapseLambda(params, body))).with({ _tag: "ELetIn" }, ({ name, value, body }) => ((param) => `((${param}) => ${genLambdaBody(ctx, body)})(${genExpr(ctx, value)})`)(suffixOr(name, hook1(ctx.annotateLetin, value)))).with({ _tag: "ELetBind" }, ({ param, monad, value, body }) => ((rt) => ((f) => ((v) => ctx.flattenPipe ? `${rt}(${f}, ${v})` : `${rt}(${f})(${v})`)(genExpr(ctx, value)))(`(${genParam(param)}) => ${genLambdaBody(ctx, body)}`))(bindRuntime(monad))).with({ _tag: "EPipe" }, ({ left, right, fast, span: sp }) => fast ? match13(right).with({ _tag: "ECall" }, ({ fn: rfn, args: rargs, origin }) => genExpr(ctx, ECall(rfn, _Array_prepend7(left, rargs), origin, sp))).otherwise(() => genExpr(ctx, ECall(right, [left], None13, sp))) : match13(right).with((_v) => {
  const _g = _v;
  return _g._tag === "ECall" && (({ fn: rfn, args: rargs }) => ctx.flattenPipe)(_g);
}, ({ fn: rfn, args: rargs }) => `${genCallee(ctx, rfn)}(${_Str_join5(", ", map9((a) => genExpr(ctx, a), _Array_append10(left, rargs)))})`).otherwise(() => `${genCallee(ctx, right)}(${genExpr(ctx, left)})`)).with({ _tag: "EDo" }, ({ exprs }) => genDo(ctx, exprs)).with({ _tag: "ETernary" }, ({ cond, thenE, elseE }) => `(${genExpr(ctx, cond)} ? ${genExpr(ctx, thenE)} : ${genExpr(ctx, elseE)})`).with({ _tag: "EMatch" }, ({ scrutinee, arms }) => genMatch(ctx, scrutinee, arms)).with({ _tag: "ELoop" }, ({ params, body }) => `(() => { ${genLoopBlock(ctx, params, body)} })()`).with({ _tag: "ERecur" }, ({ args }) => `_recur(${_Str_join5(", ", map9((a) => genExpr(ctx, a), args))})`).with({ _tag: "ERecord" }, ({ fields, spread }) => ((fieldStrs) => match13(spread).with({ _tag: "None" }, () => eq13(length10(fields), 0) ? "{}" : `{ ${fieldStrs} }`).with({ _tag: "Some" }, ({ value: s }) => ((spreadStr) => eq13(length10(fields), 0) ? `{ ${spreadStr} }` : `{ ${spreadStr}, ${fieldStrs} }`)(`...${genExpr(ctx, s)}`)).exhaustive())(_Str_join5(", ", map9((f) => `${isJsIdent(f.name) ? f.name : jsStringLit(f.name)}: ${genExpr(ctx, f.value)}`, fields)))).with({ _tag: "EField" }, ({ target, name, optional }) => match13(emptyNsEmit(target, name, hook1(ctx.annotateEmpty, e))).with({ _tag: "Some" }, ({ value: js }) => js).with({ _tag: "None" }, () => match13(nsRuntimeId(ctx, target, name)).with({ _tag: "Some" }, ({ value: rt }) => rt).with({ _tag: "None" }, () => ((member) => optional ? `((v) => v != null ? { _tag: "Some", value: v } : { _tag: "None" })(${member})` : member)(`${genMember(ctx, target)}.${name}`)).exhaustive()).exhaustive()).with({ _tag: "ETuple" }, ({ elements }) => ((elems) => ctx.tupleHelper ? `_tuple(${elems})` : `[${elems}]`)(_Str_join5(", ", map9((el) => genExpr(ctx, el), elements)))).with({ _tag: "EArr" }, ({ elements }) => ((body) => castOr(body, eq13(length10(elements), 0) ? hook1(ctx.annotateEmpty, e) : None13))(`[${_Str_join5(", ", map9((el) => genSeqSlot(ctx, el), elements))}]`)).with({ _tag: "EList" }, ({ elements }) => genList(ctx, elements)).with({ _tag: "ESet" }, ({ elements }) => `new Set([${_Str_join5(", ", map9((el) => genSeqSlot(ctx, el), elements))}])`).with({ _tag: "EMap" }, ({ entries }) => match13(eq13(length10(entries), 0) ? hook1(ctx.annotateEmpty, e) : None13).with({ _tag: "Some" }, ({ value: t }) => `new ${t}()`).with({ _tag: "None" }, () => `new Map([${_Str_join5(", ", map9((en) => `[${genExpr(ctx, en.key)}, ${genExpr(ctx, en.value)}]`, entries))}])`).exhaustive()).with({ _tag: "EInterp" }, ({ parts }) => ((body) => `\`${body}\``)(_Str_join5("", map9((p) => match13(p).with({ _tag: "IPLit" }, ({ value }) => escapeTemplateLiteral(value)).with({ _tag: "IPExpr" }, ({ expr: ex }) => `\${${genExpr(ctx, ex)}}`).exhaustive(), parts)))).exhaustive());
var genDo = _curry14(2, (ctx, exprs) => `(() => { ${genDoSteps(ctx, exprs)} })()`);
var genDoSteps = _curry14(2, (ctx, exprs) => match13(exprs).with((_v) => {
  const _g = _v;
  return _g.length === 1;
...
var genSeqSlot = _curry14(2, (ctx, el) => match13(el).with({ _tag: "SEExpr" }, ({ expr: ex }) => genExpr(ctx, ex)).with({ _tag: "SESpread" }, ({ expr: ex }) => `...${genExpr(ctx, ex)}`).exhaustive());
var genList = _curry14(2, (ctx, elements) => {
  const yields = _Str_join5(" ", map9((el) => match13(el).with({ _tag: "SEExpr" }, ({ expr: ex }) => `yield (${genExpr(ctx, ex)});`).with({ _tag: "SESpread" }, ({ expr: ex }) => `yield* (${genExpr(ctx, ex)});`).exhaustive(), elements));
  return `_list(function* () {${eq13(yields, "") ? "" : ` ${yields} `}})`;
});
var genParam = (p) => match13(p).with({ _tag: "LPSpanned" }, ({ param: inner }) => genParam(inner)).with({ _tag: "LPName" }, ({ name }) => name).with({ _tag: "LPTuple" }, ({ names }) => `[${_Str_join5(", ", names)}]`).with({ _tag: "LPRecord" }, ({ fields }) => `{ ${_Str_join5(", ", fields)} }`).with({ _tag: "LPLabeled" }, ({ name }) => name).exhaustive();
var genCallee = _curry14(2, (ctx, e) => match13(e).with({ _tag: "ELambda" }, () => `(${genExpr(ctx, e)})`).otherwise(() => genExpr(ctx, e)));
var genMember = _curry14(2, (ctx, e) => match13(e).with({ _tag: "ERecord" }, () => `(${genExpr(ctx, e)})`).with({ _tag: "ELambda" }, () => `(${genExpr(ctx, e)})`).otherwise(() => genExpr(ctx, e)));
var seqElemExpr3 = (el) => match13(el).with({ _tag: "SEExpr" }, ({ expr: e }) => e).with({ _tag: "SESpread" }, ({ expr: e }) => e).exhaustive();
var hasRecur = (e) => match13(e).with({ _tag: "ERecur" }, () => true).with({ _tag: "ELoop" }, () => false).with({ _tag: "ELambda" }, () => false).with({ _tag: "ELetBind" }, () => false).with({ _tag: "EInterp" }, ({ parts }) => someOf3((p) => match13(p).with({ _tag: "IPExpr" }, ({ expr: x }) => hasRecur(x)).with({ _tag: "IPLit" }, () => false).exhaustive(), parts)).with({ _tag: "ECall" }, ({ fn, args }) => or7(hasRecur(fn), someOf3(hasRecur, args))).with({ _tag: "ELetIn" }, ({ value, body }) => or7(hasRecur(value), hasRecur(body))).with({ _tag: "EPipe" }, ({ left, right }) => or7(hasRecur(left), hasRecur(right))).with({ _tag: "EDo" }, ({ exprs }) => someOf3(hasRecur, exprs)).with({ _tag: "ETernary" }, ({ cond, thenE, elseE }) => or7(hasRecur(cond), or7(hasRecur(thenE), hasRecur(elseE)))).with({ _tag: "EMatch" }, ({ scrutinee, arms }) => or7(hasRecur(scrutinee), someOf3((a) => or7(match13(a.guard).with({ _tag: "Some" }, ({ value: g }) => hasRecur(g)).with({ _tag: "None" }, () => false).exhaustive(), hasRecur(a.body)), arms))).with({ _tag: "ERecord" }, ({ fields, spread }) => or7(match13(spread).with({ _tag: "Some" }, ({ value: sp }) => hasRecur(sp)).with({ _tag: "None" }, () => false).exhaustive(), someOf3((f) => hasRecur(f.value), fields))).with({ _tag: "EField" }, ({ target }) => hasRecur(target)).with({ _tag: "ETuple" }, ({ elements }) => someOf3(hasRecur, elements)).with({ _tag: "EArr" }, ({ elements }) => someOf3((el) => hasRecur(seqElemExpr3(el)), elements)).with({ _tag: "EList" }, ({ elements }) => someOf3((el) => hasRecur(seqElemExpr3(el)), elements)).with({ _tag: "ESet" }, ({ elements }) => someOf3((el) => hasRecur(seqElemExpr3(el)), elements)).with({ _tag: "EMap" }, ({ entries }) => someOf3((en) => or7(hasRecur(en.key), hasRecur(en.value)), entries)).otherwise(() => false);
var loopNeedsStep = (e) => match13(e).with({ _tag: "ETernary" }, ({ thenE, elseE }) => or7(loopNeedsStep(thenE), loopNeedsStep(elseE))).with({ _tag: "ELetIn" }, ({ body }) => loopNeedsStep(body)).with({ _tag: "EDo" }, ({ exprs }) => loopNeedsStep(lastDoExpr(exprs))).with({ _tag: "EMatch" }, () => hasRecur(e)).otherwise(() => false);
var lastDoExpr = (exprs) => match13(exprs).with((_v) => {
  const _g = _v;
  return _g.length === 1;
}, ([last]) => last).with((_v) => {
  const _g = _v;
...
var genDoLoopTail = _curry14(3, (ctx, exprs, params) => match13(exprs).with((_v) => {
  const _g = _v;
  return _g.length === 1;
...
var genLoopBlock = _curry14(3, (ctx, params, body) => {
...
var loopParamFree = _curry14(3, (params, i, seen) => match13(_Array_get10(i, params)).with({ _tag: "None" }, () => true).with({ _tag: "Some" }, ({ value: p }) => _Set_has5(p.name, seen) ? false : loopParamFree(params, i + 1, seen)).exhaustive());
var genLambdaBody = _curry14(2, (ctx, e) => match13(e).with({ _tag: "ERecord" }, () => `(${genExpr(ctx, e)})`).otherwise(() => genExpr(ctx, e)));
var paramNames = (p) => match13(p).with({ _tag: "LPSpanned" }, ({ param: inner }) => paramNames(inner)).with({ _tag: "LPName" }, ({ name }) => [name]).with({ _tag: "LPTuple" }, ({ names }) => names).with({ _tag: "LPRecord" }, ({ fields }) => fields).with({ _tag: "LPLabeled" }, ({ name }) => [name]).exhaustive();
var genLabeledFill = _curry14(3, (ctx, labVar, lab) => match13(lab).with({ _tag: "LPSpanned" }, ({ param: inner }) => genLabeledFill(ctx, labVar, inner)).with({ _tag: "LPLabeled" }, ({ name, optional, defaultValue }) => ((access) => match13(defaultValue).with({ _tag: "Some" }, ({ value: d }) => `const ${name} = ${access} != null ? ${access} : ${genExpr(ctx, d)};`).with({ _tag: "None" }, () => optional ? `const ${name} = ${access} != null ? { _tag: "Some", value: ${access} } : { _tag: "None" };` : `const ${name} = ${access};`).exhaustive())(`(${labVar} ?? {}).${name}`)).otherwise(() => ""));
var genFillDecls = _curry14(2, (ctx, fills) => match13(fills).with((_v) => {
  const _g = _v;
  return _g.length === 0;
}, () => "").otherwise(() => `${_Str_join5(" ", map9((g) => _Str_join5(" ", map9((lab) => genLabeledFill(ctx, g.labVar, lab), g.labs)), fills))} `));
var fillNames = _curry14(2, (fills, acc) => match13(fills).with((_v) => _v.length === 0, () => acc).with((_v) => _v.length >= 1, ([g, ...rest]) => fillNames(rest, reduce3(_curry14(2, (s, lab) => match13(lab).with({ _tag: "LPSpanned" }, ({ param: inner }) => match13(inner).with({ _tag: "LPLabeled" }, ({ name }) => _Set_add5(name, s)).otherwise(() => s)).with({ _tag: "LPLabeled" }, ({ name }) => _Set_add5(name, s)).otherwise(() => s)), acc, g.labs))).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
var addNames = _curry14(3, (names, i, acc) => match13(_Array_get10(i, names)).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, ({ value: n }) => addNames(names, i + 1, _Set_add5(n, acc))).exhaustive());
var paramNameSet = _curry14(3, (params, i, acc) => match13(_Array_get10(i, params)).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, ({ value: p }) => paramNameSet(params, i + 1, addNames(paramNames(p), 0, acc))).exhaustive());
var letBlockLoop = _curry14(4, (ctx, e, seen, decls) => match13(e).with({ _tag: "ELetIn" }, ({ name, value, body }) => or7(_Set_has5(name, seen), match13(value).with({ _tag: "ELambda" }, () => false).otherwise(() => _Set_has5(name, exprRefs(ctx, value, _Set_fromArray5([]))))) ? _tuple7(decls, e, seen) : letBlockLoop(ctx, body, _Set_add5(name, seen), _Array_append10(`const ${suffixOr(name, hook1(ctx.annotateLetin, value))} = ${genExpr(ctx, value)};`, decls))).otherwise(() => _tuple7(decls, e, seen)));
var genLambdaBodyIn = _curry14(4, (ctx, e, bound, prefix) => (([decls, rest, seen]) => eq13(length10(decls), 0) ? match13(e).with({ _tag: "ELoop" }, ({ params, body }) => loopParamFree(params, 0, bound) ? `{ ${prefix}${genLoopBlock(ctx, params, body)} }` : eq13(prefix, "") ? genLambdaBody(ctx, e) : `{ ${prefix}return ${genLambdaBody(ctx, e)}; }`).otherwise(() => eq13(prefix, "") ? genLambdaBody(ctx, e) : `{ ${prefix}return ${genLambdaBody(ctx, e)}; }`) : ((block) => match13(rest).with({ _tag: "ELoop" }, ({ params, body }) => loopParamFree(params, 0, seen) ? `{ ${prefix}${block} ${genLoopBlock(ctx, params, body)} }` : `{ ${prefix}${block} return ${genExpr(ctx, rest)}; }`).otherwise(() => `{ ${prefix}${block} return ${genExpr(ctx, rest)}; }`))(_Str_join5(" ", decls)))(letBlockLoop(ctx, e, bound, [])));
var isCatchAll2 = (p) => match13(p).with({ _tag: "PAs" }, ({ pat }) => isCatchAll2(pat)).with({ _tag: "PWild" }, () => true).with({ _tag: "PUnit" }, () => true).with({ _tag: "PBind" }, () => true).with({ _tag: "PRecord" }, ({ fields }) => allOf2((f) => isCatchAll2(f.pat), fields)).with({ _tag: "PTuple" }, ({ elems }) => allOf2(isCatchAll2, elems)).with({ _tag: "PArr" }, ({ elems, rest }) => and9(eq13(length10(elems), 0), _Option_isSome3(rest))).with({ _tag: "PList" }, ({ elems, rest }) => and9(eq13(length10(elems), 0), _Option_isSome3(rest))).otherwise(() => false);
var isPList2 = (p) => match13(p).with({ _tag: "PList" }, () => true).otherwise(() => false);
var keyedSlot = _curry14(2, (key, sub4) => eq13(sub4, key) ? key : `${key}: ${sub4}`);
var pctorEntries = _curry14(4, (ctx, ctor, args, i) => match13(_Array_get10(i, args)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: a }) => ((s) => ((restEntries) => eq13(s, "") ? restEntries : _Array_prepend7(keyedSlot(keyAt(ctx, ctor, i), s), restEntries))(pctorEntries(ctx, ctor, args, i + 1)))(patSlot(ctx, a))).exhaustive());
var precordEntries = _curry14(3, (ctx, fields, i) => match13(_Array_get10(i, fields)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: f }) => ((s) => ((restEntries) => eq13(s, "") ? restEntries : _Array_prepend7(keyedSlot(f.label, s), restEntries))(precordEntries(ctx, fields, i + 1)))(patSlot(ctx, f.pat))).exhaustive());
var patSlot = _curry14(2, (ctx, p) => match13(p).with({ _tag: "PAs" }, ({ pat, name }) => ((inner) => eq13(inner, "") ? name : `${inner}, ${name}`)(patSlot(ctx, pat))).with({ _tag: "PBind" }, ({ name }) => name).with({ _tag: "PWild" }, () => "").with({ _tag: "PUnit" }, () => "").with({ _tag: "PLit" }, () => "").with({ _tag: "PBool" }, () => "").with({ _tag: "PStr" }, () => "").with({ _tag: "PList" }, () => "").with({ _tag: "PCtor" }, ({ ctor, args }) => ((entries) => eq13(length10(entries), 0) ? "" : `{ ${_Str_join5(", ", entries)} }`)(pctorEntries(ctx, ctor, args, 0))).with({ _tag: "PRecord" }, ({ fields }) => ((entries) => eq13(length10(entries), 0) ? "" : `{ ${_Str_join5(", ", entries)} }`)(precordEntries(ctx, fields, 0))).with({ _tag: "PTuple" }, ({ elems }) => ((slots) => someOf3((s) => not8(eq13(s, "")), slots) ? `[${_Str_join5(", ", slots)}]` : "")(map9((el) => patSlot(ctx, el), elems))).with({ _tag: "PArr" }, ({ elems, rest }) => ((slots) => ((slots2) => someOf3((s) => not8(eq13(s, "")), slots2) ? `[${_Str_join5(", ", slots2)}]` : "")(match13(rest).with((_v) => {
  const _g = _v;
  return _g._tag === "Some" && _g.value._tag === "PBind";
}, ({ value: { name } }) => _Array_append10(`...${name}`, slots)).otherwise(() => slots)))(map9((el) => patSlot(ctx, el), elems))).with({ _tag: "POr" }, ({ alts }) => match13(_Array_head5(alts)).with({ _tag: "Some" }, ({ value: first }) => patSlot(ctx, first)).with({ _tag: "None" }, () => "").exhaustive()).exhaustive());
var pctorConds = _curry14(5, (ctx, ctor, args, i, path) => match13(_Array_get10(i, args)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: a }) => _Array_concat5(patConds(ctx, a, `${path}.${keyAt(ctx, ctor, i)}`), pctorConds(ctx, ctor, args, i + 1, path))).exhaustive());
var precordConds = _curry14(4, (ctx, fields, i, path) => match13(_Array_get10(i, fields)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: f }) => _Array_concat5(patConds(ctx, f.pat, `${path}.${f.label}`), precordConds(ctx, fields, i + 1, path))).exhaustive());
var ptupleConds = _curry14(4, (ctx, elems, i, path) => match13(_Array_get10(i, elems)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: el }) => _Array_concat5(patConds(ctx, el, `${path}[${show6(i)}]`), ptupleConds(ctx, elems, i + 1, path))).exhaustive());
var parrConds = _curry14(4, (ctx, elems, i, path) => match13(_Array_get10(i, elems)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: el }) => _Array_concat5(patConds(ctx, el, `${path}[${show6(i)}]`), parrConds(ctx, elems, i + 1, path))).exhaustive());
var patConds = _curry14(3, (ctx, p, path) => match13(p).with({ _tag: "PAs" }, ({ pat }) => patConds(ctx, pat, path)).with({ _tag: "PWild" }, () => []).with({ _tag: "PUnit" }, () => []).with({ _tag: "PBind" }, () => []).with({ _tag: "PList" }, () => []).with({ _tag: "PLit" }, () => [`${path} === ${litValue(p)}`]).with({ _tag: "PBool" }, () => [`${path} === ${litValue(p)}`]).with({ _tag: "PStr" }, () => [`${path} === ${litValue(p)}`]).with({ _tag: "PCtor" }, ({ ctor, args }) => _Array_prepend7(`${path}._tag === ${jsStringLit(ctor)}`, pctorConds(ctx, ctor, args, 0, path))).with({ _tag: "PRecord" }, ({ fields }) => precordConds(ctx, fields, 0, path)).with({ _tag: "PTuple" }, ({ elems }) => ptupleConds(ctx, elems, 0, path)).with({ _tag: "PArr" }, ({ elems, rest }) => _Array_prepend7(`${path}.length ${_Option_isSome3(rest) ? ">=" : "==="} ${show6(length10(elems))}`, parrConds(ctx, elems, 0, path))).with({ _tag: "POr" }, ({ alts }) => ((altCond) => [_Str_join5(" || ", map9((alt) => `(${altCond(alt)})`, alts))])((alt) => {
  const conds = patConds(ctx, alt, path);
  return eq13(length10(conds), 0) ? "true" : _Str_join5(" && ", map9((c) => `(${c})`, conds));
})).exhaustive());
var catchAllParam = _curry14(2, (ctx, p) => match13(p).with({ _tag: "PArr" }, ({ rest }) => match13(rest).with((_v) => {
  const _g = _v;
  return _g._tag === "Some" && _g.value._tag === "PBind";
}, ({ value: { name } }) => `(${name})`).otherwise(() => "()")).with({ _tag: "PList" }, ({ rest }) => match13(rest).with((_v) => {
  const _g = _v;
  return _g._tag === "Some" && _g.value._tag === "PBind";
}, ({ value: { name } }) => `(${name})`).otherwise(() => "()")).otherwise(() => ((slot) => eq13(slot, "") ? "()" : `(${slot})`)(patSlot(ctx, p))));
...
var listArmGuards = _curry14(3, (ctx, elems, i) => match13(_Array_get10(i, elems)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: el }) => _Array_concat5(patConds(ctx, el, `_b[${show6(i)}]`), listArmGuards(ctx, elems, i + 1))).exhaustive());
var listArmBinds = _curry14(3, (ctx, elems, i) => match13(_Array_get10(i, elems)).with({ _tag: "None" }, () => _tuple7([], [])).with({ _tag: "Some" }, ({ value: el }) => (([restParams, restArgs]) => {
  const slot = patSlot(ctx, el);
  return eq13(slot, "") ? _tuple7(restParams, restArgs) : _tuple7(_Array_prepend7(slot, restParams), _Array_prepend7(`_b[${show6(i)}]`, restArgs));
})(listArmBinds(ctx, elems, i + 1))).exhaustive());
var genListArm = _curry14(3, (ctx, p, body) => match13(p).with({ _tag: "PList" }, ({ elems, rest }) => ((n) => ((guards) => ((head) => ((cond) => (([params0, args0]) => (([params, args]) => `  if (${cond}) return ((${_Str_join5(", ", params)}) => ${genLambdaBody(ctx, body)})(${_Str_join5(", ", args)});`)(match13(rest).with((_v) => {
  const _g = _v;
  return _g._tag === "Some" && _g.value._tag === "PBind";
...
var listMatchLoop = _curry14(3, (ctx, arms, i) => match13(_Array_get10(i, arms)).with({ _tag: "None" }, () => _tuple7([], '(() => { throw new Error("non-exhaustive lazy-list switch"); })()')).with({ _tag: "Some" }, ({ value: a }) => and9(isPList2(a.pattern), not8(isCatchAll2(a.pattern))) ? (([restLines, fallback]) => _tuple7(_Array_prepend7(genListArm(ctx, a.pattern, a.body), restLines), fallback))(listMatchLoop(ctx, arms, i + 1)) : isCatchAll2(a.pattern) ? ((restName) => ((fallback) => _tuple7([], fallback))(match13(restName).with({ _tag: "Some" }, ({ value: name }) => `((${name}) => ${genLambdaBody(ctx, a.body)})(${listTail(0)})`).with({ _tag: "None" }, () => genExpr(ctx, a.body)).exhaustive()))(match13(a.pattern).with((_v) => {
  const _g = _v;
  return _g._tag === "PList" && _g.rest._tag === "Some" && _g.rest.value._tag === "PBind";
}, ({ rest: { value: { name } } }) => Some13(name)).otherwise(() => None13)) : listMatchLoop(ctx, arms, i + 1)).exhaustive());
var genListMatch = _curry14(3, (ctx, scrutinee, arms) => (([armLines, fallback]) => concat(concat(concat(concat(concat(concat(concat(concat("((_it) => { const _b = []; let _done = false; ", "const _pull = (_n) => { while (_b.length < _n && !_done) { const _s = _it.next(); "), `if (_s.done) _done = true; else _b.push(_s.value); } return _b.length >= _n; };
`), _Str_join5(`
`, armLines)), `
  return `), fallback), `;
})(`), genExpr(ctx, scrutinee)), "[Symbol.iterator]())"))(listMatchLoop(ctx, arms, 0)));
var matchArmsLoop = _curry14(4, (ctx, arms, i, base) => match13(_Array_get10(i, arms)).with({ _tag: "None" }, () => _tuple7([], None13)).with({ _tag: "Some" }, ({ value: a }) => (([restLines, restCatch]) => match13(a.guard).with({ _tag: "Some" }, ({ value: g }) => _tuple7(_Array_prepend7(`  ${genGuardArm(ctx, a.pattern, a.body, Some13(g), base)}`, restLines), restCatch)).with({ _tag: "None" }, () => isCatchAll2(a.pattern) ? _tuple7(restLines, Some13(_tuple7(a.pattern, a.body))) : _tuple7(_Array_prepend7(`  ${genWithArm(ctx, a.pattern, a.body, base)}`, restLines), restCatch)).exhaustive())(matchArmsLoop(ctx, arms, i + 1, base))).exhaustive());
var hasArrArm = (arms) => someOf3((a) => match13(a.pattern).with({ _tag: "PArr" }, () => true).otherwise(() => false), arms);
var genMatch = _curry14(3, (ctx, scrutinee, arms) => isListMatch(arms) ? genListMatch(ctx, scrutinee, arms) : ((base) => (([armLines, catchAll]) => {
  const tail = match13(catchAll).with((_v) => {
    const _g = _v;
    return _g._tag === "Some";
...
var litValue = (p) => match13(p).with({ _tag: "PStr" }, ({ value: v }) => jsStringLit(v)).with({ _tag: "PLit" }, ({ raw }) => raw).with({ _tag: "PBool" }, ({ value: v }) => v ? "true" : "false").otherwise(() => "");
var fieldRefine = _curry14(3, (ctx, p, fieldBase) => match13(p).with({ _tag: "PCtor" }, () => Some13(patTarget(ctx, p, fieldBase))).with({ _tag: "PRecord" }, () => ((t) => eq13(t, fieldBase) ? None13 : Some13(t))(patTarget(ctx, p, fieldBase))).with({ _tag: "PTuple" }, () => ((t) => eq13(t, fieldBase) ? None13 : Some13(t))(patTarget(ctx, p, fieldBase))).with({ _tag: "PArr" }, () => ((t) => eq13(t, fieldBase) ? None13 : Some13(t))(patTarget(ctx, p, fieldBase))).otherwise(() => None13));
var ctorRefines = _curry14(5, (ctx, args, keys, member, i) => match13(_Array_get10(i, args)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: a }) => ((rest) => ((key) => match13(fieldRefine(ctx, a, `${member}[${jsStringLit(key)}]`)).with({ _tag: "Some" }, ({ value: sub4 }) => _Array_prepend7(`${jsStringLit(key)}: ${sub4}`, rest)).with({ _tag: "None" }, () => rest).exhaustive())(_Option_unwrapOr7(`_${show6(i)}`, _Array_get10(i, keys))))(ctorRefines(ctx, args, keys, member, i + 1))).exhaustive());
var recordRefines = _curry14(4, (ctx, fields, base, i) => match13(_Array_get10(i, fields)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: f }) => ((rest) => match13(fieldRefine(ctx, f.pat, `${base}[${jsStringLit(f.label)}]`)).with({ _tag: "Some" }, ({ value: sub4 }) => _Array_prepend7(`${jsStringLit(f.label)}: ${sub4}`, rest)).with({ _tag: "None" }, () => rest).exhaustive())(recordRefines(ctx, fields, base, i + 1))).exhaustive());
var tupleSlotBase = _curry14(2, (base, i) => `(${base})[${show6(i)}]`);
var tupleTargets = _curry14(4, (ctx, elems, base, i) => match13(_Array_get10(i, elems)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: el }) => ((slotBase) => _Array_prepend7(_Option_unwrapOr7(slotBase, fieldRefine(ctx, el, slotBase)), tupleTargets(ctx, elems, base, i + 1)))(tupleSlotBase(base, i))).exhaustive());
var tupleRefines = _curry14(4, (ctx, elems, base, i) => match13(_Array_get10(i, elems)).with({ _tag: "None" }, () => false).with({ _tag: "Some" }, ({ value: el }) => or7(_Option_isSome3(fieldRefine(ctx, el, tupleSlotBase(base, i))), tupleRefines(ctx, elems, base, i + 1))).exhaustive());
var arrTargets = _curry14(4, (ctx, elems, elemBase, i) => match13(_Array_get10(i, elems)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: el }) => _Array_prepend7(_Option_unwrapOr7(elemBase, fieldRefine(ctx, el, elemBase)), arrTargets(ctx, elems, elemBase, i + 1))).exhaustive());
var arrRefines = _curry14(4, (ctx, elems, elemBase, i) => match13(_Array_get10(i, elems)).with({ _tag: "None" }, () => false).with({ _tag: "Some" }, ({ value: el }) => or7(_Option_isSome3(fieldRefine(ctx, el, elemBase)), arrRefines(ctx, elems, elemBase, i + 1))).exhaustive());
var patTarget = _curry14(3, (ctx, p, base) => match13(p).with({ _tag: "PAs" }, ({ pat }) => patTarget(ctx, pat, base)).with({ _tag: "PCtor" }, ({ ctor, args }) => ((member) => ((keys) => ((refines) => eq13(length10(refines), 0) ? member : `${member} & { ${_Str_join5("; ", refines)} }`)(ctorRefines(ctx, args, keys, member, 0)))(_Option_unwrapOr7([], _Map_get7(ctor, ctx.keys))))(`Extract<${base}, { _tag: ${jsStringLit(ctor)} }>`)).with({ _tag: "PRecord" }, ({ fields }) => ((refines) => eq13(length10(refines), 0) ? base : `${base} & { ${_Str_join5("; ", refines)} }`)(recordRefines(ctx, fields, base, 0))).with({ _tag: "PTuple" }, ({ elems }) => not8(tupleRefines(ctx, elems, base, 0)) ? base : `[${_Str_join5(", ", tupleTargets(ctx, elems, base, 0))}]`).with({ _tag: "PArr" }, ({ elems, rest: restOpt }) => ((elemBase) => not8(arrRefines(ctx, elems, elemBase, 0)) ? base : ((heads) => match13(restOpt).with({ _tag: "Some" }, () => `[${heads}, ...${base}]`).with({ _tag: "None" }, () => `[${heads}]`).exhaustive())(_Str_join5(", ", arrTargets(ctx, elems, elemBase, 0))))(`(${base})[number]`)).otherwise(() => base));
var genGuardArm = _curry14(5, (ctx, p, body, guardOpt, base) => {
  const root = _Option_isSome3(base) ? "_g" : "_v";
  const conds0 = patConds(ctx, p, root);
  const slot = match13(p).with({ _tag: "PAs" }, ({ pat }) => patSlot(ctx, pat)).otherwise(() => patSlot(ctx, p));
  const conds = match13(guardOpt).with({ _tag: "Some" }, ({ value: g }) => match13(p).with({ _tag: "PAs" }, ({ name }) => _Array_append10(eq13(slot, "") ? `((${name}) => ${genExpr(ctx, g)})(${root})` : `((${name}) => ((${slot}) => ${genExpr(ctx, g)})(${name}))(${root})`, conds0)).otherwise(() => _Array_append10(eq13(slot, "") ? `(${genExpr(ctx, g)})` : `((${slot}) => ${genExpr(ctx, g)})(${root})`, conds0))).with({ _tag: "None" }, () => conds0).exhaustive();
  const test = eq13(length10(conds), 0) ? "true" : _Str_join5(" && ", conds);
  const handler = match13(p).with({ _tag: "PAs" }, ({ name }) => `(${name}) => ${eq13(slot, "") ? genLambdaBody(ctx, body) : `((${slot}) => ${genLambdaBody(ctx, body)})(${name})`}`).otherwise(() => `${eq13(slot, "") ? "()" : `(${slot})`} => ${genLambdaBody(ctx, body)}`);
  return match13(base).with({ _tag: "None" }, () => `.with((_v) => ${test}, ${handler})`).with({ _tag: "Some" }, ({ value: b }) => ((target) => eq13(target, b) ? `.with((_v) => { const _g: any = _v; return ${test}; }, ${handler})` : `.with((_v): _v is ${target} => { const _g: any = _v; return ${test}; }, ${handler})`)(patTarget(ctx, p, b))).exhaustive();
});
var isFlatSub = (p) => match13(p).with({ _tag: "PAs" }, () => false).with({ _tag: "PBind" }, () => true).with({ _tag: "PWild" }, () => true).with({ _tag: "PLit" }, () => true).with({ _tag: "PBool" }, () => true).with({ _tag: "PStr" }, () => true).otherwise(() => false);
var recordLits = _curry14(2, (fields, i) => match13(_Array_get10(i, fields)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: f }) => ((rest) => match13(f.pat).with({ _tag: "PLit" }, () => _Array_prepend7(`${f.label}: ${litValue(f.pat)}`, rest)).with({ _tag: "PBool" }, () => _Array_prepend7(`${f.label}: ${litValue(f.pat)}`, rest)).with({ _tag: "PStr" }, () => _Array_prepend7(`${f.label}: ${litValue(f.pat)}`, rest)).otherwise(() => rest))(recordLits(fields, i + 1))).exhaustive());
var ctorArgParts = _curry14(4, (ctx, ctor, args, i) => match13(_Array_get10(i, args)).with({ _tag: "None" }, () => _tuple7([], [])).with({ _tag: "Some" }, ({ value: a }) => (([restBinds, restLits]) => {
  const key = keyAt(ctx, ctor, i);
  return match13(a).with({ _tag: "PBind" }, ({ name }) => _tuple7(_Array_prepend7(keyedSlot(key, name), restBinds), restLits)).with({ _tag: "PLit" }, () => _tuple7(restBinds, _Array_prepend7(`${key}: ${litValue(a)}`, restLits))).with({ _tag: "PBool" }, () => _tuple7(restBinds, _Array_prepend7(`${key}: ${litValue(a)}`, restLits))).with({ _tag: "PStr" }, () => _tuple7(restBinds, _Array_prepend7(`${key}: ${litValue(a)}`, restLits))).otherwise(() => _tuple7(restBinds, restLits));
})(ctorArgParts(ctx, ctor, args, i + 1))).exhaustive());
var genWithArm = _curry14(4, (ctx, p, body, base) => match13(p).with({ _tag: "PAs" }, () => genGuardArm(ctx, p, body, None13, base)).with({ _tag: "PArr" }, () => genGuardArm(ctx, p, body, None13, base)).with({ _tag: "PTuple" }, () => genGuardArm(ctx, p, body, None13, base)).with({ _tag: "POr" }, () => genGuardArm(ctx, p, body, None13, base)).with({ _tag: "PLit" }, () => `.with(${litValue(p)}, () => ${genLambdaBody(ctx, body)})`).with({ _tag: "PBool" }, () => `.with(${litValue(p)}, () => ${genLambdaBody(ctx, body)})`).with({ _tag: "PStr" }, () => `.with(${litValue(p)}, () => ${genLambdaBody(ctx, body)})`).with({ _tag: "PRecord" }, ({ fields }) => allOf2((f) => isFlatSub(f.pat), fields) ? ((lits) => ((slot) => `.with({ ${_Str_join5(", ", lits)} }, ${eq13(slot, "") ? "()" : `(${slot})`} => ${genLambdaBody(ctx, body)})`)(patSlot(ctx, p)))(recordLits(fields, 0)) : genGuardArm(ctx, p, body, None13, base)).with({ _tag: "PCtor" }, ({ ctor, args }) => allOf2(isFlatSub, args) ? (([binds, litFields]) => {
  const patObj = _Str_join5(", ", _Array_prepend7(`_tag: ${jsStringLit(ctor)}`, litFields));
  const param = eq13(length10(binds), 0) ? "()" : `({ ${_Str_join5(", ", binds)} })`;
  return `.with({ ${patObj} }, ${param} => ${genLambdaBody(ctx, body)})`;
})(ctorArgParts(ctx, ctor, args, 0)) : genGuardArm(ctx, p, body, None13, base)).otherwise(() => genGuardArm(ctx, p, body, None13, base)));
var typedCtorParams = _curry14(3, (keys, paramTypes, i) => match13(_Array_get10(i, keys)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: k }) => _Array_prepend7(`${k}: ${_Option_unwrapOr7("unknown", _Array_get10(i, paramTypes))}`, typedCtorParams(keys, paramTypes, i + 1))).exhaustive());
var genCtor = _curry14(2, (c, ts) => {
  const tag = jsStringLit(c.name);
  return eq13(length10(c.fields), 0) ? match13(ts).with({ _tag: "Some" }, ({ value: t }) => `const ${c.name}: ${t.retMono} = { _tag: ${tag} };`).with({ _tag: "None" }, () => `const ${c.name} = { _tag: ${tag} };`).exhaustive() : ((keys) => ((params) => ((impl) => length10(c.fields) >= 2 ? ((curried) => match13(ts).with({ _tag: "Some" }, ({ value: t }) => `const ${c.name} = ${curried} as ${t.generics}(${_Str_join5(", ", typedCtorParams(keys, t.paramTypes, 0))}) => ${t.ret};`).with({ _tag: "None" }, () => `const ${c.name} = ${curried};`).exhaustive())(`_curry(${show6(length10(c.fields))}, ${impl})`) : match13(ts).with({ _tag: "Some" }, ({ value: t }) => `const ${c.name} = ${t.generics}(${_Str_join5(", ", typedCtorParams(keys, t.paramTypes, 0))}): ${t.ret} => ({ _tag: ${tag}, ${params} });`).with({ _tag: "None" }, () => `const ${c.name} = ${impl};`).exhaustive())(`(${params}) => ({ _tag: ${tag}, ${params} })`))(_Str_join5(", ", keys)))(keysOf(c.fields));
});
var genCtorsFrom = _curry14(6, (s, ctors, h, refs, exported, i) => match13(_Array_get10(i, ctors)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: c }) => ((rest) => or7(exported, _Set_has5(c.name, refs)) ? _Array_prepend7(genCtor(c, hook2(h, s, c)), rest) : rest)(genCtorsFrom(s, ctors, h, refs, exported, i + 1))).exhaustive());
var genType = _curry14(2, (ctx, s) => match13(s).with({ _tag: "SType" }, ({ ctors, exported }) => _Str_join5(`
`, genCtorsFrom(s, ctors, ctx.annotateCtor, ctx.valueRefs, exported, 0))).otherwise(() => ""));
var typeExprArity = (te) => match13(te).with({ _tag: "TyArrow" }, ({ to }) => 1 + typeExprArity(to)).otherwise(() => 0);
var externArgs = (n) => {
  let i = 0;
  let acc = "";
  while (true) {
    if (i >= n) {
      ...
      [i, acc] = [i + 1, eq13(acc, "") ? `$a${show6(i)}` : `${acc}, $a${show6(i)}`];
      continue;
    }
  }
};
...
var genExtern = (s) => match13(s).with({ _tag: "SExtern" }, ({ name, typeExpr, module: modName, imported, curried }) => _Str_startsWith3("mochi:global:", modName) ? ((target) => ((base) => `const ${name} = ${eq13(imported, "") ? base : `${base}[${jsStringLit(imported)}]`};`)(`globalThis[${jsStringLit(target)}]`))(_Str_slice3(13, _Str_length3(modName), modName)) : _Str_startsWith3("mochi:get:", modName) ? ((target) => `const ${name} = ($receiver) => $receiver[${jsStringLit(target)}];`)(_Str_slice3(10, _Str_length3(modName), modName)) : _Str_startsWith3("mochi:set:", modName) ? ((target) => `const ${name} = _curry(2, ($receiver, $value) => ($receiver[${jsStringLit(target)}] = $value));`)(_Str_slice3(10, _Str_length3(modName), modName)) : _Str_startsWith3("mochi:new:", modName) ? ((target) => ((arity) => ((args) => not8(eq13(imported, "")) ? ((raw) => ((importLine) => ((ctor) => eq13(arity, 0) ? `${importLine}
const ${name} = () => ${ctor};` : `${importLine}
const ${name} = _curry(${show6(arity)}, (${args}) => ${ctor});`)(`new ${raw}(${args})`))(`import { ${imported} as ${raw} } from ${jsStringLit(target)};`))(_Str_concat2("$", name)) : eq13(arity, 0) ? `const ${name} = () => new globalThis[${jsStringLit(target)}]();` : `const ${name} = _curry(${show6(arity)}, (${args}) => new globalThis[${jsStringLit(target)}](${args}));`)(externArgs(arity)))(typeExprArity(typeExpr)))(_Str_slice3(10, _Str_length3(modName), modName)) : _Str_startsWith3("mochi:send:", modName) ? ((target) => ((arity) => ((args) => ((fn) => arity < 2 ? `const ${name} = ${fn};` : `const ${name} = _curry(${show6(arity)}, ${fn});`)(eq13(args, "") ? `($receiver) => $receiver[${jsStringLit(target)}]()` : `($receiver, ${args}) => $receiver[${jsStringLit(target)}](${args})`))(externArgs(arity - 1)))(typeExprArity(typeExpr)))(_Str_slice3(11, _Str_length3(modName), modName)) : eq13(imported, "default") ? `import ${name} from ${jsStringLit(modName)};` : ((arity) => arity <= 1 ? ((spec) => `import { ${spec} } from ${jsStringLit(modName)};`)(eq13(imported, name) ? name : `${imported} as ${name}`) : ((raw) => ((flat) => `import { ${imported} as ${raw} } from ${jsStringLit(modName)};
...
var rewriteImportPath = _curry14(2, (from, ext) => {
  const bare = stripAlExt(from);
  return or7(_Str_startsWith3("./", bare), _Str_startsWith3("../", bare)) ? `${bare}${ext}` : bare;
});
var genImport = _curry14(2, (s, ext) => match13(s).with({ _tag: "SImport" }, ({ names, from }) => ((nameList) => ((path) => `import { ${nameList} } from ${jsStringLit(path)};`)(rewriteImportPath(from, ext)))(_Str_join5(", ", map9((n) => n.name, names)))).with({ _tag: "SImportNs" }, ({ alias, from }) => ((path) => `import * as ${alias.name} from ${jsStringLit(path)};`)(rewriteImportPath(from, ext))).otherwise(() => ""));
var exportLine = (l) => `export ${l}`;
var jsDocLine = (l) => _Str_length3(l) > 0 ? ` * ${_Str_replace("*/", "*\\/", l)}` : " *";
var jsDoc = (docOpt) => match13(docOpt).with({ _tag: "None" }, () => "").with({ _tag: "Some" }, ({ value: doc }) => ((lines) => `/**
${_Str_join5(`
`, lines)}
 */
`)(map9(jsDocLine, _Str_split2(`
`, doc)))).exhaustive();
var genStmt = _curry14(2, (ctx, s) => match13(s).with({ _tag: "SError" }, ({ span: sp }) => `throw new Error("codegen invariant: error node reached codegen at ${show6(sp.start)}");`).with({ _tag: "SImport" }, () => genImport(s, ctx.moduleExt)).with({ _tag: "SImportNs" }, () => genImport(s, ctx.moduleExt)).with({ _tag: "SType" }, ({ exported }) => ((decls) => eq13(decls, "") ? "" : exported ? _Str_join5(`
...
var usesMatchLibArm = (a) => or7(match13(a.guard).with({ _tag: "Some" }, ({ value: g }) => usesMatchLib(g)).with({ _tag: "None" }, () => false).exhaustive(), usesMatchLib(a.body));
var usesMatchLib = (e) => match13(e).with({ _tag: "ENum" }, () => false).with({ _tag: "EUnit" }, () => false).with({ _tag: "EBool" }, () => false).with({ _tag: "EStr" }, () => false).with({ _tag: "ERef" }, () => false).with({ _tag: "ECall" }, ({ fn, args }) => or7(usesMatchLib(fn), someOf3(usesMatchLib, args))).with({ _tag: "ELambda" }, ({ body }) => usesMatchLib(body)).with({ _tag: "ELetIn" }, ({ value, body }) => or7(usesMatchLib(value), usesMatchLib(body))).with({ _tag: "ELetBind" }, ({ value, body }) => or7(usesMatchLib(value), usesMatchLib(body))).with({ _tag: "EPipe" }, ({ left, right }) => or7(usesMatchLib(left), usesMatchLib(right))).with({ _tag: "EDo" }, ({ exprs }) => someOf3(usesMatchLib, exprs)).with({ _tag: "ETernary" }, ({ cond, thenE, elseE }) => or7(usesMatchLib(cond), or7(usesMatchLib(thenE), usesMatchLib(elseE)))).with({ _tag: "EMatch" }, ({ scrutinee, arms }) => or7(not8(isListMatch(arms)), or7(usesMatchLib(scrutinee), someOf3(usesMatchLibArm, arms)))).with({ _tag: "ELoop" }, ({ params, body }) => or7(someOf3((p) => usesMatchLib(p.init), params), usesMatchLib(body))).with({ _tag: "ERecur" }, ({ args }) => someOf3(usesMatchLib, args)).with({ _tag: "ERecord" }, ({ fields, spread }) => or7(match13(spread).with({ _tag: "Some" }, ({ value: s }) => usesMatchLib(s)).with({ _tag: "None" }, () => false).exhaustive(), someOf3((f) => usesMatchLib(f.value), fields))).with({ _tag: "EField" }, ({ target }) => usesMatchLib(target)).with({ _tag: "ETuple" }, ({ elements }) => someOf3(usesMatchLib, elements)).with({ _tag: "EArr" }, ({ elements }) => someOf3((el) => usesMatchLib(match13(el).with({ _tag: "SEExpr" }, ({ expr: e2 }) => e2).with({ _tag: "SESpread" }, ({ expr: e2 }) => e2).exhaustive()), elements)).with({ _tag: "EList" }, ({ elements }) => someOf3((el) => usesMatchLib(match13(el).with({ _tag: "SEExpr" }, ({ expr: e2 }) => e2).with({ _tag: "SESpread" }, ({ expr: e2 }) => e2).exhaustive()), elements)).with({ _tag: "ESet" }, ({ elements }) => someOf3((el) => usesMatchLib(match13(el).with({ _tag: "SEExpr" }, ({ expr: e2 }) => e2).with({ _tag: "SESpread" }, ({ expr: e2 }) => e2).exhaustive()), elements)).with({ _tag: "EMap" }, ({ entries }) => someOf3((en) => or7(usesMatchLib(en.key), usesMatchLib(en.value)), entries)).with({ _tag: "EInterp" }, ({ parts }) => someOf3((p) => match13(p).with({ _tag: "IPLit" }, () => false).with({ _tag: "IPExpr" }, ({ expr: ex }) => usesMatchLib(ex)).exhaustive(), parts)).exhaustive();
var loopInitRefsFrom2 = _curry14(4, (ctx, params, i, acc) => match13(_Array_get10(i, params)).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, ({ value: p }) => loopInitRefsFrom2(ctx, params, i + 1, exprRefs(ctx, p.init, acc))).exhaustive());
var exprRefsListFrom = _curry14(4, (ctx, xs, i, acc) => match13(_Array_get10(i, xs)).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, ({ value: x }) => exprRefsListFrom(ctx, xs, i + 1, exprRefs(ctx, x, acc))).exhaustive());
var exprRefsInterpPartsFrom = _curry14(4, (ctx, parts, i, acc) => match13(_Array_get10(i, parts)).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, ({ value: p }) => exprRefsInterpPartsFrom(ctx, parts, i + 1, match13(p).with({ _tag: "IPLit" }, () => acc).with({ _tag: "IPExpr" }, ({ expr: ex }) => exprRefs(ctx, ex, acc)).exhaustive())).exhaustive());
var exprRefsArmsFrom = _curry14(4, (ctx, arms, i, acc) => match13(_Array_get10(i, arms)).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, ({ value: a }) => ((acc1) => exprRefsArmsFrom(ctx, arms, i + 1, exprRefs(ctx, a.body, acc1)))(match13(a.guard).with({ _tag: "Some" }, ({ value: g }) => exprRefs(ctx, g, acc)).with({ _tag: "None" }, () => acc).exhaustive())).exhaustive());
var exprRefsFieldsFrom = _curry14(4, (ctx, fields, i, acc) => match13(_Array_get10(i, fields)).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, ({ value: f }) => exprRefsFieldsFrom(ctx, fields, i + 1, exprRefs(ctx, f.value, acc))).exhaustive());
var exprRefsEntriesFrom = _curry14(4, (ctx, entries, i, acc) => match13(_Array_get10(i, entries)).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, ({ value: en }) => exprRefsEntriesFrom(ctx, entries, i + 1, exprRefs(ctx, en.value, exprRefs(ctx, en.key, acc)))).exhaustive());
var exprRefs = _curry14(3, (ctx, e, acc) => match13(e).with({ _tag: "ENum" }, () => acc).with({ _tag: "EUnit" }, () => acc).with({ _tag: "EBool" }, () => acc).with({ _tag: "EStr" }, () => acc).with({ _tag: "ERef" }, ({ name }) => _Set_add5(name, acc)).with({ _tag: "ECall" }, ({ fn, args }) => exprRefsListFrom(ctx, args, 0, exprRefs(ctx, fn, acc))).with({ _tag: "ELambda" }, ({ params, body }) => (([cparams, cbody, fills]) => {
  const acc2 = length10(cparams) >= 2 ? _Set_add5("_curry", acc) : acc;
  const acc3 = reduce3(_curry14(2, (a, g) => reduce3(_curry14(2, (b, lab) => match13(lab).with((_v) => {
    const _g = _v;
    return _g._tag === "LPSpanned" && _g.param._tag === "LPLabeled" && _g.param.defaultValue._tag === "Some";
...
})(collapseLambda(params, body))).with({ _tag: "ELetIn" }, ({ value, body }) => exprRefs(ctx, body, exprRefs(ctx, value, acc))).with({ _tag: "ELetBind" }, ({ monad, value, body }) => exprRefs(ctx, body, exprRefs(ctx, value, _Set_add5(bindRuntime(monad), acc)))).with({ _tag: "EPipe" }, ({ left, right }) => exprRefs(ctx, right, exprRefs(ctx, left, acc))).with({ _tag: "EDo" }, ({ exprs }) => exprRefsListFrom(ctx, exprs, 0, acc)).with({ _tag: "ETernary" }, ({ cond, thenE, elseE }) => exprRefs(ctx, elseE, exprRefs(ctx, thenE, exprRefs(ctx, cond, acc)))).with({ _tag: "EMatch" }, ({ scrutinee, arms }) => ((acc1) => ((acc2) => exprRefsArmsFrom(ctx, arms, 0, acc2))(someOf3((a) => match13(a.pattern).with((_v) => {
  const _g = _v;
  return _g._tag === "PList" && _g.rest._tag === "Some" && _g.rest.value._tag === "PBind";
}, () => true).otherwise(() => false), arms) ? _Set_add5("_list", acc1) : acc1))(exprRefs(ctx, scrutinee, acc))).with({ _tag: "ERecord" }, ({ fields, spread }) => exprRefsFieldsFrom(ctx, fields, 0, match13(spread).with({ _tag: "Some" }, ({ value: s }) => exprRefs(ctx, s, acc)).with({ _tag: "None" }, () => acc).exhaustive())).with({ _tag: "EField" }, ({ target, name }) => match13(emptyNsEmit(target, name, None13)).with({ _tag: "Some" }, () => match13(target).with({ _tag: "ERef", name: "List" }, () => _Set_add5("_list", acc)).otherwise(() => acc)).with({ _tag: "None" }, () => match13(nsRuntimeId(ctx, target, name)).with({ _tag: "Some" }, ({ value: rt }) => _Set_add5(rt, acc)).with({ _tag: "None" }, () => exprRefs(ctx, target, acc)).exhaustive()).exhaustive()).with({ _tag: "ELoop" }, ({ params, body }) => ((acc1) => ((acc2) => exprRefs(ctx, body, acc2))(loopInitRefsFrom2(ctx, params, 0, acc1)))(loopNeedsStep(body) ? _Set_add5("_recur", _Set_add5("_done", acc)) : acc)).with({ _tag: "ERecur" }, ({ args }) => exprRefsListFrom(ctx, args, 0, acc)).with({ _tag: "ETuple" }, ({ elements }) => exprRefsListFrom(ctx, elements, 0, acc)).with({ _tag: "EArr" }, ({ elements }) => exprRefsListFrom(ctx, map9((el) => match13(el).with({ _tag: "SEExpr" }, ({ expr: e2 }) => e2).with({ _tag: "SESpread" }, ({ expr: e2 }) => e2).exhaustive(), elements), 0, acc)).with({ _tag: "EList" }, ({ elements }) => exprRefsListFrom(ctx, map9((el) => match13(el).with({ _tag: "SEExpr" }, ({ expr: e2 }) => e2).with({ _tag: "SESpread" }, ({ expr: e2 }) => e2).exhaustive(), elements), 0, _Set_add5("_list", acc))).with({ _tag: "ESet" }, ({ elements }) => exprRefsListFrom(ctx, map9((el) => match13(el).with({ _tag: "SEExpr" }, ({ expr: e2 }) => e2).with({ _tag: "SESpread" }, ({ expr: e2 }) => e2).exhaustive(), elements), 0, acc)).with({ _tag: "EMap" }, ({ entries }) => exprRefsEntriesFrom(ctx, entries, 0, acc)).with({ _tag: "EInterp" }, ({ parts }) => exprRefsInterpPartsFrom(ctx, parts, 0, acc)).exhaustive());
var boundNamesFrom = _curry14(3, (stmts, i, acc) => match13(_Array_get10(i, stmts)).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, ({ value: s }) => boundNamesFrom(stmts, i + 1, match13(s).with({ _tag: "SLet" }, ({ name }) => _Set_add5(name, acc)).with({ _tag: "SExtern" }, ({ name }) => _Set_add5(name, acc)).with({ _tag: "SType" }, ({ ctors }) => _Set_union(acc, _Set_fromArray5(map9((c) => c.name, ctors)))).with({ _tag: "SImport" }, ({ names }) => _Set_union(acc, _Set_fromArray5(map9((n) => n.name, names)))).with({ _tag: "SImportNs" }, ({ alias }) => _Set_add5(alias.name, acc)).with({ _tag: "SError" }, () => acc).with({ _tag: "SExpr" }, () => acc).exhaustive())).exhaustive());
var boundNames = (stmts) => boundNamesFrom(stmts, 0, _Set_fromArray5([]));
var collectValueRefs = _curry14(4, (ctx, stmts, i, acc) => match13(_Array_get10(i, stmts)).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, ({ value: s }) => collectValueRefs(ctx, stmts, i + 1, match13(s).with({ _tag: "SLet" }, ({ value }) => exprRefs(ctx, value, acc)).with({ _tag: "SExpr" }, ({ value }) => exprRefs(ctx, value, acc)).otherwise(() => acc))).exhaustive());
var refsForStmt = _curry14(2, (ctx, s) => match13(s).with({ _tag: "SLet" }, ({ value }) => exprRefs(ctx, value, _Set_fromArray5([]))).with({ _tag: "SExpr" }, ({ value }) => exprRefs(ctx, value, _Set_fromArray5([]))).with({ _tag: "SType" }, ({ ctors, exported }) => someOf3((c) => and9(length10(c.fields) >= 2, or7(exported, _Set_has5(c.name, ctx.valueRefs))), ctors) ? _Set_add5("_curry", _Set_fromArray5([])) : _Set_fromArray5([])).with({ _tag: "SExtern" }, ({ typeExpr }) => typeExprArity(typeExpr) >= 2 ? _Set_add5("_curry", _Set_fromArray5([])) : _Set_fromArray5([])).otherwise(() => _Set_fromArray5([])));
var collectRefsFrom = _curry14(4, (ctx, stmts, i, acc) => match13(_Array_get10(i, stmts)).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, ({ value: s }) => collectRefsFrom(ctx, stmts, i + 1, _Set_union(acc, refsForStmt(ctx, s)))).exhaustive());
var addDepsFrom = _curry14(4, (deps, j, refs, queue) => match13(_Array_get10(j, deps)).with({ _tag: "None" }, () => _tuple7(refs, queue)).with({ _tag: "Some" }, ({ value: d }) => _Set_has5(d, refs) ? addDepsFrom(deps, j + 1, refs, queue) : addDepsFrom(deps, j + 1, _Set_add5(d, refs), _Array_append10(d, queue))).exhaustive());
var closeRefsFrom = _curry14(4, (queue, i, refs, runtimeDeps) => match13(_Array_get10(i, queue)).with({ _tag: "None" }, () => refs).with({ _tag: "Some" }, ({ value: r }) => ((deps) => (([refs2, queue2]) => closeRefsFrom(queue2, i + 1, refs2, runtimeDeps))(addDepsFrom(deps, 0, refs, queue)))(_Option_unwrapOr7([], _Map_get7(r, runtimeDeps)))).exhaustive());
var runtimeRefNames = _curry14(4, (ctx, stmts, jsDefs, runtimeDeps) => {
...
var preludePreamble = _curry14(4, (ctx, stmts, jsDefs, runtimeDeps) => {
  const names = runtimeRefNames(ctx, stmts, jsDefs, runtimeDeps);
  const defs = map9((n) => _Map_getOr6("", n, jsDefs), names);
  return eq13(length10(defs), 0) ? "" : `${_Str_join5(`
`, defs)}

`;
});
var genStmtAllFrom = _curry14(3, (ctx, stmts, i) => match13(_Array_get10(i, stmts)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: s }) => _Array_prepend7(genStmt(ctx, s), genStmtAllFrom(ctx, stmts, i + 1))).exhaustive());
var codegenWith = _curry14(7, (stmts, imported, useRuntime, ns, jsDefs, runtimeDeps, opts) => {
  ...
  const needsMatch = someOf3((s) => match13(s).with({ _tag: "SLet" }, ({ value }) => usesMatchLib(value)).with({ _tag: "SExpr" }, ({ value }) => usesMatchLib(value)).otherwise(() => false), stmts);
...
var runtimeDepNames = _curry14(5, (stmts, imported, ns, jsDefs, runtimeDeps) => {
  const keys = seedBuiltinCtorKeys(stmts, ctorKeysFromStmts(stmts, imported));
  const ctx0 = { keys, ns, annotateLet: None13, annotateCtor: None13, annotateParams: None13, annotateEmpty: None13, annotateLetin: None13, annotateCall: None13, guardBaseType: None13, flattenPipe: false, tupleHelper: false, preserveInfix: false, preserveJsx: false, moduleExt: ".js", valueRefs: _Set_fromArray5([]), docs: false };
  const valueRefs = collectValueRefs(ctx0, stmts, 0, _Set_fromArray5([]));
  return runtimeRefNames({ ...ctx0, valueRefs }, stmts, jsDefs, runtimeDeps);
});
var codegen = _curry14(6, (stmts, imported, useRuntime, ns, jsDefs, runtimeDeps) => codegenWith(stmts, imported, useRuntime, ns, jsDefs, runtimeDeps, jsGenOpts));

import { None as None15, Some as Some15, _Array_append as _Array_append12, _Array_concat as _Array_concat7, _Array_contains as _Array_contains4, _Array_dedupeBy, _Array_drop as _Array_drop3, _Array_get as _Array_get12, _Array_prepend as _Array_prepend9, _Array_reverse as _Array_reverse2, _Array_sort as _Array_sort2, _Array_sortBy, _Array_take as _Array_take3, _Map_delete as _Map_delete2, _Map_get as _Map_get9, _Map_keys as _Map_keys6, _Map_set as _Map_set8, _Map_size as _Map_size2, _Map_values as _Map_values2, _Option_flatMap as _Option_flatMap3, _Option_isSome as _Option_isSome4, _Option_map as _Option_map3, _Option_unwrapOr as _Option_unwrapOr9, _Set_add as _Set_add6, _Set_fromArray as _Set_fromArray6, _Set_has as _Set_has6, _Str_contains, _Str_fromCode as _Str_fromCode3, _Str_join as _Str_join7, _Str_split as _Str_split3, _Str_startsWith as _Str_startsWith4, _curry as _curry16, _tuple as _tuple9, and as and10, concat as concat2, eq as eq15, filter as filter5, length as length12, map as map11, not as not10, or as or8, reduce as reduce4, show as show8 } from "@mochi/compiler/runtime";
import { match as match15 } from "@onrails/pattern";

import { None as None14, Some as Some14, _Array_append as _Array_append11, _Array_concat as _Array_concat6, _Array_get as _Array_get11, _Array_prepend as _Array_prepend8, _Array_sort, _Map_get as _Map_get8, _Map_set as _Map_set7, _Map_size, _Option_flatMap as _Option_flatMap2, _Option_map as _Option_map2, _Option_unwrapOr as _Option_unwrapOr8, _Str_fromCode as _Str_fromCode2, _Str_get as _Str_get3, _Str_join as _Str_join6, _curry as _curry15, _tuple as _tuple8, eq as eq14, length as length11, map as map10, not as not9, show as show7 } from "@mochi/compiler/runtime";
import { match as match14 } from "@onrails/pattern";
var tsEnv = _curry15(2, (vars, recs) => ({ vars, recs }));
var noVars = new Map;
var noRecs = new Map;
...
var genericNamesFrom = _curry15(3, (ids, i, names) => match14(_Array_get11(i, ids)).with({ _tag: "None" }, () => names).with({ _tag: "Some" }, ({ value: id }) => genericNamesFrom(ids, i + 1, _Map_set7(id, letterAt(i), names))).exhaustive());
var primitiveTs = (name) => match14(name).with("number", () => "number").with("int", () => "number").with("float", () => "number").with("string", () => "string").with("bool", () => "boolean").with("unit", () => "undefined").otherwise(() => name);
var namesOf = _curry15(2, (ts, env) => _Str_join6(", ", map10((t) => tsOfRaw(t, env), ts)));
var nominal = _curry15(3, (name, args, env) => eq14(length11(args), 0) ? primitiveTs(name) : `${name}<${namesOf(args, env)}>`);
var tsRowFields = _curry15(2, (row, env) => match14(row).with({ _tag: "RowEmpty" }, () => _tuple8([], None14)).with({ _tag: "RowVar" }, ({ id }) => _tuple8([], Some14(id))).with({ _tag: "RowExtend" }, ({ label, fieldType, optional, rest }) => (([fields, tail]) => _tuple8(_Array_prepend8(`${label}${optional ? "?" : ""}: ${tsOfRaw(fieldType, env)}`, fields), tail))(tsRowFields(rest, env))).exhaustive());
var shapeFieldsFrom = _curry15(2, (row, vars) => match14(row).with({ _tag: "RowEmpty" }, () => Some14([])).with({ _tag: "RowVar" }, () => None14).with({ _tag: "RowExtend" }, ({ label, fieldType, optional, rest }) => _Option_map2((fs) => _Array_prepend8(`${label}${optional ? "?" : ""}: ${tsOf(fieldType, plainEnv(vars))}`, fs), shapeFieldsFrom(rest, vars))).exhaustive());
var rowShapeKey = _curry15(2, (row, vars) => _Option_map2((fs) => _Str_join6("; ", _Array_sort(fs)), shapeFieldsFrom(row, vars)));
var aliasNameFor = _curry15(2, (row, env) => eq14(_Map_size(env.recs), 0) ? None14 : _Option_flatMap2((k) => _Map_get8(k, env.recs), rowShapeKey(row, env.vars)));
var tsRow = _curry15(2, (row, env) => match14(aliasNameFor(row, env)).with({ _tag: "Some" }, ({ value: alias }) => alias).with({ _tag: "None" }, () => (([fields, tail]) => {
  const body = eq14(length11(fields), 0) ? "{}" : `{ ${_Str_join6("; ", fields)} }`;
  return match14(tail).with({ _tag: "None" }, () => body).with({ _tag: "Some" }, ({ value: id }) => match14(_Map_get8(id, env.vars)).with({ _tag: "None" }, () => body).with({ _tag: "Some" }, ({ value: name }) => eq14(length11(fields), 0) ? name : `(${body} & ${name})`).exhaustive()).exhaustive();
})(tsRowFields(row, env))).exhaustive());
var tsArrow = _curry15(3, (fromT, toT, env) => isUnit(fromT) ? `() => ${tsOfRaw(toT, env)}` : tsArrowParams(fromT, toT, env, 0, []));
var tsArrowParams = _curry15(5, (fromT, toT, env, i, params) => {
  const params1 = _Array_append11(`${_Str_fromCode2(97 + i)}: ${tsOfRaw(fromT, env)}`, params);
  return match14(toT).with((_v) => {
    const _g = _v;
...
var tsOf = _curry15(2, (t, env) => tsOfRaw(widenLits(t), env));
var tsOfRaw = _curry15(2, (t, env) => match14(t).with({ _tag: "TyVar" }, ({ id }) => _Option_unwrapOr8("unknown", _Map_get8(id, env.vars))).with((_v) => {
  const _g = _v;
  return _g._tag === "TyCon" && _g.name === "Array" && _g.args.length === 1;
}, ({ args: [elem] }) => ((inner) => match14(elem).with({ _tag: "TyFn" }, () => `(${inner})[]`).with({ _tag: "TyOneOf" }, () => `(${inner})[]`).otherwise(() => `${inner}[]`))(tsOfRaw(elem, env))).with((_v) => {
  const _g = _v;
...
var paramVarsFrom = _curry16(2, (params, i) => match15(_Array_get12(i, params)).with({ _tag: "None" }, () => new Map).with({ _tag: "Some" }, ({ value: p }) => _Map_set8(p, tVar(i), paramVarsFrom(params, i + 1))).exhaustive());
var paramNamesFrom = _curry16(2, (params, i) => match15(_Array_get12(i, params)).with({ _tag: "None" }, () => new Map).with({ _tag: "Some" }, () => _Map_set8(i, letterAt(i), paramNamesFrom(params, i + 1))).exhaustive());
var genericHead = _curry16(3, (params, i, acc) => match15(_Array_get12(i, params)).with({ _tag: "None" }, () => eq15(length12(acc), 0) ? "" : `<${_Str_join7(", ", acc)}>`).with({ _tag: "Some" }, () => genericHead(params, i + 1, _Array_append12(letterAt(i), acc))).exhaustive());
var fieldTs = _curry16(4, (te, params, aliases, recs) => {
...
var ctorFieldsFrom = _curry16(6, (fields, keys, params, aliases, recs, i) => match15(_Array_get12(i, fields)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: fld }) => _Array_prepend9(`${_Option_unwrapOr9(`_${show8(i)}`, _Array_get12(i, keys))}: ${fieldTs(fld.fieldType, params, aliases, recs)}`, ctorFieldsFrom(fields, keys, params, aliases, recs, i + 1))).exhaustive());
var ctorVariant = _curry16(4, (c, params, aliases, recs) => {
  const fields = ctorFieldsFrom(c.fields, keysOf(c.fields), params, aliases, recs, 0);
  return eq15(length12(fields), 0) ? `{ _tag: "${c.name}" }` : `{ _tag: "${c.name}"; ${_Str_join7("; ", fields)} }`;
});
var ctorVariantsFrom = _curry16(5, (ctors, params, aliases, recs, i) => match15(_Array_get12(i, ctors)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: c }) => _Array_prepend9(`  | ${ctorVariant(c, params, aliases, recs)}`, ctorVariantsFrom(ctors, params, aliases, recs, i + 1))).exhaustive());
var typeDecl = _curry16(5, (name, params, ctors, aliases, recs) => {
  const head = `${name}${genericHead(params, 0, [])}`;
  return `export type ${head} =
${_Str_join7(`
`, ctorVariantsFrom(ctors, params, aliases, recs, 0))};`;
});
var aliasFieldsFrom2 = _curry16(5, (fields, params, aliases, recs, i) => match15(_Array_get12(i, fields)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: f }) => _Array_prepend9(`${f.name}${f.optional ? "?" : ""}: ${fieldTs(f.fieldType, params, aliases, recs)}`, aliasFieldsFrom2(fields, params, aliases, recs, i + 1))).exhaustive());
var recordAliasDecl = _curry16(5, (name, params, fields, aliases, recs) => {
  const head = `${name}${genericHead(params, 0, [])}`;
  const body = aliasFieldsFrom2(fields, params, aliases, recs, 0);
  return eq15(length12(body), 0) ? `export type ${head} = {};` : `export type ${head} = { ${_Str_join7("; ", body)} };`;
});
var aliasTsDecl = _curry16(5, (name, params, template, aliases, recs) => {
  const head = `${name}${genericHead(params, 0, [])}`;
...
var mergeInto = _curry16(4, (keys, src, acc, i) => match15(_Array_get12(i, keys)).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, ({ value: k }) => mergeInto(keys, src, match15(_Map_get9(k, src)).with({ _tag: "Some" }, ({ value: v }) => _Map_set8(k, v, acc)).with({ _tag: "None" }, () => acc).exhaustive(), i + 1)).exhaustive());
var unionNamesFrom = _curry16(3, (schemes, i, acc) => match15(_Array_get12(i, schemes)).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, ({ value: sc }) => ((names) => unionNamesFrom(schemes, i + 1, mergeInto(_Map_keys6(names), names, acc, 0)))(genericNames(sc))).exhaustive());
var unionGenericNames = (schemes) => unionNamesFrom(schemes, 0, new Map);
var allVarsIn = _curry16(2, (t, names) => match15(t).with({ _tag: "TyVar" }, ({ id }) => _Option_isSome4(_Map_get9(id, names))).with({ _tag: "TyCon" }, ({ args }) => allVarsInAll(args, names, 0)).with({ _tag: "TyFn" }, ({ from: fromT, to: toT }) => and10(allVarsIn(fromT, names), allVarsIn(toT, names))).with({ _tag: "TyRecord" }, ({ row }) => allVarsInRow(row, names)).with({ _tag: "TySingleton" }, () => true).with({ _tag: "TyOneOf" }, ({ members }) => allVarsInAll(members, names, 0)).exhaustive());
var allVarsInAll = _curry16(3, (ts, names, i) => match15(_Array_get12(i, ts)).with({ _tag: "None" }, () => true).with({ _tag: "Some" }, ({ value: t }) => and10(allVarsIn(t, names), allVarsInAll(ts, names, i + 1))).exhaustive());
var allVarsInRow = _curry16(2, (row, names) => match15(row).with({ _tag: "RowEmpty" }, () => true).with({ _tag: "RowVar" }, ({ id }) => _Option_isSome4(_Map_get9(id, names))).with({ _tag: "RowExtend" }, ({ fieldType, rest }) => and10(allVarsIn(fieldType, names), allVarsInRow(rest, names))).exhaustive());
var isConcrete2 = (t) => allVarsIn(t, new Map([]));
var emptyCollTs = _curry16(2, (t, env) => allVarsIn(t, env.vars) ? Some15(tsOf(t, env)) : None15);
var ctorCallTs = _curry16(2, (t, recs) => match15(t).with({ _tag: "TyCon" }, ({ args }) => or8(eq15(length12(args), 0), not10(isConcrete2(t))) ? None15 : Some15(tsOf(t, recsEnv(recs)))).otherwise(() => None15));
var guardParamTs = _curry16(2, (t, recs) => isConcrete2(t) ? Some15(tsOf(t, recsEnv(recs))) : None15);
var lambdaParamsFrom = _curry16(4, (t, arity, env, i) => i >= arity ? [] : match15(t).with({ _tag: "TyFn" }, ({ from: fromT, to: toT }) => _Array_prepend9(allVarsIn(fromT, env.vars) ? Some15(tsOf(fromT, env)) : isConcrete2(fromT) ? Some15(tsOf(fromT, recsEnv(env.recs))) : None15, lambdaParamsFrom(toT, arity, env, i + 1))).otherwise(() => _Array_prepend9(None15, lambdaParamsFrom(t, arity, env, i + 1))));
var lambdaParamTypesTs = _curry16(3, (lamType, arity, env) => lambdaParamsFrom(lamType, arity, env, 0));
var genericParamsFrom = _curry16(4, (t, arity, env, i) => i >= arity ? [] : match15(t).with({ _tag: "TyFn" }, ({ from: fromT, to: toT }) => _Array_prepend9(Some15(tsOf(fromT, env)), genericParamsFrom(toT, arity, env, i + 1))).otherwise(() => _Array_prepend9(None15, genericParamsFrom(t, arity, env, i + 1))));
var genericLambdaParams = _curry16(3, (sc, arity, recs) => {
  const names = genericNames(sc);
  const env = tsEnv(names, recs);
  return eq15(_Map_size2(names), 0) ? None15 : Some15({ generics: `<${_Str_join7(", ", _Map_values2(names))}>`, params: genericParamsFrom(sc.ty, arity, env, 0) });
});
var neverArgs = _curry16(3, (params, i, acc) => match15(_Array_get12(i, params)).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, () => neverArgs(params, i + 1, _Array_append12("never", acc))).exhaustive());
var ctorParamTypes = _curry16(5, (fields, params, aliases, recs, i) => match15(_Array_get12(i, fields)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: fld }) => _Array_prepend9(fieldTs(fld.fieldType, params, aliases, recs), ctorParamTypes(fields, params, aliases, recs, i + 1))).exhaustive());
var ctorFactoryTs = _curry16(5, (typeName, params, c, aliases, recs) => {
  const head = genericHead(params, 0, []);
  const monos = neverArgs(params, 0, []);
  return { generics: head, paramTypes: ctorParamTypes(c.fields, params, aliases, recs, 0), ret: `${typeName}${head}`, retMono: eq15(length12(monos), 0) ? typeName : `${typeName}<${_Str_join7(", ", monos)}>` };
});
var paramDeclName = _curry16(2, (p, i) => match15(p).with({ _tag: "LPSpanned" }, ({ param: inner }) => paramDeclName(inner, i)).with({ _tag: "LPName" }, ({ name }) => name).with({ _tag: "LPLabeled" }, () => "$lab").otherwise(() => `_${show8(i)}`));
var compositions = (n) => eq15(n, 0) ? [[]] : compositionsFrom(n, 1);
var compositionsFrom = _curry16(2, (n, k) => k > n ? [] : _Array_concat7(map11(_Array_prepend9(k), compositions(n - k)), compositionsFrom(n, k + 1)));
var sliceGroups = _curry16(4, (params, groups, i, at) => match15(_Array_get12(i, groups)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: g }) => _Array_prepend9(_Array_take3(g, _Array_drop3(at, params)), sliceGroups(params, groups, i + 1, at + g))).exhaustive());
var curriedTail = _curry16(3, (slices, i, acc) => i < 1 ? acc : curriedTail(slices, i - 1, `(${_Str_join7(", ", _Option_unwrapOr9([], _Array_get12(i, slices)))}) => ${acc}`));
var overloadSig = _curry16(4, (head, params, ret, groups) => {
...
var curriedOverloads = _curry16(3, (head, params, ret) => length12(params) <= 1 ? `${head}(${_Str_join7(", ", params)}) => ${ret}` : `{ ${_Str_join7(" ", map11(overloadSig(head, params, ret), _Array_sortBy((g) => 0 - length12(g), compositions(length12(params)))))} }`);
var curriedFnType = _curry16(2, (params, ret) => length12(params) <= 1 ? `(${_Str_join7(", ", params)}) => ${ret}` : `_Curry<[${_Str_join7(", ", params)}], ${ret}>`);
var flatParamsFrom = _curry16(5, (t, value, env, n, acc) => match15(value).with({ _tag: "ELambda" }, ({ params, body }) => eq15(length12(params), 0) ? ((next) => flatParamsFrom(next, body, env, n, acc))(match15(t).with((_v) => {
  const _g = _v;
...
var takeParams = _curry16(6, (t, params, env, i, n, acc) => match15(_Array_get12(i, params)).with({ _tag: "None" }, () => _tuple9(t, n, acc)).with({ _tag: "Some" }, ({ value: p }) => match15(t).with({ _tag: "TyFn" }, ({ from: fromT, to: toT }) => takeParams(toT, params, env, i + 1, n + 1, _Array_append12(`${paramDeclName(p, n)}: ${tsOf(fromT, env)}`, acc))).otherwise(() => _tuple9(t, n, acc))).exhaustive());
var declType = _curry16(3, (t, value, env) => match15(value).with({ _tag: "ELambda" }, ({ params, body }) => eq15(length12(params), 0) ? ((next) => `() => ${declType(next, body, env)}`)(match15(t).with((_v) => {
  const _g = _v;
  return _g._tag === "TyFn" && (({ from: fromT, to: toT }) => isUnit(fromT))(_g);
...
var bindingTsType = _curry16(3, (sc, value, recs) => {
  const names = genericNames(sc);
  const env = tsEnv(names, recs);
  const head = eq15(_Map_size2(names), 0) ? "" : `<${_Str_join7(", ", _Map_values2(names))}>`;
  return match15(value).with({ _tag: "ELambda" }, () => eq15(head, "") ? (([params, ret]) => curriedFnType(params, ret))(flatParamsFrom(sc.ty, value, env, 0, [])) : `${head}${declType(sc.ty, value, env)}`).otherwise(() => tsOf(sc.ty, recsEnv(recs)));
});
var spanKey = (sp) => `${show8(sp.start)}:${show8(sp.end)}`;
var typeAtFrom = _curry16(3, (types, i, acc) => match15(_Array_get12(i, types)).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, ({ value: r }) => typeAtFrom(types, i + 1, _Map_set8(spanKey(r.span), r.ty, acc))).exhaustive());
var typeAtTable = (types) => typeAtFrom(types, 0, new Map);
var consInTy = _curry16(2, (t, acc) => match15(t).with((_v) => {
  const _g = _v;
  return _g._tag === "TyCon" && _g.name === "Task" && _g.args.length === 2;
...
var consInAll = _curry16(3, (ts, acc, i) => match15(_Array_get12(i, ts)).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, ({ value: t }) => consInAll(ts, consInTy(t, acc), i + 1)).exhaustive());
var consInRow = _curry16(2, (row, acc) => match15(row).with({ _tag: "RowExtend" }, ({ fieldType, rest }) => consInRow(rest, consInTy(fieldType, acc))).otherwise(() => acc));
var declaredTypeNames = _curry16(3, (stmts, i, acc) => match15(_Array_get12(i, stmts)).with({ _tag: "None" }, () => acc).with((_v) => {
  const _g = _v;
  return _g._tag === "Some" && _g.value._tag === "SType";
}, ({ value: { name } }) => declaredTypeNames(stmts, i + 1, _Set_add6(name, acc))).with({ _tag: "Some" }, () => declaredTypeNames(stmts, i + 1, acc)).exhaustive());
var referencedCons = _curry16(4, (stmts, env, i, acc) => match15(_Array_get12(i, stmts)).with({ _tag: "None" }, () => acc).with((_v) => {
  const _g = _v;
  return _g._tag === "Some" && _g.value._tag === "SLet";
}, ({ value: { name } }) => referencedCons(stmts, env, i + 1, _Str_startsWith4("$", name) ? acc : match15(_Map_get9(name, env)).with({ _tag: "Some" }, ({ value: sc }) => consInTy(sc.ty, acc)).with({ _tag: "None" }, () => acc).exhaustive())).with({ _tag: "Some" }, () => referencedCons(stmts, env, i + 1, acc)).exhaustive());
var builtinTypeNamesFor = _curry16(4, (declared, wanted, body, i) => match15(_Array_get12(i, builtinTypeDecls)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: bt }) => ((rest) => and10(not10(_Set_has6(bt.name, declared)), or8(_Set_has6(bt.name, wanted), _Str_contains(bt.name, body))) ? _Array_prepend9(bt.name, rest) : rest)(builtinTypeNamesFor(declared, wanted, body, i + 1))).exhaustive());
var aliasRowOf = _curry16(3, (fields, aliases, i) => match15(_Array_get12(i, fields)).with({ _tag: "None" }, () => RowEmpty).with({ _tag: "Some" }, ({ value: f }) => (([t, _vars, _st]) => RowExtend(f.name, t, f.optional, aliasRowOf(fields, aliases, i + 1)))(typeExprToType(f.fieldType, new Map, mkSt(0), aliases, _Set_fromArray6([])))).exhaustive());
var aliasShapeKey = _curry16(2, (fields, aliases) => rowShapeKey(aliasRowOf(fields, aliases, 0), new Map));
var bareName = (name) => {
  const parts = _Str_split3(".", name);
  return _Option_unwrapOr9(name, _Array_get12(length12(parts) - 1, parts));
};
var indexAlias = _curry16(4, (key, name, aliases, acc) => match15(_Map_get9(key, aliases)).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, ({ value: info }) => match15(info.expr).with({ _tag: "Some" }, () => acc).with({ _tag: "None" }, () => or8(not10(eq15(length12(info.params), 0)), eq15(length12(info.fields), 0)) ? acc : match15(aliasShapeKey(info.fields, aliases)).with({ _tag: "Some" }, ({ value: k }) => _Map_set8(k, name, acc)).with({ _tag: "None" }, () => acc).exhaustive()).exhaustive()).exhaustive());
var recordAliasIndexFrom = _curry16(4, (keys, aliases, i, acc) => match15(_Array_get12(i, keys)).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, ({ value: key }) => recordAliasIndexFrom(keys, aliases, i + 1, indexAlias(key, bareName(key), aliases, acc))).exhaustive());
var recordAliasIndex = (aliases) => recordAliasIndexFrom(_Array_sort2(_Map_keys6(aliases)), aliases, 0, new Map);
var withoutOwnShape = _curry16(4, (fields, params, aliases, recs) => match15(_Array_get12(0, params)).with({ _tag: "Some" }, () => recs).with({ _tag: "None" }, () => match15(aliasShapeKey(fields, aliases)).with({ _tag: "Some" }, ({ value: k }) => _Map_delete2(k, recs)).with({ _tag: "None" }, () => recs).exhaustive()).exhaustive());
var typeHeaderFrom = _curry16(4, (stmts, aliases, recs, i) => match15(_Array_get12(i, stmts)).with({ _tag: "None" }, () => []).with((_v) => {
  const _g = _v;
  return _g._tag === "Some" && _g.value._tag === "SType";
}, ({ value: { name, params, ctors, alias, aliasType, doc } }) => ((rest) => ((docComment) => match15(alias).with({ _tag: "Some" }, ({ value: fields }) => _Array_prepend9(`${docComment}${recordAliasDecl(name, params, fields, aliases, withoutOwnShape(fields, params, aliases, recs))}`, rest)).with({ _tag: "None" }, () => match15(aliasType).with({ _tag: "Some" }, ({ value: te }) => _Array_prepend9(`${docComment}${aliasTsDecl(name, params, te, aliases, recs)}`, rest)).with({ _tag: "None" }, () => eq15(length12(ctors), 0) ? _Array_prepend9(`declare const ${name}: unique symbol;
...
var genericLambdasFrom = _curry16(4, (stmts, env, i, acc) => match15(_Array_get12(i, stmts)).with({ _tag: "None" }, () => acc).with((_v) => {
  const _g = _v;
  return _g._tag === "Some" && _g.value._tag === "SLet";
}, ({ value: { name, value } }) => genericLambdasFrom(stmts, env, i + 1, match15(value).with((_v) => {
  const _g = _v;
  return _g._tag === "ELambda" && (({ span: sp }) => not10(_Str_startsWith4("$", name)))(_g);
}, ({ span: sp }) => match15(_Map_get9(name, env)).with({ _tag: "Some" }, ({ value: sc }) => or8(length12(sc.vars) > 0, length12(sc.rvars) > 0) ? _Map_set8(spanKey(sp), sc, acc) : acc).with({ _tag: "None" }, () => acc).exhaustive()).otherwise(() => acc))).with({ _tag: "Some" }, () => genericLambdasFrom(stmts, env, i + 1, acc)).exhaustive());
var scopedSpans = (e) => match15(e).with({ _tag: "ELambda" }, ({ body, span: sp }) => _Array_prepend9(sp, scopedSpans(body))).with({ _tag: "ECall" }, ({ fn, args }) => _Array_concat7(scopedSpans(fn), scopedSpansAt(args, 0))).with({ _tag: "ELetIn" }, ({ value, body }) => _Array_concat7(scopedSpans(value), scopedSpans(body))).with({ _tag: "ELetBind" }, ({ value, body }) => _Array_concat7(scopedSpans(value), scopedSpans(body))).with({ _tag: "EPipe" }, ({ left, right }) => _Array_concat7(scopedSpans(left), scopedSpans(right))).with({ _tag: "EDo" }, ({ exprs }) => scopedSpansAt(exprs, 0)).with({ _tag: "ETernary" }, ({ cond, thenE, elseE }) => _Array_concat7(scopedSpans(cond), _Array_concat7(scopedSpans(thenE), scopedSpans(elseE)))).with({ _tag: "EMatch" }, ({ scrutinee, arms }) => _Array_concat7(scopedSpans(scrutinee), scopedSpansInArms(arms, 0))).with({ _tag: "ERecord" }, ({ fields, spread }) => _Array_concat7(scopedSpansInFields(fields, 0), match15(spread).with({ _tag: "Some" }, ({ value: s }) => scopedSpans(s)).with({ _tag: "None" }, () => []).exhaustive())).with({ _tag: "EField" }, ({ target, name, span: sp }) => ((rest) => and10(eq15(name, "empty"), isRefExpr(target)) ? _Array_prepend9(sp, rest) : rest)(scopedSpans(target))).with({ _tag: "ETuple" }, ({ elements }) => scopedSpansAt(elements, 0)).with({ _tag: "EArr" }, ({ elements, span: sp }) => scopedSpansInSeq(elements, sp)).with({ _tag: "EList" }, ({ elements, span: sp }) => scopedSpansInSeq(elements, sp)).with({ _tag: "ESet" }, ({ elements, span: sp }) => scopedSpansInSeq(elements, sp)).with({ _tag: "EMap" }, ({ entries, span: sp }) => ((inner) => eq15(length12(entries), 0) ? _Array_prepend9(sp, inner) : inner)(scopedSpansInEntries(entries, 0))).with({ _tag: "ELoop" }, ({ params, body }) => _Array_concat7(scopedSpansInLoop(params, 0), scopedSpans(body))).with({ _tag: "ERecur" }, ({ args }) => scopedSpansAt(args, 0)).with({ _tag: "EInterp" }, ({ parts }) => scopedSpansInParts(parts, 0)).otherwise(() => []);
var isRefExpr = (e) => match15(e).with({ _tag: "ERef" }, () => true).otherwise(() => false);
var scopedSpansAt = _curry16(2, (exprs, i) => match15(_Array_get12(i, exprs)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: e }) => _Array_concat7(scopedSpans(e), scopedSpansAt(exprs, i + 1))).exhaustive());
var scopedSpansInArms = _curry16(2, (arms, i) => match15(_Array_get12(i, arms)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: a }) => _Array_concat7(match15(a.guard).with({ _tag: "Some" }, ({ value: g }) => scopedSpans(g)).with({ _tag: "None" }, () => []).exhaustive(), _Array_concat7(scopedSpans(a.body), scopedSpansInArms(arms, i + 1)))).exhaustive());
var scopedSpansInFields = _curry16(2, (fields, i) => match15(_Array_get12(i, fields)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: f }) => _Array_concat7(scopedSpans(f.value), scopedSpansInFields(fields, i + 1))).exhaustive());
var scopedSpansInEntries = _curry16(2, (entries, i) => match15(_Array_get12(i, entries)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: en }) => _Array_concat7(scopedSpans(en.key), _Array_concat7(scopedSpans(en.value), scopedSpansInEntries(entries, i + 1)))).exhaustive());
var scopedSpansInElems = _curry16(2, (elements, i) => match15(_Array_get12(i, elements)).with({ _tag: "None" }, () => []).with((_v) => {
  const _g = _v;
...
var scopedSpansInSeq = _curry16(2, (elements, sp) => {
  const inner = scopedSpansInElems(elements, 0);
  return eq15(length12(elements), 0) ? _Array_prepend9(sp, inner) : inner;
});
var scopedSpansInLoop = _curry16(2, (params, i) => match15(_Array_get12(i, params)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: p }) => _Array_concat7(scopedSpans(p.init), scopedSpansInLoop(params, i + 1))).exhaustive());
var scopedSpansInParts = _curry16(2, (parts, i) => match15(_Array_get12(i, parts)).with({ _tag: "None" }, () => []).with((_v) => {
  const _g = _v;
...
var scopedNamesAt = _curry16(4, (spans, i, names, acc) => match15(_Array_get12(i, spans)).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, ({ value: sp }) => scopedNamesAt(spans, i + 1, names, _Map_set8(spanKey(sp), names, acc))).exhaustive());
var scopedNamesFrom = _curry16(4, (stmts, env, i, acc) => match15(_Array_get12(i, stmts)).with({ _tag: "None" }, () => acc).with((_v) => {
  const _g = _v;
  return _g._tag === "Some" && _g.value._tag === "SLet";
}, ({ value: { name, value } }) => scopedNamesFrom(stmts, env, i + 1, match15(value).with((_v) => {
  const _g = _v;
  return _g._tag === "ELambda" && not10(_Str_startsWith4("$", name));
}, () => match15(_Map_get9(name, env)).with({ _tag: "Some" }, ({ value: sc }) => or8(length12(sc.vars) > 0, length12(sc.rvars) > 0) ? scopedNamesAt(scopedSpans(value), 0, unionGenericNames([sc]), acc) : acc).with({ _tag: "None" }, () => acc).exhaustive()).otherwise(() => acc))).with({ _tag: "Some" }, () => scopedNamesFrom(stmts, env, i + 1, acc)).exhaustive());
var tsGenOpts = _curry16(5, (stmts, env, types, letParams, aliases) => {
  ...
  const envAt = (key) => match15(_Map_get9(key, scopedNames)).with({ _tag: "Some" }, ({ value: vars }) => tsEnv(vars, recs)).with({ _tag: "None" }, () => recsEnv(recs)).exhaustive();
  return { ...jsGenOpts, annotateLet: Some15(_curry16(2, (name, value) => _Str_startsWith4("$", name) ? None15 : match15(value).with({ _tag: "ELambda" }, () => match15(_Map_get9(name, env)).with({ _tag: "Some" }, ({ value: sc }) => Some15(`: ${bindingTsType(sc, value, recs)}`)).with({ _tag: "None" }, () => None15).exhaustive()).otherwise(() => _Option_map3((ts) => `: ${ts}`, _Option_flatMap3((t) => emptyCollTs(t, recsEnv(recs)), _Map_get9(spanKey(exprSpan3(value)), letParamAt)))))), annotateCtor: Some15(_curry16(2, (s, c) => match15(s).with({ _tag: "SType" }, ({ name, params }) => Some15(ctorFactoryTs(name, params, c, aliases, recs))).otherwise(() => None15))), annotateParams: Some15(_curry16(2, (sp, arity) => match15(_Map_get9(spanKey(sp), genericLams)).with({ _tag: "Some" }, ({ value: sc }) => _Option_unwrapOr9({ generics: "", params: [] }, genericLambdaParams(sc, arity, recs))).with({ _tag: "None" }, () => ({ generics: "", params: match15(_Map_get9(spanKey(sp), typeAt)).with({ _tag: "Some" }, ({ value: t }) => lambdaParamTypesTs(t, arity, envAt(spanKey(sp)))).with({ _tag: "None" }, () => []).exhaustive() })).exhaustive())), annotateEmpty: Some15((e) => {
    const key = spanKey(exprSpan3(e));
    return _Option_flatMap3((t) => emptyCollTs(t, envAt(key)), _Map_get9(key, typeAt));
  }), annotateLetin: Some15((value) => _Option_flatMap3((t) => emptyCollTs(t, recsEnv(recs)), _Map_get9(spanKey(exprSpan3(value)), letParamAt))), annotateCall: Some15((e) => _Option_flatMap3((t) => ctorCallTs(t, recs), typeOf(e))), guardBaseType: Some15((e) => _Option_flatMap3((t) => guardParamTs(t, recs), typeOf(e))), flattenPipe: true, tupleHelper: true, preserveInfix: true, preserveJsx: true, moduleExt: "" };
});
var anyOf = _curry16(2, (f, xs) => reduce4(_curry16(2, (acc, x) => or8(acc, f(x))), false, xs));
var hasJsxExpr = (e) => match15(e).with((_v) => {
  const _g = _v;
...
`, filter5((part) => not10(eq15(part, "")), [_Str_join7(`
`, header), _Str_join7(`
`, importLines), typeImportLine, runtimeLine, body]))}`, `
`))(eq15(length12(typeDeps), 0) ? "" : `import type { ${_Str_join7(", ", _Array_sort2(typeDeps))} } from "${runtimeImport}";`))(_Array_concat7(_Str_contains("_Curry<", `${_Str_join7(`
`, header)}
...
var lettersFor = _curry16(3, (ids, i, acc) => match15(_Array_get12(i, ids)).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, ({ value: id }) => lettersFor(ids, i + 1, _Map_set8(id, letterAt(i), acc))).exhaustive());
var anyFor = (ids) => reduce4(_curry16(2, (acc, id) => _Map_set8(id, "any", acc)), new Map, ids);
var genericHeadOf = _curry16(2, (ids, names) => eq15(length12(ids), 0) ? "" : `<${_Str_join7(", ", _Map_values2(names))}>`);
var arrowCount = (t) => match15(t).with({ _tag: "TyFn" }, ({ to: toT }) => 1 + arrowCount(toT)).otherwise(() => 0);
var hostParams = _curry16(4, (t, arity, names, i) => i >= arity ? [] : match15(t).with({ _tag: "TyFn" }, ({ from: fromT, to: toT }) => _Array_prepend9(`${_Str_fromCode3(97 + i)}: ${tsOf(fromT, plainEnv(names))}`, hostParams(toT, arity, names, i + 1))).otherwise(() => []));
var hostReturn = _curry16(3, (t, arity, i) => i >= arity ? t : match15(t).with({ _tag: "TyFn" }, ({ to: toT }) => hostReturn(toT, arity, i + 1)).otherwise(() => t));
var curriedHostType = _curry16(2, (t, arity) => {
  const ids = freeIdsIn(t, []);
  const names = lettersFor(ids, 0, new Map);
  return `${genericHeadOf(ids, names)}${reduce4(_curry16(2, (acc, p) => `(${p}) => ${acc}`), tsOf(hostReturn(t, arity, 0), plainEnv(names)), _Array_reverse2(hostParams(t, arity, names, 0)))}`;
});
var flatHostType = _curry16(2, (t, arity) => {
  const ids = freeIdsIn(t, []);
  const names = lettersFor(ids, 0, new Map);
  const head = genericHeadOf(ids, names);
  return eq15(arity, 0) ? `${head}${tsOf(t, plainEnv(names))}` : curriedOverloads(head, hostParams(t, arity, names, 0), tsOf(hostReturn(t, arity, 0), plainEnv(names)));
});
var externDecl = (e) => {
  const t = e.scheme.ty;
  const n = arrowCount(t);
  return and10(n >= 1, e.curried) ? `export declare const ${e.imported}: ${curriedHostType(t, n)};` : eq15(n, 0) ? `export declare const ${e.imported}: ${tsOf(t, plainEnv(anyFor(freeIdsIn(t, []))))};` : `export declare const ${e.imported}: ${flatHostType(t, n)};`;
};
var externModuleDts = _curry16(2, (externs, aliases) => {
  const wanted = reduce4(_curry16(2, (acc, e) => consInTy(e.scheme.ty, acc)), _Set_fromArray6([]), externs);
...
import { _Array_append as _Array_append13, _Array_concat as _Array_concat8, _Array_get as _Array_get13, _Array_prepend as _Array_prepend10, _Map_get as _Map_get10, _Map_set as _Map_set9, _curry as _curry17, _tuple as _tuple10 } from "@mochi/compiler/runtime";
import { match as match16 } from "@onrails/pattern";
var def = _curry17(2, (name, span) => ({ name, defStart: span.start, defEnd: span.end, start: span.start, end: span.end, role: "def" }));
var use = _curry17(3, (name, span, env) => match16(_Map_get10(name, env)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: binding }) => [{ name, defStart: binding.start, defEnd: binding.end, start: span.start, end: span.end, role: "use" }]).exhaustive());
var bind = _curry17(3, (name, span, env) => _Map_set9(name, { name, start: span.start, end: span.end }, env));
var bindSpannedNames = _curry17(4, (names, spans, env, i) => match16(_tuple10(_Array_get13(i, names), _Array_get13(i, spans))).with((_v) => {
  const _g = _v;
...
var bindParam2 = _curry17(2, (param, env) => match16(param).with((_v) => {
  const _g = _v;
...
var bindParams = _curry17(3, (params, env, i) => match16(_Array_get13(i, params)).with({ _tag: "None" }, () => ({ env, occurrences: [] })).with({ _tag: "Some" }, ({ value: param }) => ((head) => ((tail) => ({ env: tail.env, occurrences: _Array_concat8(head.occurrences, tail.occurrences) }))(bindParams(params, head.env, i + 1)))(bindParam2(param, env))).exhaustive());
var walkExprs = _curry17(3, (exprs, env, i) => match16(_Array_get13(i, exprs)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: expr }) => _Array_concat8(walkExpr(expr, env), walkExprs(exprs, env, i + 1))).exhaustive());
var walkSeqs = _curry17(3, (elems, env, i) => match16(_Array_get13(i, elems)).with({ _tag: "None" }, () => []).with((_v) => {
  const _g = _v;
  return _g._tag === "Some" && _g.value._tag === "SEExpr";
...
var walkPattern = _curry17(2, (pat, env) => match16(pat).with({ _tag: "PBind" }, ({ name, span }) => ({ env: bind(name, span, env), occurrences: [def(name, span)] })).with({ _tag: "PAs" }, ({ pat: inner, name, nameSpan }) => ((innerResult) => ({ env: bind(name, nameSpan, innerResult.env), occurrences: _Array_append13(def(name, nameSpan), innerResult.occurrences) }))(walkPattern(inner, env))).with({ _tag: "PTuple" }, ({ elems }) => walkPatterns(elems, env, 0)).with({ _tag: "PRecord" }, ({ fields }) => walkPatFields(fields, env, 0)).with({ _tag: "PCtor" }, ({ args }) => walkPatterns(args, env, 0)).with({ _tag: "PArr" }, ({ elems, rest }) => ((result) => match16(rest).with({ _tag: "None" }, () => result).with({ _tag: "Some" }, ({ value: tail }) => ((tailResult) => ({ env: tailResult.env, occurrences: _Array_concat8(result.occurrences, tailResult.occurrences) }))(walkPattern(tail, result.env))).exhaustive())(walkPatterns(elems, env, 0))).with({ _tag: "PList" }, ({ elems, rest }) => ((result) => match16(rest).with({ _tag: "None" }, () => result).with({ _tag: "Some" }, ({ value: tail }) => ((tailResult) => ({ env: tailResult.env, occurrences: _Array_concat8(result.occurrences, tailResult.occurrences) }))(walkPattern(tail, result.env))).exhaustive())(walkPatterns(elems, env, 0))).with((_v) => {
  const _g = _v;
  return _g._tag === "POr" && _g.alts.length >= 1;
}, ({ alts: [first] }) => walkPattern(first, env)).otherwise(() => ({ env, occurrences: [] })));
var walkPatterns = _curry17(3, (patterns, env, i) => match16(_Array_get13(i, patterns)).with({ _tag: "None" }, () => ({ env, occurrences: [] })).with({ _tag: "Some" }, ({ value: pat }) => ((head) => ((tail) => ({ env: tail.env, occurrences: _Array_concat8(head.occurrences, tail.occurrences) }))(walkPatterns(patterns, head.env, i + 1)))(walkPattern(pat, env))).exhaustive());
var walkPatFields = _curry17(3, (fields, env, i) => match16(_Array_get13(i, fields)).with({ _tag: "None" }, () => ({ env, occurrences: [] })).with({ _tag: "Some" }, ({ value: field }) => ((head) => ((tail) => ({ env: tail.env, occurrences: _Array_concat8(head.occurrences, tail.occurrences) }))(walkPatFields(fields, head.env, i + 1)))(walkPattern(field.pat, env))).exhaustive());
var walkArms = _curry17(3, (arms, env, i) => match16(_Array_get13(i, arms)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: arm }) => ((pat) => ((guard) => _Array_concat8(pat.occurrences, _Array_concat8(guard, _Array_concat8(walkExpr(arm.body, pat.env), walkArms(arms, env, i + 1)))))(match16(arm.guard).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: expr }) => walkExpr(expr, pat.env)).exhaustive()))(walkPattern(arm.pattern, env))).exhaustive());
var walkFields = _curry17(3, (fields, env, i) => match16(_Array_get13(i, fields)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: field }) => _Array_concat8(walkExpr(field.value, env), walkFields(fields, env, i + 1))).exhaustive());
var walkEntries = _curry17(3, (entries, env, i) => match16(_Array_get13(i, entries)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: entry }) => _Array_concat8(walkExpr(entry.key, env), _Array_concat8(walkExpr(entry.value, env), walkEntries(entries, env, i + 1)))).exhaustive());
var walkLoopParams = _curry17(3, (params, env, i) => match16(_Array_get13(i, params)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: param }) => _Array_concat8(walkExpr(param.init, env), walkLoopParams(params, env, i + 1))).exhaustive());
var loopEnv = _curry17(3, (params, env, i) => match16(_Array_get13(i, params)).with({ _tag: "None" }, () => env).with({ _tag: "Some" }, ({ value: param }) => loopEnv(params, bind(param.name, param.nameSpan, env), i + 1)).exhaustive());
var loopDefs = _curry17(2, (params, i) => match16(_Array_get13(i, params)).with({ _tag: "None" }, () => []).with({ _tag: "Some" }, ({ value: param }) => _Array_concat8([def(param.name, param.nameSpan)], loopDefs(params, i + 1))).exhaustive());
var walkExpr = _curry17(2, (expr, env) => match16(expr).with({ _tag: "ERef" }, ({ name, span }) => use(name, span, env)).with({ _tag: "ECall" }, ({ fn, args }) => _Array_concat8(walkExpr(fn, env), walkExprs(args, env, 0))).with({ _tag: "ELambda" }, ({ params, body }) => ((bound) => _Array_concat8(bound.occurrences, walkExpr(body, bound.env)))(bindParams(params, env, 0))).with({ _tag: "ELetIn" }, ({ name, nameSpan, value, body }) => ((recursive) => ((bodyEnv) => recursive ? _Array_concat8([def(name, nameSpan)], _Array_concat8(walkExpr(value, bodyEnv), walkExpr(body, bodyEnv))) : _Array_concat8(walkExpr(value, env), _Array_concat8([def(name, nameSpan)], walkExpr(body, bodyEnv))))(bind(name, nameSpan, env)))(match16(value).with({ _tag: "ELambda" }, () => true).otherwise(() => false))).with({ _tag: "ELetBind" }, ({ value, body }) => _Array_concat8(walkExpr(value, env), walkExpr(body, env))).with({ _tag: "EPipe" }, ({ left, right }) => _Array_concat8(walkExpr(left, env), walkExpr(right, env))).with({ _tag: "EDo" }, ({ exprs }) => walkExprs(exprs, env, 0)).with({ _tag: "ETernary" }, ({ cond, thenE, elseE }) => _Array_concat8(walkExpr(cond, env), _Array_concat8(walkExpr(thenE, env), walkExpr(elseE, env)))).with({ _tag: "EMatch" }, ({ scrutinee, arms }) => _Array_concat8(walkExpr(scrutinee, env), walkArms(arms, env, 0))).with({ _tag: "ERecord" }, ({ fields, spread }) => match16(spread).with({ _tag: "None" }, () => walkFields(fields, env, 0)).with({ _tag: "Some" }, ({ value: base }) => _Array_concat8(walkExpr(base, env), walkFields(fields, env, 0))).exhaustive()).with({ _tag: "EField" }, ({ target }) => walkExpr(target, env)).with({ _tag: "ETuple" }, ({ elements }) => walkExprs(elements, env, 0)).with({ _tag: "EArr" }, ({ elements }) => walkSeqs(elements, env, 0)).with({ _tag: "EList" }, ({ elements }) => walkSeqs(elements, env, 0)).with({ _tag: "ESet" }, ({ elements }) => walkSeqs(elements, env, 0)).with({ _tag: "EMap" }, ({ entries }) => walkEntries(entries, env, 0)).with({ _tag: "ELoop" }, ({ params, body }) => ((scoped) => _Array_concat8(walkLoopParams(params, env, 0), _Array_concat8(loopDefs(params, 0), walkExpr(body, scoped))))(loopEnv(params, env, 0))).with({ _tag: "ERecur" }, ({ args }) => walkExprs(args, env, 0)).with({ _tag: "EInterp" }, ({ parts }) => walkInterp(parts, env, 0)).otherwise(() => []));
var walkInterp = _curry17(3, (parts, env, i) => match16(_Array_get13(i, parts)).with({ _tag: "None" }, () => []).with((_v) => {
  const _g = _v;
...
var topEnv = _curry17(3, (stmts, env, i) => match16(_Array_get13(i, stmts)).with({ _tag: "None" }, () => env).with((_v) => {
  const _g = _v;
  return _g._tag === "Some" && _g.value._tag === "SLet";
...
var topDefs = _curry17(2, (stmts, i) => match16(_Array_get13(i, stmts)).with({ _tag: "None" }, () => []).with((_v) => {
  const _g = _v;
  return _g._tag === "Some" && _g.value._tag === "SLet";
...
var walkStmts = _curry17(3, (stmts, env, i) => match16(_Array_get13(i, stmts)).with({ _tag: "None" }, () => []).with((_v) => {
  const _g = _v;
  return _g._tag === "Some" && _g.value._tag === "SLet";
...
`;
  ...
  return eq16(length13(imports), 0) ? body : `${_Str_join8(`
`, imports)}
${body}`;
});
...
var resolveImport2 = _curry20(2, resolveImport);
