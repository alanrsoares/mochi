...
var depthStep = _curry18(3, (s, i, depth) => {
  ...
  return or9(or9(or9(eq16(ch, "<"), eq16(ch, "{")), eq16(ch, "(")), eq16(ch, "[")) ? depth + 1 : or9(or9(eq16(ch, "}"), eq16(ch, ")")), eq16(ch, "]")) ? depth - 1 : and12(eq16(ch, ">"), or9(eq16(i, 0), not10(eq16(_Str_slice4(i - 1, i, s), "=")))) ? depth - 1 : depth;
});
...
var headLetters = _curry18(2, (names, pins) => {
  const letters = map10((id) => _Option_unwrapOr9("", _Map_get8(id, names)), filter5((id) => not10(_Map_has5(id, pins)), _Map_keys7(names)));
  return eq16(length13(letters), 0) ? "" : `<${_Str_join6(", ", letters)}>`;
});
...
var allVarsInRow = _curry19(2, (row, names) => match18(row).with({ _tag: "RowEmpty" }, () => true).with({ _tag: "RowVar" }, ({ id }) => _Option_isSome4(_Map_get9(id, names))).with({ _tag: "RowExtend" }, ({ fieldType, rest }) => and13(allVarsIn(fieldType, names), allVarsInRow(rest, names))).exhaustive());
var isConcrete2 = (t) => allVarsIn(t, new Map([]));
var emptyCollTs = _curry19(2, (t, env) => allVarsIn(t, env.vars) ? Some18(tsOf(t, env)) : None18);
var ctorCallTs = _curry19(2, (t, recs) => match18(t).with({ _tag: "TyCon" }, ({ args }) => or10(eq17(length14(args), 0), not11(isConcrete2(t))) ? None18 : Some18(tsOf(t, recsEnv(recs)))).otherwise(() => None18));
...
var indexAlias = _curry19(4, (key, name, aliases, acc) => match18(_Map_get9(key, aliases)).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, ({ value: info }) => match18(info.expr).with({ _tag: "Some" }, () => acc).with({ _tag: "None" }, () => or10(not11(eq17(length14(info.params), 0)), eq17(length14(info.fields), 0)) ? acc : match18(aliasShapeKey(info.fields, aliases)).with({ _tag: "Some" }, ({ value: k }) => _Map_set8(k, name, acc)).with({ _tag: "None" }, () => acc).exhaustive()).exhaustive()).exhaustive());
...
var dropAmbiguous = _curry19(5, (keys, recs, bad, localNames, i) => match18(_Array_get15(i, keys)).with({ _tag: "None" }, () => recs).with({ _tag: "Some" }, ({ value: k }) => match18(_Map_get9(k, recs)).with({ _tag: "Some" }, ({ value: name }) => dropAmbiguous(keys, and13(_Set_has7(name, bad), not11(_Set_has7(name, localNames))) ? _Map_set8(k, "", recs) : recs, bad, localNames, i + 1)).with({ _tag: "None" }, () => dropAmbiguous(keys, recs, bad, localNames, i + 1)).exhaustive()).exhaustive());
var withoutAmbiguousAlias = _curry19(3, (recs, aliases, localNames) => dropAmbiguous(_Map_keys8(recs), recs, parameterizedBares(_Map_keys8(aliases), aliases, 0, _Set_fromArray8([])), localNames, 0));
...
var genericLambdasFrom = _curry19(4, (stmts, env, i, acc) => match18(_Array_get15(i, stmts)).with({ _tag: "None" }, () => acc).with((_v) => {
  ...
  const _g = _v;
  return _g._tag === "ELambda" && (({ span: sp }) => not11(_Str_startsWith6("$", name)))(_g);
...
var scopedNamesFrom = _curry19(5, (stmts, env, recs, i, acc) => match18(_Array_get15(i, stmts)).with({ _tag: "None" }, () => acc).with((_v) => {
  ...
  const _g = _v;
  return _g._tag === "ELambda" && not11(_Str_startsWith6("$", name));
...
`, _Array_concat7(map11((bt) => typeDecl(bt.name, bt.params, bt.ctors, aliases, new Map), filter6((bt) => _Set_has7(bt.name, wanted), builtinTypeDecls)), map11(externDecl, _Array_dedupeBy((e) => e.imported, externs)))), `
`);
});
...
import { _Array_concat as _Array_concat9, _Array_contains as _Array_contains5, _Array_get as _Array_get17, _Array_prepend as _Array_prepend11, _Map_get as _Map_get11, _Map_getOr as _Map_getOr7, _Map_has as _Map_has6, _Map_keys as _Map_keys9, _Map_set as _Map_set10, _Result_map as _Result_map8, _Set_add as _Set_add9, _Set_fromArray as _Set_fromArray9, _Set_has as _Set_has8, _Set_toArray as _Set_toArray4, _Str_contains as _Str_contains3, _Str_endsWith as _Str_endsWith3, _Str_join as _Str_join8, _Str_length as _Str_length7, _Str_slice as _Str_slice5, _Str_startsWith as _Str_startsWith8, _curry as _curry22, eq as eq18, length as length15, map as map13, not as not12, or as or12 } from "@mochi/compiler/runtime";
...
var openDirective = (src) => {
  ...
  return and14(_Str_startsWith7('"use open"', t), match20(afterBlanks(t, 10)).with({ _tag: "None" }, () => true).with({ _tag: "Some", value: `
` }, () => true).with({ _tag: "Some", value: "r" }, () => true).otherwise(() => false));
};
var openMode = _curry21(2, (src, requested) => or11(requested, openDirective(src)));
var noSuggestions2 = [];
...
var mochiDtsSpec = (from) => {
  ...
  return or12(_Str_startsWith8("./", bare), _Str_startsWith8("../", bare)) ? `${bare}.mochi` : from;
};
...
var emitDtsFromTypedWith = _curry22(6, (stmts, env, aliases, qualify, runtimeImport, docs) => {
  ...
  const recs = withoutAmbiguousAlias(recordAliasIndex(aliases), aliases, local);
...
`;
  ...
  const imports = _Array_concat9(curry, nsTypeImportsFrom(stmts, body, _Set_fromArray9([]), 0));
...
var mergeMap = _curry23(2, (from, into) => mergeInto2(_Map_keys10(from), from, into));
var exportedTypeNames = (stmts) => _Set_fromArray10(_Array_flatMap4((s) => match22(s).with({ _tag: "SType", exported: true }, ({ name }) => [name]).otherwise(() => []), stmts));
...
var openFor = _curry23(3, (loaded, isEntry, opts) => and15(isEntry, opts.strictEntry) ? opts.open : openMode(loaded.src, opts.open));
...
var depsPublished = _curry23(4, (ctx, stmts, i, path) => match22(_Array_get18(i, stmts)).with({ _tag: "None" }, () => true).with((_v) => {
  const _g = _v;
  return _g._tag === "Some" && _g.value._tag === "SImport";
}, ({ value: { from } }) => and15(_Map_has7(resolveImport2(path, from), ctx.exportsByPath), depsPublished(ctx, stmts, i + 1, path))).with({ _tag: "Some" }, () => depsPublished(ctx, stmts, i + 1, path)).exhaustive());
...
var sameErr = _curry23(2, (a, b) => and15(and15(eq19(a.message, b.message), eq19(a.start, b.start)), eq19(a.end, b.end)));
var mergeRecovered = _curry23(2, (e, checks) => length16(filter7((c) => sameErr(c, e), checks)) > 0 ? checks : _Array_concat10(checks, [e]));
...
var isIdentChar = (c) => match22(_Str_codeAt9(0, c)).with({ _tag: "None" }, () => false).with({ _tag: "Some" }, ({ value: n }) => or13(or13(or13(or13(and15(n >= 48, n <= 57), and15(n >= 65, n <= 90)), and15(n >= 97, n <= 122)), eq19(n, 95)), eq19(n, 36))).exhaustive();
...
var occursAsWordFrom = _curry23(2, (parts, i) => match22(_Array_get18(i, parts)).with({ _tag: "None" }, () => false).with({ _tag: "Some" }, ({ value: after }) => and15(_Option_mapOr(false, endsAtBoundary, _Array_get18(i - 1, parts)), startsAtBoundary(after)) ? true : occursAsWordFrom(parts, i + 1)).exhaustive());
...
`, ts)));
var ownTypesInto = _curry23(3, (stmts, path, acc) => reduce5(_curry23(2, (a, s) => match22(s).with({ _tag: "SType" }, ({ name }) => _Map_has7(name, a.owner) ? { owner: _Map_set11(name, path, a.owner), dups: _Set_add10(name, a.dups), dupNames: _Set_has9(name, a.dups) ? a.dupNames : _Array_append15(name, a.dupNames) } : { owner: _Map_set11(name, path, a.owner), dups: a.dups, dupNames: a.dupNames }).otherwise(() => a)), acc, stmts));
var typeOwnerOf = (graph) => reduce5(_curry23(2, (acc, m) => ownTypesInto(m.stmts, m.path, acc)), { owner: new Map, dups: _Set_fromArray10([]), dupNames: [] }, graph);
var nullaryDeclared = _curry23(2, (name, local) => match22(_Map_get12(name, local)).with({ _tag: "None" }, () => false).with({ _tag: "Some" }, ({ value: info }) => and15(eq19(length16(info.params), 0), length16(info.fields) > 0)).exhaustive());
var addDupMarkers = _curry23(4, (names, local, acc, i) => match22(_Array_get18(i, names)).with({ _tag: "None" }, () => acc).with({ _tag: "Some" }, ({ value: name }) => addDupMarkers(names, local, nullaryDeclared(name, local) ? acc : _Map_set11(`dup.${name}`, { params: ["_"], fields: [], expr: None22 }, acc), i + 1)).exhaustive());
var aliasesForTs = _curry23(3, (merged, local, dupNames) => addDupMarkers(dupNames, local, merged, 0));
var localTypeNames = (stmts) => _Set_fromArray10(_Array_flatMap4((s) => match22(s).with({ _tag: "SType" }, ({ name }) => [name]).otherwise(() => []), stmts));
var groupByOwner = _curry23(2, (names, ctx) => reduce5(_curry23(2, (acc, name) => {
  const owner = _Map_getOr8("", name, ctx.typeOwner);
  return or13(or13(or13(eq19(owner, ctx.importer), _Set_has9(name, ctx.localTypes)), _Set_has9(name, ctx.bound)), not13(occursAsWord(name, ctx.ts))) ? acc : ((spec) => _Map_set11(spec, _Array_append15(name, _Map_getOr8([], spec, acc)), acc))(relSpec2(ctx.importer, owner));
}), new Map, names));
var crossModuleTypeImports = _curry23(4, (ts, importer, localTypes, typeOwner) => {
  ...
  return map14((spec) => `import type { ${_Str_join9(", ", _Array_sort3(_Map_getOr8([], spec, byOwner)))} } from "${spec}";`, _Map_keys10(byOwner));
});
var externBindingsInto = _curry23(4, (stmts, path, env, acc) => reduce5(_curry23(2, (a, s) => match22(s).with({ _tag: "SExtern" }, ({ name, module: hostModule, imported, curried }) => _Str_startsWith9("mochi:", hostModule) ? a : match22(_Map_get12(name, env)).with({ _tag: "None" }, () => a).with({ _tag: "Some" }, ({ value: sc }) => ((dp) => _Map_set11(dp, _Array_append15({ imported, scheme: sc, curried }, _Map_getOr8([], dp, a)), a))(externDtsPath2(path, hostModule))).exhaustive()).otherwise(() => a)), acc, stmts));
...
var externOutputs = (externs) => map14((dp) => ({ path: dp, js: externModuleDts(_Map_getOr8([], dp, externs), noAliases) }), _Map_keys10(externs));
...
var compileGraphTsWith = _curry23(3, (graph, runtimeImport, opts) => {
