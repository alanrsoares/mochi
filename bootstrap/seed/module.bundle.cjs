...
var compileAllRecovering = _curry21(4, (ctx, graph, errors, opts) => match20(graph).with((_v) => {
  ...
  const _g = _v;
  return _g.length >= 1;
}, ([m, ...rest]) => match20(compileOne(ctx, m, true, eq18(length15(rest), 0), opts)).with({ _tag: "Err" }, ({ error: e }) => compileAllRecovering(ctx, rest, _Array_append14(e, errors), opts)).with({ _tag: "Ok" }, ({ value: ctx1 }) => compileAllRecovering(ctx1, rest, errors, opts)).exhaustive()).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
...
var buildModulesWith = _curry21(2, (entry, opts) => _Result_flatMap9((graph) => compileGraphWith(graph, opts), loadGraph(entry)));
...
var buildModulesTsWith = _curry21(3, (entry, runtimeImport, opts) => _Result_flatMap9((graph) => compileGraphTsWith(graph, runtimeImport, opts), loadGraph(entry)));
