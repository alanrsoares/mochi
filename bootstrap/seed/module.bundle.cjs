...
var checkWith = _curry11(3, (stmts, imported, quals) => match10(checkReservedNames(stmts)).with({ _tag: "Some" }, ({ value: e }) => Err7(e)).with({ _tag: "None" }, () => match10(checkCtorFieldVars(stmts)).with({ _tag: "Some" }, ({ value: e }) => Err7(e)).with({ _tag: "None" }, () => match10(checkQualifiedTypeNames(stmts, quals)).with({ _tag: "Some" }, ({ value: e }) => Err7(e)).with({ _tag: "None" }, () => _Result_flatMap6((reg0) => ((reg) => match10(firstSome((s) => match10(s).with({ _tag: "SLet" }, ({ value }) => checkExpr(value, reg)).with({ _tag: "SExpr" }, ({ value }) => checkExpr(value, reg)).otherwise(() => None10), stmts)).with({ _tag: "Some" }, ({ value: e }) => Err7(e)).with({ _tag: "None" }, () => Ok8(stmts)).exhaustive())({ ctors: mergeMissing(_Map_keys3(imported.ctors), imported.ctors, reg0.ctors), types: mergeMissing(_Map_keys3(imported.types), imported.types, reg0.types) }), buildRegistry(stmts))).exhaustive()).exhaustive()).exhaustive());
...
var inferFieldAccess = _curry14(5, (ctx, target, name, sp, st) => _Result_flatMap7(([targetT, st1]) => ((zonked) => match13(zonked).with({ _tag: "TyRecord" }, ({ row }) => match13(lookupField(row, name)).with((_v) => {
  const _g = _v;
  return _g._tag === "Some";
}, ({ value: [ft, optional] }) => Ok9(_tuple7(optional ? tCon("Option", [ft]) : ft, st1))).with({ _tag: "None" }, () => rowEndsEmpty(row) ? Err8(typeErr(`record missing field '${name}'`, sp)) : inferDuckField(targetT, name, sp, st1)).exhaustive()).otherwise(() => inferDuckField(targetT, name, sp, st1)))(zonk(targetT, st1)), inferExpr(ctx, target, st)));
...
var typecheckWith = _curry19(2, (prog, open) => _Result_flatMap8(($env) => Ok10(prog), inferProgram(prog, builtins, namespaces, open)));
...
var visit = _curry21(2, (path, acc) => match20(_Map_get12(path, acc.state)).with({ _tag: "Some", value: "done" }, () => Ok13(acc)).with({ _tag: "Some", value: "loading" }, () => Err10(mErr(`import cycle through '${path}'`))).otherwise(() => ((acc1) => match20(readFile(path)).with({ _tag: "Err" }, () => Err10(mErr(`cannot read module '${path}'`))).with({ _tag: "Ok" }, ({ value: src }) => match20(parseModule(src)).with({ _tag: "Err" }, ({ error: e }) => Err10(e)).with({ _tag: "Ok" }, ({ value: stmts }) => match20(visitAll(importFroms(stmts), path, acc1)).with({ _tag: "Err" }, ({ error: e }) => Err10(e)).with({ _tag: "Ok" }, ({ value: acc2 }) => Ok13({ state: _Map_set11(path, "done", acc2.state), order: _Array_append14({ path, src, stmts }, acc2.order) })).exhaustive()).exhaustive()).exhaustive())({ state: _Map_set11(path, "loading", acc.state), order: acc.order })));
var visitAll = _curry21(3, (froms, importer, acc) => match20(froms).with((_v) => {
  ...
  const _g = _v;
  return _g.length >= 1;
}, ([from, ...rest]) => match20(visit(resolveImport2(importer, from), acc)).with({ _tag: "Err" }, ({ error: e }) => Err10(e)).with({ _tag: "Ok" }, ({ value: acc1 }) => visitAll(rest, importer, acc1)).exhaustive()).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
...
var compileAll = _curry21(3, (ctx, graph, opts) => match20(graph).with((_v) => {
  ...
  const _g = _v;
  return _g.length >= 1;
}, ([m, ...rest]) => match20(compileOne(ctx, m, false, eq18(length15(rest), 0), opts)).with({ _tag: "Err" }, ({ error: e }) => Err10(e)).with({ _tag: "Ok" }, ({ value: ctx1 }) => compileAll(ctx1, rest, opts)).exhaustive()).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
...
var compileAllTs = _curry21(3, (ctx, graph, opts) => match20(graph).with((_v) => _v.length === 0, () => Ok13(_Array_concat10(ctx.outputs, externOutputs(ctx.externs)))).with((_v) => _v.length >= 1, ([m, ...rest]) => match20(compileOneTs(ctx, m, opts)).with({ _tag: "Err" }, ({ error: e }) => Err10(e)).with({ _tag: "Ok" }, ({ value: ctx1 }) => compileAllTs(ctx1, rest, opts)).exhaustive()).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
...
var dtsAll = _curry21(3, (ctx, graph, opts) => match20(graph).with((_v) => _v.length === 0, () => Ok13(ctx.dts)).with((_v) => _v.length >= 1, ([m, ...rest]) => match20(dtsOne(ctx, m, opts)).with({ _tag: "Err" }, ({ error: e }) => Err10(e)).with({ _tag: "Ok" }, ({ value: ctx1 }) => dtsAll(ctx1, rest, opts)).exhaustive()).otherwise(() => {
  throw new Error("non-exhaustive match");
}));
