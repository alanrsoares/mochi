...
var checkAllWith = _curry11(3, (stmts, imported, quals) => match10(checkReservedNames(stmts)).with({ _tag: "Some" }, ({ value: e }) => Err7([e])).with({ _tag: "None" }, () => match10(checkCtorFieldVars(stmts)).with({ _tag: "Some" }, ({ value: e }) => Err7([e])).with({ _tag: "None" }, () => match10(checkQualifiedTypeNames(stmts, quals)).with({ _tag: "Some" }, ({ value: e }) => Err7([e])).with({ _tag: "None" }, () => match10(buildRegistry(stmts)).with({ _tag: "Err" }, ({ error: e }) => Err7([e])).with({ _tag: "Ok" }, ({ value: reg0 }) => ((reg) => ((errors) => eq10(length9(errors), 0) ? Ok8(stmts) : Err7(errors))([...checkLoopsAll(stmts), ..._Array_flatMap2((stmt) => match10(stmt).with({ _tag: "SLet" }, ({ value }) => checkExprs(value, reg)).with({ _tag: "SExpr" }, ({ value }) => checkExprs(value, reg)).otherwise(() => []), stmts)]))({ ctors: mergeMissing(_Map_keys3(imported.ctors), imported.ctors, reg0.ctors), types: mergeMissing(_Map_keys3(imported.types), imported.types, reg0.types) })).exhaustive()).exhaustive()).exhaustive()).exhaustive());
