...
let collapseLambda = (params, body) => switch body {
  | ELambda(params2, body2, _) => collapseLambda(Array.concat(params, params2), body2)
  | _ => (params, body)
}
...
let pctorEntries = (ctor, args, i, ctx) => switch Array.get(i, args) {
  | None => []
  | Some(a) =>
      let s = patSlot(a, ctx) in
      let restEntries = pctorEntries(ctor, args, add(i, 1), ctx) in
      eq(s, "") ? restEntries : Array.prepend(keyedSlot(keyAt(ctor, i, ctx), s), restEntries)
}

let precordEntries = (fields, i, ctx) => switch Array.get(i, fields) {
  | None => []
  | Some(f) =>
      let s = patSlot(f.pat, ctx) in
...
let isListMatch = arms => someOf(a => and(isPList(a.pattern), not(isCatchAll(a.pattern))), arms)
...
let listTail = from =>
  cat([
    "_list(function* () { for (let _i = ", show(from), "; _i < _b.length; _i++) yield _b[_i]; ",
...
let listArmBinds = (elems, i, ctx) => switch Array.get(i, elems) {
  | None => ([], [])
  | Some(el) =>
      let (restParams, restArgs) = listArmBinds(elems, add(i, 1), ctx) in
...
let listMatchLoop = (arms, i, ctx) => switch Array.get(i, arms) {
  | None => ([], "(() => { throw new Error(\"non-exhaustive lazy-list switch\"); })()")
  | Some(a) =>
...
let genListMatch = (scrutinee, arms, ctx) =>
  ...
  cat([
    ...
    Str.join("\n", armLines), "\n  return ", fallback, ";\n})(",
    genExpr(scrutinee, ctx), "[Symbol.iterator]())"
  ])
...
let ctorArgParts = (ctor, args, i, ctx) => switch Array.get(i, args) {
  | None => ([], [])
  | Some(a) =>
      let (restBinds, restLits) = ctorArgParts(ctor, args, add(i, 1), ctx) in
...
let stripAlExt = s => Str.endsWith(".al", s) ? Str.slice(0, sub(Str.length(s), 3), s) : s
...
let genStmt = (s, ctx) => switch s {
  ...
  | SType(_, _, _, _, exported, _) =>
      let decls = genType(s) in
      eq(decls, "")
...
let usesMatchLibArm = a =>
  or(switch a.guard { | Some(g) => usesMatchLib(g) | None => false }, usesMatchLib(a.body))
...
let usesMatchLib = e => switch e {
  ...
  | ELetBind(_, _, value, body, _) => or(usesMatchLib(value), usesMatchLib(body))
...
let exprRefsArmsFrom = (arms, i, ctx, acc) => switch Array.get(i, arms) {
  | None => acc
  | Some(a) =>
      let acc1 = switch a.guard { | Some(g) => exprRefs(g, ctx, acc) | None => acc } in
...
let exprRefsFieldsFrom = (fields, i, ctx, acc) => switch Array.get(i, fields) {
  | None => acc
  | Some(f) => exprRefsFieldsFrom(fields, add(i, 1), ctx, exprRefs(f.value, ctx, acc))
}
let exprRefsEntriesFrom = (entries, i, ctx, acc) => switch Array.get(i, entries) {
  | None => acc
  | Some(en) => exprRefsEntriesFrom(entries, add(i, 1), ctx, exprRefs(en.value, ctx, exprRefs(en.key, ctx, acc)))
}
...
let exprRefs = (e, ctx, acc) => switch e {
  ...
  | ELambda(params, body, _) =>
      let (cparams, cbody) = collapseLambda(params, body) in
      let acc2 = gte(Array.length(cparams), 2) ? Set.add("_curry", acc) : acc in // arity >= 2 lowers `_curry(...)`
      exprRefs(cbody, ctx, acc2)
  | ELetIn(_, _, value, body, _) => exprRefs(body, ctx, exprRefs(value, ctx, acc))
  | ELetBind(_, _, value, body, _) => exprRefs(body, ctx, exprRefs(value, ctx, Set.add("_Result_flatMap", acc)))
  | EPipe(left, right, _) => exprRefs(right, ctx, exprRefs(left, ctx, acc))
  | ETernary(cond, thenE, elseE, _) => exprRefs(elseE, ctx, exprRefs(thenE, ctx, exprRefs(cond, ctx, acc)))
  | EMatch(scrutinee, arms, _) =>
      let acc1 = exprRefs(scrutinee, ctx, acc) in
      let acc2 =
...
let boundNamesFrom = (stmts, i, acc) => switch Array.get(i, stmts) {
  | None => acc
  | Some(s) =>
      boundNamesFrom(stmts, add(i, 1), switch s {
        | SLet(name, _, _, _, _, _) => Set.add(name, acc)
        | SExtern(name, _, _, _, _, _, _) => Set.add(name, acc)
        | SType(_, _, ctors, _, _, _) => Set.union(acc, Set.fromArray(ctors |> map(c => c.name)))
        | SImport(names, _, _) => Set.union(acc, Set.fromArray(names |> map(n => n.name)))
      })
}
...
let refsForStmt = (s, ctx) => switch s {
  ...
  | SType(_, _, ctors, _, _, _) =>
      someOf(c => gte(Array.length(c.fields), 2), ctors) ? Set.add("_curry", Set.fromArray([])) : Set.fromArray([])
  | _ => Set.fromArray([])
}
let collectRefsFrom = (stmts, i, ctx, acc) => switch Array.get(i, stmts) {
  | None => acc
  | Some(s) => collectRefsFrom(stmts, add(i, 1), ctx, Set.union(acc, refsForStmt(s, ctx)))
}
...
let closeRefsFrom = (queue, i, refs, runtimeDeps) => switch Array.get(i, queue) {
  | None => refs
  | Some(r) =>
      let deps = Option.unwrapOr([], Map.get(r, runtimeDeps)) in
...
let seedCtorsFrom = (ctors, i, m) => switch Array.get(i, ctors) {
  | None => m
  | Some(c) => seedCtorsFrom(ctors, add(i, 1), Map.set(c.name, keysOf(c.fields), m))
}
let seedCtorsFromStmts = (stmts, i, m) => switch Array.get(i, stmts) {
  | None => m
  | Some(s) =>
      seedCtorsFromStmts(stmts, add(i, 1), switch s {
...
export let codegen = (stmts, imported, useRuntime, ns, jsDefs, runtimeDeps) =>
  ...
  let needsMatch = someOf(s => switch s { | SLet(_, _, value, _, _, _) => usesMatchLib(value) | _ => false }, stmts) in
