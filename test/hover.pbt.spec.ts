...
// throws, always returns a string or null) for arbitrary input at arbitrary
...
test("hover is total: string | null for any source and offset, never throws", () => {
  fc.assert(
...
test("a numeric literal always hovers as number", () => {
  fc.assert(
    fc.property(fc.nat(), fc.nat({ max: 6 }), (n, pad) => {
      ...
      expect(hoverAt(src, at)).toBe("number");
    }),
  );
});
