...
export const watchEat = (
  ...
  const cell = BOARD_PX / board.cols;
...
/**
 * Paint one frame from board state + particle bag (steps particles once).
 * No-op until the canvas ref is mounted.
 */
export const paintBoard = (
  ...
  drawGrid(ctx, board.cols, board.rows, cell);
  drawFood(ctx, cell, board.food[0], board.food[1]);
  drawSnake(ctx, cell, board.snake);
...
/**
 * rAF while particles remain; each frame re-paints via `boardRef.current`.
...
export const startParticleLoop = (
  ...
  const tick = () => {
    ...
    paintBoard(canvasRef, boardRef.current, particlesRef);
