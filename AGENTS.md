...
- **No attribution trailers** on commits/PRs — no `Co-Authored-By`, “Made with Cursor”,
  “Generated by …”, or other tool credit (`.cursor/rules/no-attribution-trailers.mdc`).
  Issue lines (`Closes #N` / `Refs #N`) are fine. Strip residual injections with
  `git config core.hooksPath .githooks` (or `bun install` → `prepare` /
  `scripts/setup-hooks.ts`). Repo hooks: `.githooks/prepare-commit-msg` (attribution
  strip) and `.githooks/pre-push` (`bun run check`, same as CI). Turn off
  **Cursor Settings → Agents → Attribution** (commit + PR).
